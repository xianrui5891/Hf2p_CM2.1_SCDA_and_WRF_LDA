...
use mpp_domains_mod,          only: mpp_get_compute_domain ! snz
use mpp_domains_mod,          only: mpp_global_field ! snz 20250427 add for AI scda
use mpp_mod,                  only: mpp_error, mpp_pe, mpp_npes, stdlog, stdout,mpp_root_pe !lulv add mpp_root_pe 2019-11-04
use mpp_mod,                  only: mpp_clock_id, mpp_clock_begin, mpp_clock_end
use mpp_mod,                  only: CLOCK_COMPONENT, CLOCK_SUBCOMPONENT, CLOCK_MODULE, CLOCK_ROUTINE
use mpp_memutils_mod,         only: mpp_print_memuse_stats ! snz
...
use oda_types_mod,                only : da_flux_type ! snz

#ifdef ENABLE_ODA    
#ifdef ENABLE_ECDA
  use ada_types_mod, only : scda_type ! snz 20250427 add for AI scda
  use oda_types_mod, only : da_flux_type
  use oda_driver_ecda_mod, only : init_oda, oda, oda_end, assim_frequency,&
      & corr_t_stn, corr_s_stn, corr_t_lfq, corr_s_lfq, corr_t_hfq, corr_s_hfq !lulv
#else
...
#include <ocean_memory.h>
  ...
  character(len=128) :: version = '$Id: ocean_model.F90,v 16.0.2.3.2.1.2.5.10.4.2.9.6.6.6.1.4.2.4.8 2010/03/29 16:50:41 smg Exp $'
  character(len=128) :: tagname = '$Name: mom4p1_riga_29mar2010_smg $'
  ...
  real, dimension(:,:,:), allocatable, save :: corr_t_stn0, corr_s_stn0, corr_t_stn00,corr_s_stn00 !lulv 2019-11-03
#endif
...
#ifdef ENABLE_ODA 
  integer :: sec_snz, day_snz ! snz
  real :: beta_txty                   = 0.0       ! snz
  real :: beta_tf                     = 0.0       ! snz
  real :: beta_qf                     = 0.0       ! snz
  real :: beta_lwsw                   = 0.0       ! snz
  real :: dt_const,dt_const_stn,dt_const_lfq,dt_const_hfq !lulv
#endif
  ...
  namelist /ocean_model_nml/ time_tendency, impose_init_from_restart, reinitialize_thickness,    &
			     ...
			     beta_txty, beta_tf, beta_qf, beta_lwsw, dt_const_stn, dt_const_lfq, &
                             dt_const_hfq ! snz
#else
  namelist /ocean_model_nml/ time_tendency, impose_init_from_restart, reinitialize_thickness,    &
                             baroclinic_split, barotropic_split, surface_height_split,           &
...
! </DESCRIPTION>
!
!!$ subroutine ocean_model_init(Ocean, Ocean_state, Time_init, Time_in, sis_layout) ! snz
subroutine ocean_model_init(Ocean, Ocean_state, Time_init, Time_in) ! snz
    type(ocean_public_type), intent(inout)      :: Ocean
    type(ocean_state_type),          pointer    :: Ocean_state
    type(time_type),       intent(in)           :: Time_init
    type(time_type),       intent(in)           :: Time_in 

!!$    integer, intent(in) :: sis_layout(2) ! snz ida
...
!    da_flux%salt_flux(:,:) = 0.0 ! snz
    ...
    ! initialize diagnostics inside ocean_tracer module, which require density to already by initialized 
    call ocean_tracer_diagnostics_init(Time, Dens, T_diag)  
    ...
    call ocean_vert_mix_init(Grid, Domain, Time, Dens, Time_steps, Ocean_options, T_prog(:), T_diag(:), &
    ...
    call ocean_tracer_advect_init(Grid, Domain, Time, Dens, T_prog(:), dtime_t, have_obc, debug)
    ...
    call ocean_rivermix_init(Grid, Domain, Time, Time_steps, Dens, T_prog(:), Ocean_options, debug)    
...
#ifdef ENABLE_ODA   
    allocate(corr_t_stn0(isd:ied,jsd:jed,1:nk), corr_s_stn0(isd:ied,jsd:jed,1:nk)) !lulv 2019-11-03
    allocate(corr_t_stn00(isd:ied,jsd:jed,1:nk), corr_s_stn00(isd:ied,jsd:jed,1:nk)) !lulv 2019-11-03
    corr_t_stn0(:,:,:) = 0.0 !lulv 2019-11-03
    corr_s_stn0(:,:,:) = 0.0 !lulv 2019-11-03
    corr_t_stn00(:,:,:) = 0.0 !lulv 2019-11-03
    corr_s_stn00(:,:,:) = 0.0 !lulv 2019-11-03
    call init_oda(Time, Domain, Grid, T_prog(:)) ! snz
#endif
...
! snz add one (1) line following
    call mpp_get_compute_domain(Ocean%Domain, is_sfc, ie_sfc, js_sfc, je_sfc)
...
!       time_start_update, Ocean_coupling_time_step)
  subroutine update_ocean_model(Ice_ocean_boundary, Ocean_state, Ocean_sfc, &
       time_start_update, Ocean_coupling_time_step, ocn_end_flag, CBVar) !lulv 20201118,20250427snz
    ...
    type(scda_type), intent(inout) :: CBVar ! snz 20250427 add for AI SCDA
    ...
    if (CBVar%do_scda_flag) then
       T_prog(1)%field(isd:ied,jsd:jed,1,taup1) = T_prog(1)%field(isd:ied,jsd:jed,1,taup1) + &
                                               scda_coeff*CBVar%to_top_adj(isd:ied,jsd:jed) 
       Velocity%u(isd:ied,jsd:jed,1,1,taup1) = Velocity%u(isd:ied,jsd:jed,1,1,taup1) + &
                                               scda_coeff*CBVar%uo_top_adj(isd:ied,jsd:jed) 
       Velocity%u(isd:ied,jsd:jed,1,2,taup1) = Velocity%u(isd:ied,jsd:jed,1,2,taup1) + &
                                               scda_coeff*CBVar%vo_top_adj(isd:ied,jsd:jed) 
       Ext_mode%eta_surf_salt(isd:ied,jsd:jed,taup1) = Ext_mode%eta_surf_salt(isd:ied,jsd:jed,taup1) + &
                                               scda_coeff*CBVar%eta_top_adj(isd:ied,jsd:jed)
       print*,'ocean scda data got merged'
    end if
...
!                                         beta_lwsw*da_flux%salt_flux(ii,jj)
      enddo
    enddo
! snz add the above lines
#endif

       ! obtain surface boundary fluxes using boundary field from coupler
       call mpp_clock_begin(id_sbc)
       call get_ocean_sbc(Time, Ice_ocean_boundary, Thickness, Dens, Ext_mode, T_prog(1:num_prog_tracers), &
       ...
       call get_ocean_bbc(Time, Thickness, Ext_mode, Dens, Velocity, T_prog(1:num_prog_tracers))
...
#ifdef ENABLE_ECDA
    call get_time(Time%model_time, sec_snz, day_snz) ! snz
    if (mod(sec_snz,assim_frequency*3600) == 0) then ! snz

    do j=js_sfc, je_sfc
    jj = j + j_shift
    do i=is_sfc, ie_sfc
    ii = i + i_shift
      da_flux%u_flux(ii,jj) = Ice_ocean_boundary%u_flux(i,j)       ! snz
      da_flux%v_flux(ii,jj) = Ice_ocean_boundary%v_flux(i,j)       ! snz
!      da_flux%t_flux(ii,jj) = Ice_ocean_boundary%t_flux(i,j)       ! snz
!      da_flux%q_flux(ii,jj) = Ice_ocean_boundary%q_flux(i,j)       ! snz
!      da_flux%lw_flux(ii,jj) = Ice_ocean_boundary%lw_flux(i,j)       ! snz
!      da_flux%salt_flux(ii,jj) = Ice_ocean_boundary%salt_flux(i,j)       ! snz
    enddo                                                ! snz
    enddo                                                ! snz
...
!    call mpp_print_memuse_stats('Before oda call')
    corr_t_stn0(isd:ied,jsd:jed,1:nk) = T_prog(1)%field(isd:ied,jsd:jed,1:nk,taup1) !lulv add 2019-11-02
    corr_s_stn0(isd:ied,jsd:jed,1:nk) = T_prog(2)%field(isd:ied,jsd:jed,1:nk,taup1) !lulv add 2019-11-02
    call mpp_update_domains(corr_t_stn0, Domain%domain2d) !lulv add 2019-11-03
    call mpp_update_domains(corr_s_stn0, Domain%domain2d) !lulv add 2019-11-03

    corr_t_stn00(isd:ied,jsd:jed,1:nk) = T_prog(1)%field(isd:ied,jsd:jed,1:nk,taup1) !lulv add 2019-11-02
    corr_s_stn00(isd:ied,jsd:jed,1:nk) = T_prog(2)%field(isd:ied,jsd:jed,1:nk,taup1) !lulv add 2019-11-02
    call mpp_update_domains(corr_t_stn00, Domain%domain2d) !lulv add 2019-11-03
    call mpp_update_domains(corr_s_stn00, Domain%domain2d) !lulv add 2019-11-03

    !call oda(Time, T_prog(1:num_prog_tracers), Velocity, da_flux, Ext_mode)
    call oda(Time, T_prog(1:num_prog_tracers), Velocity, da_flux, Ext_mode,ocn_end_flag) !lulv 20201118

    corr_t_stn0(isd:ied,jsd:jed,1:nk)  = T_prog(1)%field(isd:ied,jsd:jed,1:nk,taup1) - corr_t_stn0(isd:ied,jsd:jed,1:nk) !lulv add 2019-11-03
    corr_s_stn0(isd:ied,jsd:jed,1:nk)  = T_prog(2)%field(isd:ied,jsd:jed,1:nk,taup1) - corr_s_stn0(isd:ied,jsd:jed,1:nk) !lulv add 2019-11-03
    call mpp_update_domains(corr_t_stn0, Domain%domain2d) !lulv add 2019-11-03
    call mpp_update_domains(corr_s_stn0, Domain%domain2d) !lulv add 2019-11-03
if(mod(Time%itt,12)==0) then !lulv 20201013
   if(mpp_pe()==mpp_root_pe()) print*,'mpp_pe(),sum(corr_t_stn)=',mpp_pe(),  sum(corr_t_stn(:,:,1))
   if(mpp_pe()==mpp_root_pe()) print*,'mpp_pe(),sum(corr_t_lfq)=',mpp_pe(),  sum(corr_t_lfq(:,:,1))
   if(mpp_pe()==mpp_root_pe()) print*,'mpp_pe(),sum(corr_t_hfq)=',mpp_pe(),  sum(corr_t_hfq(:,:,1))
endif
!lulv add lines below 2019-11-02

     dt_const=1./12
     if(mpp_pe()==0) print*,'for stn, sec_snz, dt_const_stn, lfq, dt_const= ', sec_snz, dt_const_stn,dt_const_lfq,dt_const
    T_prog(1)%field(isd:ied,jsd:jed,1:nk,taup1) = T_prog(1)%field(isd:ied,jsd:jed,1:nk,taup1) + &
                                                & dt_const*dt_const_stn*corr_t_stn(isd:ied,jsd:jed,1:nk) + &
                                                & dt_const*dt_const_lfq*corr_t_lfq(isd:ied,jsd:jed,1:nk) + &
                                                & dt_const*dt_const_hfq*corr_t_hfq(isd:ied,jsd:jed,1:nk)
    T_prog(2)%field(isd:ied,jsd:jed,1:nk,taup1) = T_prog(2)%field(isd:ied,jsd:jed,1:nk,taup1) + &
                                                & dt_const*dt_const_stn*corr_s_stn(isd:ied,jsd:jed,1:nk) + &
                                                & dt_const*dt_const_lfq*corr_s_lfq(isd:ied,jsd:jed,1:nk) + &
                                                & dt_const*dt_const_hfq*corr_s_hfq(isd:ied,jsd:jed,1:nk)
...
! snz 20250427 add following lines for AI scda

    if (.not. CBVar%otop_data_flag) then
      call mpp_global_field(Domain%domain2d, T_prog(1)%field(:,:,1,taup1),&
                            CBVar%to_top)
      call mpp_global_field(Domain%domain2d, Velocity%u(:,:,1,1,taup1),&
                            CBVar%uo_top)
      call mpp_global_field(Domain%domain2d, Velocity%u(:,:,1,2,taup1),&
                            CBVar%vo_top)
      call mpp_global_field(Domain%domain2d, Ext_mode%eta_surf_salt(:,:,taup1),&
                            CBVar%eta_top)
      CBVar%otop_data_flag = .true.
    end if

    

! end snz 20250427 add following lines for AI scda
...
!
  ...
  stdoutunit=stdout() 
    ...
    write (stdoutunit,'(2x,a)')             Ocean_options%bmf_barotropic

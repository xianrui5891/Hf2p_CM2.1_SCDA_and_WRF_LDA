...
use ocean_tracer_util_mod, only: tracer_psom_chksum, remap_onto_nrho
...
character(len=256) :: version='CVS $Id: ocean_tracer_advect.F90,v 16.0.2.1.20.1.54.1.18.5 2010/03/29 16:53:59 smg Exp $'
...
integer                            :: id_neutral_rho_advect
integer                            :: id_neutral_temp_advect
integer                            :: id_neutral_salt_advect
integer                            :: id_wdian_rho_advect
integer                            :: id_wdian_temp_advect
integer                            :: id_wdian_salt_advect

integer                            :: id_neutral_rho_advect_on_nrho
...
subroutine ocean_tracer_advect_init (Grid, Domain, Time, Dens, T_prog, dtime, obc, debug)
  ...
  id_neutral_rho_advect = register_diag_field ('ocean_model', 'neutral_rho_advect',       &
  ...
  id_neutral_temp_advect = register_diag_field ('ocean_model', 'neutral_temp_advect',     &
                   Grd%tracer_axes(1:3), Time%model_time, 'advection tendency for temp*drhodT', &
                   'rho*rho_dz/sec', missing_value=missing_value, range=(/-1.e18,1.e18/))
  id_neutral_salt_advect = register_diag_field ('ocean_model', 'neutral_salt_advect',     &
  ...
  id_neutral_rho_advect_on_nrho = register_diag_field ('ocean_model',                 &
     'neutral_rho_advect_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,        &
     'update of neutral density from advection as mapped to neutral density surfaces',&
  ...
  id_wdian_rho_advect_on_nrho = register_diag_field ('ocean_model',                          &
     ...
     'dianeutral velocity component due to advection as mapped to neutral density surfaces', &
...
subroutine vert_advect_tracer(Time, Adv_vel, Dens, Thickness, T_prog, Tracer, ntracer, dtime)
  ...
  if(id_neutral_rho_advect         > 0    .or. &
     id_neutral_temp_advect        > 0    .or. &
     id_neutral_salt_advect        > 0    .or. &
     id_wdian_rho_advect           > 0    .or. &
     id_wdian_temp_advect          > 0    .or. &
     id_wdian_salt_advect          > 0    .or. &
     id_neutral_rho_advect_on_nrho > 0    .or. &
     id_wdian_rho_advect_on_nrho   > 0) then  
  ...
  if(.not. advect_sweby_all) then 
      ...
      if(id_neutral_advection > 0) then 
              ...
              do k=1,nk
                 do j=jsc,jec
                    do i=isc,iec
                       ...
                       neutral_temp_advect(i,j,k) = neutral_temp_advect(i,j,k)*Dens%drhodT(i,j,k)/(epsln+temporary)
                    enddo
                 enddo
              enddo
              if(id_neutral_temp_advect > 0) then 
                  used = send_data(id_neutral_temp_advect, neutral_temp_advect(:,:,:),&
                       Time%model_time, rmask=Grd%tmask(:,:,:),                       &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif
                  ...
                  do k=1,nk
                     do j=jsc,jec
                        do i=isc,iec
                           ...
                           wrk1(i,j,k) = Grd%tmask(i,j,k)*neutral_temp_advect(i,j,k)/(epsln+temporary)
                        enddo
                     enddo
                  enddo
                  used = send_data(id_wdian_temp_advect, wrk1(:,:,:), &
                       Time%model_time, rmask=Grd%tmask(:,:,:),       &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif
          endif
              ...
              do k=1,nk
                 do j=jsc,jec
                    do i=isc,iec
                       ...
                       neutral_salt_advect(i,j,k) = neutral_salt_advect(i,j,k)*Dens%drhodS(i,j,k)/(epsln+temporary)
                    enddo
                 enddo
              enddo
              if(id_neutral_salt_advect > 0) then 
                  used = send_data(id_neutral_salt_advect, neutral_salt_advect(:,:,:),&
                       Time%model_time, rmask=Grd%tmask(:,:,:),                       &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif
              if(id_wdian_salt_advect > 0) then 
                  ...
                  do k=1,nk
                     do j=jsc,jec
                        do i=isc,iec
                           temporary   = Dens%drhodz_zt(i,j,k)
                           wrk1(i,j,k) = Grd%tmask(i,j,k)*neutral_salt_advect(i,j,k)/(epsln+temporary)
                        enddo
                     enddo
                  enddo
                  used = send_data(id_wdian_salt_advect, wrk1(:,:,:),&
                       Time%model_time, rmask=Grd%tmask(:,:,:),      &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif
          endif
          ...
          if(ntracer==num_prog_tracers) then 
              ...
              if(id_neutral_rho_advect > 0) then
                     used = send_data(id_neutral_rho_advect,                &
                     neutral_temp_advect(:,:,:)+neutral_salt_advect(:,:,:), &
                     Time%model_time, rmask=Grd%tmask(:,:,:),               &
                     is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif 
                  ...
                  do k=1,nk
                     do j=jsc,jec
                        do i=isc,iec
                           ...
                           (neutral_salt_advect(i,j,k)+neutral_temp_advect(i,j,k))/(epsln+temporary)
                        enddo
                     enddo
                  enddo
                  used = send_data(id_wdian_rho_advect, wrk1(:,:,:), &
                       Time%model_time, rmask=Grd%tmask(:,:,:),      &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif
              if(nrho_diagnostics) then 
                  ...
                  do k=1,nk
                     do j=jsc,jec
                        do i=isc,iec
                           wrk1(i,j,k) = Grd%tmask(i,j,k)*(neutral_salt_advect(i,j,k)+neutral_temp_advect(i,j,k))
                           temporary   = Dens%drhodz_zt(i,j,k)
                           wrk2(i,j,k) = wrk1(i,j,k)/(epsln+temporary)
                        enddo
                     enddo
                 enddo
                 if(id_neutral_rho_advect_on_nrho > 0) then
                     nrho_work(:,:,:) = 0.0
                     call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
                     used = send_data (id_neutral_rho_advect_on_nrho, nrho_work(:,:,:),&
                          Time%model_time,                                             &
              ...
              endif ! endif for nrho_diagnostics

          endif     ! endif for ntracer==num_prog_tracers

      endif         ! endif for id_neutral_advection > 0
...
subroutine advect_tracer_sweby_all(Time, Adv_vel, Dens, T_prog, Thickness, dtime)
  ...
  do n=1,num_prog_tracers 
     ...
     ! neutral_rho and wdian diagnostics 
     if(id_neutral_advection > 0) then 
         ...
         endif     ! endif for n==num_prog_tracers

...
!  </DATA> 
!
!  <DATA NAME="nrho_diagnostics=" TYPE="logical">
!  For computing some neutral density diagnostics, which require mapping to 
!  neutral density surfaces. Default nrho_diagnostics=.false. 
!  </DATA> 
!
...
use ocean_parameters_mod,       only: missing_value, sec_in_yr_r, rho0, cp_ocean
...
character(len=256) :: version='CVS $Id: ocean_tracer.F90,v 1.1.2.2.2.43.20.6.18.3.2.3.10.6.20.1.4.2.6.10 2010/03/29 16:53:59 smg Exp $'
character(len=256) :: tagname='Tag $Name: mom4p1_riga_29mar2010_smg $'
...
logical :: nrho_diagnostics=.false.
...
integer                            :: id_neutral_rho_frazil
integer                            :: id_wdian_rho_frazil

integer                            :: id_neutral_rho_tendency_on_nrho
integer                            :: id_wdian_rho_tendency_on_nrho 
integer                            :: id_neutral_rho_smooth_on_nrho
integer                            :: id_wdian_rho_smooth_on_nrho
integer                            :: id_neutral_rho_pme_on_nrho
integer                            :: id_wdian_rho_pme_on_nrho
integer                            :: id_neutral_rho_frazil_on_nrho
...
namelist /ocean_tracer_nml/ debug_this_module, zero_tendency, zero_tracer_source, write_a_restart,   &
                            ...
                            compute_tmask_limit_on, nrho_diagnostics      
...
subroutine update_ocean_tracer (Time, Dens, Adv_vel, Thickness, pme, diff_cbt, &
  ...
  if (zero_tendency) then
     ...
     if(frazil_heating_before_vphysics) then 
         ...
         call compute_frazil_heating(Time, Thickness, Dens, pressure_at_depth, &
  ...
  if (id_neutral_rho_tendency         > 0 .or. id_wdian_rho_tendency         > 0 .or.  &
      id_neutral_rho_tendency_on_nrho > 0 .or. id_wdian_rho_tendency_on_nrho > 0) then
      wrk1(:,:,:) = 0.0
      wrk2(:,:,:) = 0.0
      wrk3(:,:,:) = 0.0
      wrk4(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               ...
               wrk3(i,j,k) = Grd%tmask(i,j,k)*dtimer*(wrk1(i,j,k)+wrk2(i,j,k))/(temporary+epsln)

               temporary   = Dens%drhodz_zt(i,j,k)
               wrk4(i,j,k) = wrk3(i,j,k)/(temporary+epsln)

            enddo
         enddo
      enddo
      if(id_neutral_rho_tendency > 0) then
           used = send_data(id_neutral_rho_tendency,wrk3(:,:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,:),              &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif 
      if(id_wdian_rho_tendency > 0) then 
          used = send_data(id_wdian_rho_tendency, wrk4(:,:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,:),        &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif
      if(nrho_diagnostics) then 
          if(id_neutral_rho_tendency_on_nrho > 0) then
              nrho_work(:,:,:) = 0.0
              call remap_onto_nrho (Time, Dens, wrk3, nrho_work) 
              used = send_data (id_neutral_rho_tendency_on_nrho, nrho_work(:,:,:),&
                   Time%model_time,                                               &
                   is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
          endif
          if(id_wdian_rho_tendency_on_nrho > 0) then
              nrho_work(:,:,:) = 0.0
              call remap_onto_nrho (Time, Dens, wrk4, nrho_work) 
  ...
  if (id_neutral_rho_smooth         > 0 .or. id_wdian_rho_smooth         > 0 .or.  &
      id_neutral_rho_smooth_on_nrho > 0 .or. id_wdian_rho_smooth_on_nrho > 0) then
      wrk1(:,:,:) = 0.0
      wrk2(:,:,:) = 0.0
      wrk3(:,:,:) = 0.0
      wrk4(:,:,:) = 0.0
      ...
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               ...
               temporary   = Dens%drhodz_zt(i,j,k)
               wrk4(i,j,k) = wrk3(i,j,k)/(epsln+temporary)
            enddo
         enddo
      enddo
      if(id_neutral_rho_smooth > 0) then
          used = send_data(id_neutral_rho_smooth,wrk3(:,:,:), &
                 Time%model_time, rmask=Grd%tmask(:,:,:),     &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif
      if(id_wdian_rho_smooth > 0) then 
          used = send_data(id_wdian_rho_smooth, wrk4(:,:,:), &
                 Time%model_time, rmask=Grd%tmask(:,:,:),    &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif
      if(nrho_diagnostics) then 
         if(id_neutral_rho_smooth_on_nrho > 0) then
            nrho_work(:,:,:) = 0.0
            call remap_onto_nrho (Time, Dens, wrk3, nrho_work) 
            used = send_data (id_neutral_rho_smooth_on_nrho, nrho_work(:,:,:),&
            Time%model_time,                                                  &
            is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
         endif             
  ...
  if (id_neutral_rho_pme         > 0 .or. id_wdian_rho_pme         > 0 .or.  &
      id_neutral_rho_pme_on_nrho > 0 .or. id_wdian_rho_pme_on_nrho > 0) then
      wrk1(:,:,:) = 0.0
      wrk2(:,:,:) = 0.0
      k=1
      do j=jsc,jec
         do i=isc,iec
            ...
            temporary   = Thickness%rho_dzt(i,j,k,tau)
            wrk1(i,j,k) = wrk1(i,j,k)/(epsln+temporary)
            temporary   = Dens%drhodz_zt(i,j,k)
            wrk2(i,j,k) = wrk1(i,j,k)/(epsln+temporary)
         enddo
      enddo
      ...
      if(nrho_diagnostics) then 
         if(id_neutral_rho_pme_on_nrho > 0) then
            nrho_work(:,:,:) = 0.0
            call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
            used = send_data (id_neutral_rho_pme_on_nrho, nrho_work(:,:,:),&
            Time%model_time,                                               &
            is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
         endif             
         if(id_wdian_rho_pme_on_nrho > 0) then
            nrho_work(:,:,:) = 0.0
            call remap_onto_nrho (Time, Dens, wrk2, nrho_work) 
  ...
  if(index_frazil > 0) then 
      ...
      if (id_neutral_rho_frazil         > 0 .or. id_wdian_rho_frazil         > 0 .or.  &
          id_neutral_rho_frazil_on_nrho > 0 .or. id_wdian_rho_frazil_on_nrho > 0) then
          wrk1(:,:,:) = 0.0
          wrk2(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
                do i=isc,iec
                   wrk1(i,j,k) = Grd%tmask(i,j,k)*dtimer &
                                *T_diag(index_frazil)%field(i,j,k)*cp_oceanr*Dens%drhodT(i,j,k)
                   temporary   = Thickness%rho_dzt(i,j,k,tau)
                   wrk1(i,j,k) = wrk1(i,j,k)/(epsln+temporary)
                   temporary   = Dens%drhodz_zt(i,j,k)
                   wrk2(i,j,k) = wrk1(i,j,k)/(epsln+temporary)
             ...
             enddo
          enddo
          if(id_neutral_rho_frazil > 0) then
              used = send_data(id_neutral_rho_frazil,wrk1(:,:,:), &
                   Time%model_time, rmask=Grd%tmask(:,:,:),       &
                   is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
          endif
          if(id_wdian_rho_frazil > 0) then 
              used = send_data(id_wdian_rho_frazil, wrk2(:,:,:), &
                   Time%model_time, rmask=Grd%tmask(:,:,:),      &
                   is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
          endif
          if(nrho_diagnostics) then 
              if(id_neutral_rho_frazil_on_nrho > 0) then
                  nrho_work(:,:,:) = 0.0
                  call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
                  used = send_data (id_neutral_rho_frazil_on_nrho, nrho_work(:,:,:),&
                       Time%model_time,                                             &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
              endif
  ...
  ! send some further diagnostic tracers to diag manager 
  do n=1,num_diag_tracers
...
!
! <DESCRIPTION>
! Initialize a work array that is needed for diagnostics that 
! remap to neutral density space. 
!
! </DESCRIPTION>
!
subroutine ocean_tracer_diagnostics_init(Time, Dens, T_diag)
  ...
  id_neutral_rho_tendency = register_diag_field ('ocean_model','neutral_rho_tendency', & 
       Grd%tracer_axes(1:3), Time%model_time,'total time tendency of neutral density', &
       '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1e10/))

  id_neutral_rho_tendency_on_nrho = register_diag_field ('ocean_model',               &
       'neutral_rho_tendency_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,    &
       'total time tendency of neutral density as mapped to neutral density surfaces',&
  ...
  id_wdian_rho_tendency_on_nrho = register_diag_field ('ocean_model',                                   &
       ...
       'dianeutral velocity component due to total time tendency as mapped to neutral density surfaces',&
  ...
  id_neutral_rho_smooth_on_nrho = register_diag_field ('ocean_model',                                  &
       'neutral_rho_smooth_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                       &
       'time tendency of neutral density from eta/pbot smoother as mapped to neutral density surfaces',&
       '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_smooth_on_nrho = register_diag_field ('ocean_model',                                   &
       ...
       'dianeutral velocity component due to eta/pbot smoother as mapped to neutral density surfaces',&
       'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_neutral_rho_pme_on_nrho = register_diag_field ('ocean_model',                                &
       'neutral_rho_pme_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                     &
       'time tendency of neutral density from eta/pbot pme as mapped to neutral density surfaces',&
       '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_pme_on_nrho = register_diag_field ('ocean_model',                                 &
       ...
       'dianeutral velocity component due to eta/pbot pme as mapped to neutral density surfaces',&
       'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_frazil = register_diag_field ('ocean_model', 'neutral_rho_frazil', &
       Grd%tracer_axes(1:3), Time%model_time,                                       &
       'update of neutral density from frazil formation',                           &
       '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_frazil_on_nrho = register_diag_field ('ocean_model',                          &
       'neutral_rho_frazil_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,               &
       'update of neutral density from frazil formation as mapped to neutral density surfaces',&
       '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_wdian_rho_frazil = register_diag_field ('ocean_model', 'wdian_rho_frazil', &
       Grd%tracer_axes(1:3), Time%model_time,                                   &
       'dianeutral velocity component due to frazil formation',                 &
       'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_frazil_on_nrho = register_diag_field ('ocean_model',                                  &
       ...
       'dianeutral velocity component due to frazil formation as mapped to neutral density surfaces',&
       'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

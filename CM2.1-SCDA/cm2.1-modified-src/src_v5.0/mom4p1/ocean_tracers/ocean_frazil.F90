...
integer :: id_eta_tendency_frazil=-1
...
subroutine ocean_frazil_init (Domain, Grid, Time, Time_steps, Ocean_options, &
  ...
  id_eta_tendency_frazil = register_diag_field('ocean_model','eta_tendency_frazil',  &
       Grd%tracer_axes(1:2),Time%model_time,'sea level tendency from frazil heating',&
...
! </DESCRIPTION>
!
subroutine compute_frazil_heating (Time, Thickness, Dens, pressure_at_depth, T_prog, T_diag)
  ...
  ! compute contribution to sea level from frazil heating. 
  ! assume all frazil heating is in the surface 
  if (id_eta_tendency_frazil > 0) then 
      wrk1_2d = 0.0
      do j=jsc,jec
         do i=isc,iec
            rhosfc_inv   = Grd%tmask(i,j,1)/(epsln+Dens%rho(i,j,1,tau))
            alphasfc     = -rhosfc_inv*Dens%drhodT(i,j,1)
            wrk1_2d(i,j) = alphasfc*rhosfc_inv*T_diag(index_frazil)%field(i,j,1)*dtimer*cp_ocean_r
         enddo
      enddo
      used = send_data(id_eta_tendency_frazil, wrk1_2d(:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,1),          &
           is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif

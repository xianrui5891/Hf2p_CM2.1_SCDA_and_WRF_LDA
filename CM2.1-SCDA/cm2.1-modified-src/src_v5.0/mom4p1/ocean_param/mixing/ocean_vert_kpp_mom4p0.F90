...
integer                            :: id_neutral_rho_kpp_nloc        =-1
integer                            :: id_wdian_rho_kpp_nloc          =-1
integer                            :: id_neutral_rho_kpp_nloc_on_nrho=-1
...
character(len=256) :: version=&
     '$Id: ocean_vert_kpp_mom4p0.F90,v 16.0.2.2.40.1.12.1.18.4 2010/03/29 16:52:38 smg Exp $'
...
namelist /ocean_vert_kpp_mom4p0_nml/ use_this_module, shear_instability, double_diffusion,  &
                              ...
                              wsfc_combine_runoff_calve, nrho_diagnostics 
...
! </DESCRIPTION>
!
subroutine ocean_vert_kpp_mom4p0_init (Grid, Domain, Time, Time_steps, Dens, &
                                       T_prog, T_diag, ver_coordinate)
...
!-----------------------------------------------------------------------
  ...
  ! for nonlocal impacts on neutral density and dianeutral velocity component  
  id_neutral_rho_kpp_nloc = register_diag_field ('ocean_model', &
          'neutral_rho_kpp_nloc',                               &
  ...
  id_neutral_rho_kpp_nloc_on_nrho = register_diag_field ('ocean_model',                 &
     'neutral_rho_kpp_nloc_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,        &
     'update of neutral density from KPP nolocal as mapped to neutral density surfaces',&
  ...
  id_wdian_rho_kpp_nloc_on_nrho = register_diag_field ('ocean_model',                           &
     ...
     'dianeutral velocity component due to KPP nonlocal as mapped to neutral density surfaces', &
...
!-----------------------------------------------------------------------
       ...
       if (non_local_kpp) then
           ...
           do n=1,num_prog_tracers
              ...
              ! neutral density and dianeutral velocity component 
              if(n==index_temp) then 
                  do k=1,nk
                     do j=jsc,jec
                        do i=isc,iec
           ...
           ! diagnostics for neutral density and dianeutral advection component 
           if(id_neutral_rho_kpp_nloc > 0) then
               wrk1(:,:,:) = 0.0
               do k=1,nk
                  do j=jsc,jec
                     do i=isc,iec
                  ...
                  enddo
               enddo
               used = send_data(id_neutral_rho_kpp_nloc, wrk1(:,:,:), &
                      Time%model_time, rmask=Grd%tmask(:,:,:),        &
                      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
           ...
           if(id_wdian_rho_kpp_nloc > 0) then
               ...
               do k=1,nk
                  ...
                  enddo
               enddo
               used = send_data(id_wdian_rho_kpp_nloc, wrk1(:,:,:), &
                      Time%model_time, rmask=Grd%tmask(:,:,:),      &
                      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
           endif

           if(nrho_diagnostics) then 
               if(id_neutral_rho_kpp_nloc_on_nrho > 0) then
                   wrk1(:,:,:)      = 0.0
                   nrho_work(:,:,:) = 0.0
                   do k=1,nk
                      do j=jsc,jec
                   ...
                   used = send_data(id_neutral_rho_kpp_nloc_on_nrho, nrho_work(:,:,:), &
                        Time%model_time,                                               &

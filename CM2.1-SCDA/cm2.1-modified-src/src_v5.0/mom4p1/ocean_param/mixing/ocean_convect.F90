...
! for diagnostics 
integer :: id_neutral_rho_convect
integer :: id_wdian_rho_convect
integer :: id_neutral_rho_convect_on_nrho
...
character(len=128) :: version=&
  '$Id: ocean_convect.F90,v 16.0.108.1.18.4 2010/03/29 16:51:42 smg Exp $'
...
subroutine ocean_convect_init (Grid, Domain, Time, Dens, T_prog, Ocean_options, dtime)
  ...
  id_neutral_rho_convect = register_diag_field ('ocean_model', 'neutral_rho_convect', &
                           Grd%tracer_axes(1:3), Time%model_time,                     &
                           'update of neutral density from convective adjustment',    &
  ...
  id_wdian_rho_convect = register_diag_field ('ocean_model', 'wdian_rho_convect',     & 
                         Grd%tracer_axes(1:3), Time%model_time,                       &
                         'dianeutral velocity component due to convective adjustment',&
                         'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_convect_on_nrho = register_diag_field ('ocean_model',                            &
     'neutral_rho_convect_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                   &
     'update of neutral density from convective adjustment as mapped to neutral density surfaces',&
                           '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_convect_on_nrho = register_diag_field ('ocean_model',                                    &
     ...
     'dianeutral velocity component due to convective adjustment as mapped to neutral density surfaces',&
     'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine convection_diag (Time, Thickness, T_prog, Dens)
  ...
  ! send diagnostics for update of neutral density from convective adjustment 
  if (id_neutral_rho_convect > 0         .or. id_wdian_rho_convect > 0 .or.  &
      id_neutral_rho_convect_on_nrho > 0 .or. id_wdian_rho_convect_on_nrho > 0) then
      wrk1(:,:,:) = 0.0
      wrk2(:,:,:) = 0.0
      wrk3(:,:,:) = 0.0
      wrk4(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               temporary   =  Thickness%rho_dzt(i,j,k,taup1)/(epsln+Thickness%rho_dzt(i,j,k,tau))
               wrk1(i,j,k) =  dtimer*(T_prog(index_temp)%field(i,j,k,taup1) - T_prog(index_temp)%wrk1(i,j,k)) &
                              *Dens%drhodT(i,j,k)*temporary
               wrk2(i,j,k) =  dtimer*(T_prog(index_salt)%field(i,j,k,taup1) - T_prog(index_salt)%wrk1(i,j,k)) &
      ...
      if(id_neutral_rho_convect > 0) then 
          used = send_data (id_neutral_rho_convect, wrk3(:,:,:),&
               Time%model_time,rmask=Grd%tmask(:,:,:),          &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif

      if (id_wdian_rho_convect > 0) then 
          used = send_data (id_wdian_rho_convect, wrk4(:,:,:), &
               Time%model_time,rmask=Grd%tmask(:,:,:),         &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif

      if(nrho_diagnostics) then 
          if (id_neutral_rho_convect_on_nrho > 0) then
              nrho_work(:,:,:) = 0.0
              call remap_onto_nrho (Time, Dens, wrk3, nrho_work) 
              used = send_data (id_neutral_rho_convect_on_nrho, nrho_work(:,:,:),&
                   Time%model_time,                                              &
  ...
  endif   ! endif for id_neutral_rho_convect,         id_wdian_rho_convect, 
          !           id_neutral_rho_convect_on_nrho, id_wdian_rho_convect_on_nrho

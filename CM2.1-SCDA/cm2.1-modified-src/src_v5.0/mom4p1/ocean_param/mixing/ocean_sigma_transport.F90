...
integer :: id_neutral_rho_sigma_on_nrho=-1
...
character(len=128) :: version=&
     '$Id: ocean_sigma_transport.F90,v 16.0.42.1.54.1.18.4 2010/03/29 16:52:38 smg Exp $'
...
subroutine ocean_sigma_transport_init(Grid, Domain, Time, Dens, T_prog, Ocean_options, &
  ...
  id_neutral_rho_sigma_on_nrho = register_diag_field ('ocean_model',                        &
     'neutral_rho_sigma_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,               &
     'update of neutral density from sigma transport as mapped to neutral density surfaces',&
                           '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_sigma_on_nrho = register_diag_field ('ocean_model',                                 &
     ...
     'dianeutral velocity component due to sigma transport as mapped to neutral density surfaces', &
     'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine sigma_transport (Time, Thickness, Dens, T_prog, Adv_vel, bott_blthick)
  ...
  if(nrho_diagnostics) then 
      if (id_neutral_rho_sigma_on_nrho > 0) then
          wrk2(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_sigma_on_nrho, nrho_work(:,:,:),&
               Time%model_time,                                            &

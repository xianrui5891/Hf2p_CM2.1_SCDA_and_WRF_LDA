...
!  </DATA> 
!
!  <DATA NAME="diagnose_cabbeling_thermob" TYPE="logical">
!  For diagnosing some terms related to cabbeling and thermobaricity.  
!  Default is diagnose_cabbeling_thermob=.false.
!  </DATA> 
...
!  </DATA> 
!
!  <DATA NAME="diagnose_eta_tendency_gm" TYPE="logical">
!  For diagnosing the contribution to steric sea level 
!  time tendency associated with the GM scheme. 
!  Default is diagnose_eta_tendency_gm=.false.
!  </DATA> 
!  <DATA NAME="smooth_eta_tendency_gm" TYPE="logical">
!  For smoothing the diagnosed contribution to steric sea level 
!  time tendency associated with the GM scheme. 
!  Default is smooth_eta_tendency_gm=.true.
!  </DATA> 
!</NAMELIST>
...
use ocean_workspace_mod,  only: wrk1, wrk2, wrk3, wrk4, wrk1_v, wrk1_2d, wrk2_2d, wrk3_2d
...
public compute_eta_tendency_gm
...
integer :: id_clock_eta_tendency_gm
...
integer :: id_wdianeutral_cabbeling =-1
integer :: id_wdianeutral_thermob   =-1
integer :: id_eta_tendency_gm       =-1
...
logical :: diagnose_cabbeling_thermob=.false. 
...
logical :: diagnose_eta_tendency_gm=.false. 
logical :: smooth_eta_tendency_gm  =.true.
...
character(len=128) :: version=&
     '$Id: ocean_nphysics_util.F90,v 1.1.2.28.14.5.18.2.18.2.30.1.4.1.14.10 2010/03/29 16:52:38 smg Exp $'
...
namelist /ocean_nphysics_util_nml/ debug_this_module, nphysics_util_zero_init,      &
          ...
          diagnose_cabbeling_thermob, smax_grad_gamma_scalar,                       &
          epsln_drhodz_diagnostics, wdianeutral_smooth,                             &
          diagnose_eta_tendency_gm, smooth_eta_tendency_gm 
...
subroutine ocean_nphysics_util_init(Grid, Domain, Time, Time_steps, Dens, T_prog, &
  ...
  if(diagnose_cabbeling_thermob) then 
     write(stdoutunit,'(a)') &
      '==>Note: diagnose_cabbeling_thermob==.true., diagnosing tendencies from cabbeling and thermobaricity.'
  ...
  if(diagnose_eta_tendency_gm) then 
     write(stdoutunit,'(a)') &
      '==>Note: diagnose_eta_tendency_gm==.true., diagnosing steric sea level tendency from Gent-McWilliams.'
  ...
  id_clock_eta_tendency_gm       = mpp_clock_id('(Ocean neutral: eta_tendency_gm)',grain=CLOCK_ROUTINE)
  ...
  id_wdianeutral_cabbeling= -1          
  id_wdianeutral_cabbeling= register_diag_field ('ocean_model', 'wdianeutral_cabbeling',&
                  Grd%tracer_axes(1:3), Time%model_time,                                &
                  'dianeutral velocity component due to cabbeling', 'm/s',              &  
                  missing_value=missing_value, range=(/-1e10,1.e10/))

  id_wdianeutral_thermob= -1          
  id_wdianeutral_thermob= register_diag_field ('ocean_model', 'wdianeutral_thermob',&
                  Grd%tracer_axes(1:3), Time%model_time,                            &
                  'dianeutral velocity component due to thermobaricity', 'm/s',     &  
                  missing_value=missing_value, range=(/-1e10,1.e10/))
  ...
  id_wdian_cabbeling_on_nrho= register_diag_field ('ocean_model', 'wdian_cabbeling_on_nrho',       &
     ...
     'dianeutral velocity component due to cabbeling as mapped to neutral density surfaces', 'm/s',&  
     missing_value=missing_value, range=(/-1e10,1.e10/))

  id_wdian_thermob_on_nrho= -1          
  id_wdian_thermob_on_nrho= register_diag_field ('ocean_model', 'wdian_thermob_on_nrho',                &
     Dens%neutralrho_axes(1:3), Time%model_time,                                                        &
     'dianeutral velocity component due to thermobaricity as mapped to neutral density surfaces', 'm/s',&  
     missing_value=missing_value, range=(/-1e10,1.e10/))

  id_eta_tendency_gm= -1          
  id_eta_tendency_gm= register_diag_field ('ocean_model', 'eta_tendency_gm',&
                  Grd%tracer_axes(1:2), Time%model_time,                    &
                  'steric sea level tendency from Gent-McWilliams', 'm/s',  &
                  missing_value=missing_value, range=(/-1e10,1.e10/))
...
!#######################################################################
! <SUBROUTINE NAME="compute_eta_tendency_gm">
!
! <DESCRIPTION>
! Diagnose contribution to steric sea level evolution arising from 
...
! 
! Subroutine history: 
! Feb2010 version 1.0: Stephen.Griffies@noaa.gov 
! 
! </DESCRIPTION>
!
subroutine compute_eta_tendency_gm (Time, Thickness, Dens,   &
                                       dtheta_dx, dtheta_dy, dtheta_dz, & 
                                       dsalt_dx , dsalt_dy , dsalt_dz,  & 
                                       drhodzh,                         &
  ...
  real, dimension(isd:,jsd:,0:),      intent(in) :: dsalt_dz
  ...
  integer :: extended_mask, xmask, ymask 
  real    :: absslope, absslopex, absslopey
  real    :: drho_dz, drho_dz_rhor2, divergence

  if(.not. diagnose_eta_tendency_gm) return 

  if (.not.module_is_initialized) then 
    call mpp_error(FATAL, &
    '==>Error ocean_neutral_util_mod (compute_eta_tendency_gm): module needs initialization ')
  endif 

  call mpp_clock_begin(id_clock_eta_tendency_gm)
  ...
  do k=1,nk
     ...
     do j=jsd,jed
        do i=isd,ied
           dslopex_dz(i,j,k) = Grd%tmask(i,j,kp1)*dzwtr(i,j,k) &
                               *(agm_rho_slopex(i,j,k)-agm_rho_slopex(i,j,kp1))
           dslopey_dz(i,j,k) = Grd%tmask(i,j,kp1)*dzwtr(i,j,k) &
                               *(agm_rho_slopey(i,j,k)-agm_rho_slopey(i,j,kp1))
        enddo
     enddo
  enddo
  ...
  call calc_gradx_gamma_scalar(Thickness,Dens%drhodT(:,:,:),Dens%drhodS(:,:,:), &
                               dtheta_dx,dtheta_dz,dsalt_dx,dsalt_dz,           &
                               drhodzh,dtwedyt,dzwtr,delqc,                     &
  ...
  if (id_eta_tendency_gm > 0) then 
      if(smooth_eta_tendency_gm) then 
          call mpp_update_domains (wrk1_2d(:,:), Dom%domain2d) 
          wrk1_2d(:,:) = S2D(wrk1_2d(:,:))
      endif
      used = send_data (id_eta_tendency_gm, wrk1_2d(:,:),&
             Time%model_time, rmask=Grd%tmask(:,:,1),    &
             is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif

  call mpp_clock_end(id_clock_eta_tendency_gm)


end subroutine compute_eta_tendency_gm
! </SUBROUTINE> NAME="compute_eta_tendency_gm"
...
! 
! Subroutine history: 
! Jan2010 version 1.0: initial coding by Stephen.Griffies@noaa.gov  
...
subroutine cabbeling_thermob_tendency (Time, Thickness, T_prog, Dens,   &
  ...
  real    :: drho_dz, dz_drho_rho, strat_mask
  ...
  call pressure_derivs(Thickness, Dens%pressure_at_depth(:,:,:), dzwtr)
  ...
  call calc_grady_gamma_scalar(Thickness,Dens%drhodT(:,:,:),Dens%drhodS(:,:,:), &
                               dtheta_dy,dtheta_dz,dsalt_dy,dsalt_dz,           &
                               drhodzh,dxtdtsn,dzwtr,delqc,                     &
  ...
  wrk1_v(:,:,:,:) = 0.0   ! dianeutral velocity component (m/s) from (cabbeling,thermobaricity)
  do k=1,nk
     kp1=min(nk,k+1)
     do j=jsc,jec
        do i=isc,iec
           ...
           wrk1(i,j,k)     = extended_mask*aredi_array(i,j,k)*cabbeling_param(i,j,k) &
            *(gradx_gamma_temp(i,j,k)*gradx_gamma_temp(i,j,k)+grady_gamma_temp(i,j,k)*grady_gamma_temp(i,j,k))
           wrk2(i,j,k)     = extended_mask*aredi_array(i,j,k)*thermobaric_param(i,j,k) &
           ...
           dz_drho_rho     = Grd%tmask(i,j,k)*strat_mask*Dens%rho(i,j,k,tau)/(epsln+drho_dz)
           wrk1_v(i,j,k,1) = wrk1(i,j,k)*dz_drho_rho
           wrk1_v(i,j,k,2) = wrk2(i,j,k)*dz_drho_rho
  ...
  if (id_cabbeling_tend > 0) then 
      used = send_data (id_cabbeling_tend, wrk1(:,:,:), &
             Time%model_time, rmask=Grd%tmask(:,:,:),   &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if (id_thermobaric_tend > 0) then 
      used = send_data (id_thermobaric_tend, wrk2(:,:,:), &
             Time%model_time, rmask=Grd%tmask(:,:,:),     &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if (id_cabbeling_speed > 0) then 
      used = send_data (id_cabbeling_speed, wrk3(:,:,:), &
             Time%model_time, rmask=Grd%tmask(:,:,:),    &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if (id_thermobaric_speed > 0) then 
      used = send_data (id_thermobaric_speed, wrk4(:,:,:), &
             Time%model_time, rmask=Grd%tmask(:,:,:),      &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if (id_cabbeling_tend_intz > 0) then 
      used = send_data (id_cabbeling_tend_intz, wrk1_2d(:,:),&
             Time%model_time, rmask=Grd%tmask(:,:,1),        &
             is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif
  if (id_thermobaric_tend_intz > 0) then 
      used = send_data (id_thermobaric_tend_intz, wrk2_2d(:,:),&
             Time%model_time, rmask=Grd%tmask(:,:,1),          &
             is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif

  if (id_wdianeutral_cabbeling > 0) then 
      ...
      used = send_data (id_wdianeutral_cabbeling, wrk1_v(:,:,:,1),&
           Time%model_time, rmask=Grd%tmask(:,:,:),               &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if (id_wdian_cabbeling_on_nrho > 0) then 
      ...
      nrho_work(:,:,:) = 0.0
      call remap_onto_nrho (Time, Dens, wrk1_v(:,:,:,1), nrho_work) 
      used = send_data (id_wdian_cabbeling_on_nrho, nrho_work,&
           Time%model_time,                                   &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif


  if (id_wdianeutral_thermob > 0) then 
      if(wdianeutral_smooth) then 
      ...
      used = send_data (id_wdianeutral_thermob, wrk1_v(:,:,:,2),&
             Time%model_time, rmask=Grd%tmask(:,:,:),           &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if (id_wdian_thermob_on_nrho > 0) then 
      ...
      nrho_work(:,:,:) = 0.0
      call remap_onto_nrho (Time, Dens, wrk1_v(:,:,:,2), nrho_work) 
      used = send_data (id_wdian_thermob_on_nrho, nrho_work,&
           Time%model_time,                                 &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
...
! </DESCRIPTION>
!
subroutine pressure_derivs(Thickness, pressure, dzwtr)
  ...
  do k=1,nk
     ...
     do j=jsd,jed
        do i=isd,ied
           deriv_z_press(i,j,k) = Grd%tmask(i,j,kp1)*dzwtr(i,j,k)*(pressure(i,j,k)-pressure(i,j,kp1))
        enddo
     enddo
  enddo

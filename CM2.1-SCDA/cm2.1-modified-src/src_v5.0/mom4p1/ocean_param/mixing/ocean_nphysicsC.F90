...
integer                             :: id_neutral_rho_gm        ! tendency of neutral density from GM
integer                             :: id_wdian_rho_gm          ! contribution to wdian from GM
...
integer, dimension(:), allocatable  :: id_flux_x_gm             ! i-directed heat flux from GM
integer, dimension(:), allocatable  :: id_flux_y_gm             ! j-directed heat flux from GM
...
character(len=128) :: version=&
     '$Id: ocean_nphysicsC.F90,v 1.1.2.28.4.1.2.7.30.1.2.1.16.1.32.1.4.1.14.11 2010/03/29 16:52:37 smg Exp $'
...
subroutine compute_ndiffusion(Time, Thickness, Dens, T_prog)
  ...
  do n=1,num_prog_tracers
     ...
     if(id_neutral_physics_ndiffuse(n) > 0) then 
        used = send_data (id_neutral_physics_ndiffuse(n), T_prog(n)%wrk1(:,:,:)*T_prog(n)%conversion, &
...
subroutine compute_gmskewsion(Time, Thickness, Dens, T_prog)
  ...
  do n=1,num_prog_tracers
     ...
     if(id_neutral_physics_gm(n) > 0) then 
        used = send_data (id_neutral_physics_gm(n), T_prog(n)%wrk1(:,:,:)*T_prog(n)%conversion, &
               Time%model_time, rmask=Grd%tmask(:,:,:),                                         &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
     endif
     ...
     if(id_flux_x_gm(n) > 0) then 
        used = send_data (id_flux_x_gm(n), -1.0*T_prog(n)%conversion*flux_x(n)%field(:,:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,:),                                      &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
     endif
     if(id_flux_y_gm(n) > 0) then 
        used = send_data (id_flux_y_gm(n), -1.0*T_prog(n)%conversion*flux_y(n)%field(:,:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,:),                                      &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
     endif
     if(id_flux_x_gm_int_z(n) > 0) then 
        ...
        do k=1,nk
            tmp_flux(isc:iec,jsc:jec) = tmp_flux(isc:iec,jsc:jec) + flux_x(n)%field(isc:iec,jsc:jec,k)
        enddo        
        used = send_data (id_flux_x_gm_int_z(n), -1.0*T_prog(n)%conversion*tmp_flux(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),                                   &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
     endif
     if(id_flux_y_gm_int_z(n) > 0) then 
        ...
        do k=1,nk
            tmp_flux(isc:iec,jsc:jec) = tmp_flux(isc:iec,jsc:jec) + flux_y(n)%field(isc:iec,jsc:jec,k)
        enddo        
        used = send_data (id_flux_y_gm_int_z(n), -1.0*T_prog(n)%conversion*tmp_flux(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),                                   &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
     endif

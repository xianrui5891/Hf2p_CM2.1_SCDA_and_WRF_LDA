...
!  </DATA> 
!
!  <DATA NAME="diagnose_gm_redi" TYPE="logical">
!  For diagnosing the separate tendencies and fluxes from GM and Redi, rather 
!  than their combined effects.  The approach is to recompute some of the 
!  terms associated JUST with neutral diffusion, and then get the GM contribution
!  as a GM = TOTAL - NDIFFUSE.  This approach saves calculations and memory.  
!  This diagnostic is NOT available with neutral_physics_simple=.true.
!  This diagnostic adds memory and calculations to the module.
!  Default diagnose_gm_redi=.false.
!  </DATA> 
!
!</NAMELIST>
...
use ocean_nphysics_util_mod,   only: cabbeling_thermob_tendency, compute_eta_tendency_gm
...
integer                             :: id_neutral_rho_ndiffuse
integer                             :: id_neutral_rho_gm
integer                             :: id_wdian_rho_nphysics
integer                             :: id_wdian_rho_ndiffuse
integer                             :: id_wdian_rho_gm

integer                             :: id_neutral_rho_ndiffuse_on_nrho
integer                             :: id_neutral_rho_gm_on_nrho
integer                             :: id_wdian_rho_ndiffuse_on_nrho
...
real, dimension(:,:,:), allocatable                   :: nrho_work !work array on neutral density space
...
character(len=128) :: version=&
     '$Id: ocean_nphysicsA.F90,v 1.1.2.20.22.5.18.1.18.1.32.1.4.1.14.9 2010/03/29 16:52:37 smg Exp $'
...
! for diagnosing the effects from GM and Redi separately 
...
namelist /ocean_nphysicsA_nml/ use_this_module, debug_this_module,          &
          ...
          transport_units, diagnose_gm_redi
...
subroutine ocean_nphysicsA_init(Grid, Domain, Time, Time_steps, Thickness, Dens, T_prog, &
  ...
  if(diagnose_gm_redi) then 
     if(neutral_physics_simple) then 
        call mpp_error(NOTE, &
        ' ==> ocean_nphysicsA_mod: diagnostics from diagnose_gm_redi=.true. are not available w/ neutral_physics_simple=.true.')
     endif 
     write(stdoutunit,'(1x,a)') &
     ' ==>NOTE: ocean_nphysicA has diagnose_gm_redi=.true. to diagnose GM and Redi. It adds much to memory requirements.'   
  ...
  id_neutral_rho_ndiffuse = register_diag_field ('ocean_model', 'neutral_rho_ndiffuse',       &
                          Grd%tracer_axes(1:3), Time%model_time,                              &
                          'update of neutral density from explicit in time neutral diffusion',&
                          '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  id_neutral_rho_gm = -1
  id_neutral_rho_gm = register_diag_field ('ocean_model', 'neutral_rho_gm',&
                         Grd%tracer_axes(1:3), Time%model_time,            &
                         'update of neutral density from GM stirring',     &
                         '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_ndiffuse = register_diag_field ('ocean_model', 'wdian_rho_ndiffuse',                 &
                          Grd%tracer_axes(1:3), Time%model_time,                                    &
                          'dianeutral velocity component due to explicit in time neutral diffusion',&
                          'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  id_wdian_rho_gm = -1
  id_wdian_rho_gm = register_diag_field ('ocean_model', 'wdian_rho_gm',      &
                          Grd%tracer_axes(1:3), Time%model_time,             &
                          'dianeutral velocity component due to GM stirring',&
                          'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_ndiffuse_on_nrho = -1
  id_neutral_rho_ndiffuse_on_nrho = register_diag_field ('ocean_model', 'neutral_rho_ndiffuse_on_nrho',     &
       Dens%neutralrho_axes(1:3), Time%model_time,                                                          &
       'update of neutral density from explicit in time neutral diffusion as mapped to neutral rho surface',&
       '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_gm_on_nrho = -1
  id_neutral_rho_gm_on_nrho = register_diag_field ('ocean_model', 'neutral_rho_gm_on_nrho',            &
                         Dens%neutralrho_axes(1:3), Time%model_time,                                   &
                         'update of neutral density from GM stirring as mapped to neutral rho surface',&
                         '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_wdian_rho_ndiffuse_on_nrho = -1
  id_wdian_rho_ndiffuse_on_nrho = register_diag_field ('ocean_model', 'wdian_rho_ndiffuse_on_nrho',           &
   Dens%neutralrho_axes(1:3), Time%model_time,                                                                &
   'dianeutral velocity component due to explicit in time neutral diffusion as mapped to neutral rho surface',&
   'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_wdian_rho_gm_on_nrho = -1
  id_wdian_rho_gm_on_nrho = register_diag_field ('ocean_model', 'wdian_rho_gm_on_nrho',   &
      Dens%neutralrho_axes(1:3), Time%model_time,                                         &
      'dianeutral velocity component due to GM stirring as mapped to neutral rho surface',&
      'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine nphysicsA (Time, Thickness, Dens, rho, T_prog, &
  ...
  dTdx(index_salt)%field(:,:,:), dTdy(index_salt)%field(:,:,:), dTdz(index_salt)%field(:,:,:), &
  drhodzh, dxtdtsn, dtwedyt, dzwtr, delqc, aredi_array)

  call compute_eta_tendency_gm(Time, Thickness, Dens,                                          &
  dTdx(index_temp)%field(:,:,:), dTdy(index_temp)%field(:,:,:), dTdz(index_temp)%field(:,:,:), &
  dTdx(index_salt)%field(:,:,:), dTdy(index_salt)%field(:,:,:), dTdz(index_salt)%field(:,:,:), &
  drhodzh, dxtdtsn, dtwedyt, dzwtr, delqc, tensor_31, tensor_32, agm_array)
...
subroutine fx_flux_diag(Time, Thickness, T_prog, k)
  ...
  do nn=1,num_prog_tracers
     ...
     do j=jsc,jec
        do i=isc-1,iec
           flux_x_redi(nn)%field(i,j,k) = Grd%dxter(i,j)*(sumz(i,j,0)+sumz(i,j,1))
        enddo
     enddo
  enddo
...
subroutine fy_flux_diag(Time, Thickness, T_prog, k)
  ...
  do nn=1,num_prog_tracers
     ...
     do j=jsc-1,jec
        do i=isc,iec
           flux_y_redi(nn)%field(i,j,k) = Grd%dytnr(i,j)*(sumz(i,j,0)+sumz(i,j,1))
        enddo
     enddo
  enddo
...
subroutine nphysics_diagnostics(Time, T_prog, Dens, Thickness)
  ...
  if (id_k33_explicit > 0) then 
      used = send_data (id_k33_explicit, rho0r*K33_explicit(:,:,:), &
                        Time%model_time, rmask=Grd%tmask(:,:,:),    &
                        is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  ...
  if(diagnose_gm_redi) then 
      ...
      do nn=1,num_prog_tracers
         ...
         if(id_flux_x_ndiffuse_int_z(nn) > 0) then 
             tmp_flux = 0.0
             do k=1,nk
                tmp_flux(isc:iec,jsc:jec) = tmp_flux(isc:iec,jsc:jec) + flux_x_redi(nn)%field(isc:iec,jsc:jec,k)
             enddo
             used = send_data (id_flux_x_ndiffuse_int_z(nn), -1.0*T_prog(nn)%conversion*tmp_flux(:,:), &
                  Time%model_time, rmask=Grd%tmask(:,:,1),                                             &
                  is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
         endif
         if(id_flux_y_ndiffuse_int_z(nn) > 0) then 
             tmp_flux = 0.0
             do k=1,nk
                tmp_flux(isc:iec,jsc:jec) = tmp_flux(isc:iec,jsc:jec) + flux_y_redi(nn)%field(isc:iec,jsc:jec,k)
             enddo
             used = send_data (id_flux_y_ndiffuse_int_z(nn), -1.0*T_prog(nn)%conversion*tmp_flux(:,:), &
                  Time%model_time, rmask=Grd%tmask(:,:,1),                                             &
                  is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
         endif
         ...
         if(id_neutral_physics_gm(nn) > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec
                      wrk1(i,j,k) = T_prog(nn)%wrk1(i,j,k) - tendency_redi(nn)%field(i,j,k)
                   enddo
                enddo
             enddo
             used = send_data (id_neutral_physics_gm(nn), &
         ...
         if(id_flux_x_gm(nn) > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec
                      wrk1(i,j,k) = flux_x(nn)%field(i,j,k) - flux_x_redi(nn)%field(i,j,k)
                   enddo
                enddo
             enddo
             used = send_data (id_flux_x_gm(nn),          &
         ...
         if(id_flux_y_gm(nn) > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec
                      wrk1(i,j,k) = flux_y(nn)%field(i,j,k) - flux_y_redi(nn)%field(i,j,k)
                   enddo
                enddo
             enddo
             used = send_data (id_flux_y_gm(nn),          &
                  -1.0*T_prog(nn)%conversion*wrk1(:,:,:), &
                  Time%model_time, rmask=Grd%tmask(:,:,:),&
                  is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
         endif
         ...
         if(id_flux_x_gm_int_z(nn) > 0) then 
             ...
             tmp_flux = 0.0
             do k=1,nk
                tmp_flux(isc:iec,jsc:jec) = tmp_flux(isc:iec,jsc:jec) + wrk1(isc:iec,jsc:jec,k)
             enddo
             used = send_data (id_flux_x_gm_int_z(nn), -1.0*T_prog(nn)%conversion*tmp_flux(:,:), &
                  Time%model_time, rmask=Grd%tmask(:,:,1),                                       &
                  is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
         endif
         ...
         if(id_flux_y_gm_int_z(nn) > 0) then 
             ...
             do k=1,nk
                tmp_flux(isc:iec,jsc:jec) = tmp_flux(isc:iec,jsc:jec) + wrk1(isc:iec,jsc:jec,k)
             enddo
             used = send_data (id_flux_y_gm_int_z(nn), -1.0*T_prog(nn)%conversion*tmp_flux(:,:), &
                  Time%model_time, rmask=Grd%tmask(:,:,1),                                       &
                  is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
         endif
          ...
          do k=1,nk
             do j=jsc,jec
                do i=isc,iec
                   temporary   = Thickness%rho_dzt(i,j,k,tau)
                   wrk1(i,j,k) = Grd%tmask(i,j,k)*                             &
                        (drhodT(i,j,k)*tendency_redi(index_temp)%field(i,j,k)  &
                        +drhodS(i,j,k)*tendency_redi(index_salt)%field(i,j,k)) &
                        /(epsln+temporary)
                enddo
             enddo
          enddo
      ...
      if(id_neutral_rho_ndiffuse_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_ndiffuse_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                                &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif

      if(id_neutral_rho_gm > 0) then 
          wrk1(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
                do i=isc,iec
                   temporary   = Thickness%rho_dzt(i,j,k,tau)
                   tmp1 = T_prog(index_temp)%wrk1(i,j,k) - tendency_redi(index_temp)%field(i,j,k)
                   tmp2 = T_prog(index_salt)%wrk1(i,j,k) - tendency_redi(index_salt)%field(i,j,k)
                   wrk1(i,j,k) = Grd%tmask(i,j,k)*(drhodT(i,j,k)*tmp1+drhodS(i,j,k)*tmp2) &
                                 /(epsln+temporary)
                enddo
             enddo
          enddo
          used = send_data (id_neutral_rho_gm, wrk1(:,:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,:),    &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif

      if(id_neutral_rho_gm_on_nrho > 0) then 
         wrk1(:,:,:)      = 0.0
         nrho_work(:,:,:) = 0.0
         do k=1,nk
             do j=jsc,jec
          ...
          enddo
          call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
          used = send_data (id_neutral_rho_gm_on_nrho, nrho_work(:,:,:),&
               Time%model_time,                                         &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
      ...
      if(id_wdian_rho_ndiffuse_on_nrho > 0) then 
         wrk1(:,:,:)      = 0.0
         nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          enddo
          call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
          used = send_data (id_wdian_rho_ndiffuse_on_nrho, nrho_work(:,:,:),&
               Time%model_time,                                             &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
      ...
      if(id_wdian_rho_gm > 0) then 
          ...
          do k=1,nk
             ...
             enddo
          enddo
          used = send_data (id_wdian_rho_gm, wrk1(:,:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,:),  &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif

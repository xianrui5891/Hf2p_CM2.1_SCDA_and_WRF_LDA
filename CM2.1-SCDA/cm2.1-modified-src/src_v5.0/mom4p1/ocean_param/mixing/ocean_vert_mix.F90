...
use ocean_tracer_util_mod,     only: remap_onto_nrho
...
integer :: id_eta_tendency_vdiffuse =-1
...
integer :: id_neutral_rho_vdiffuse_on_nrho  =-1
integer :: id_neutral_rho_k33_on_nrho       =-1
integer :: id_neutral_rho_diff_cbt_on_nrho  =-1
integer :: id_neutral_rho_diff_wave_on_nrho =-1
integer :: id_neutral_rho_diff_drag_on_nrho =-1
...
character(len=128) :: version = &
     '$Id: ocean_vert_mix.F90,v 1.1.2.3.2.45.24.2.18.1.2.1.16.1.32.1.4.1.6.7 2010/03/29 16:52:38 smg Exp $'
character (len=128) :: tagname = &
     '$Name: mom4p1_riga_29mar2010_smg $'
...
! </DESCRIPTION>
!
subroutine ocean_vert_mix_init (Grid, Domain, Time, Dens, Time_steps, Ocean_options, T_prog, T_diag, &
  ...
  elseif(vert_mix_scheme == 'kpp') then
    ...
    call ocean_vert_kpp_init (Grid, Domain, Time, Time_steps, Dens, T_prog, T_diag, &
  ...
  ! register tendency for neutral density and dianeutral velocity component 
  ! as mapped to neutral density surfaces 
  id_neutral_rho_vdiffuse_on_nrho = register_diag_field ('ocean_model',                 &
          'neutral_rho_vdiffuse_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,   &
          'vertical diffusion of neutral density as mapped to neutral density surfaces', &
          '(kg/m^3)/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_neutral_rho_k33_on_nrho = register_diag_field ('ocean_model',                              &
          'neutral_rho_k33_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                &
          'vertical diffusion of neutral density from K33 as mapped to neutral density surfaces',&
          '(kg/m^3)/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_neutral_rho_diff_cbt_on_nrho = register_diag_field ('ocean_model',                              &
          'neutral_rho_diff_cbt_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                &
          'vertical diffusion of neutral density from diff_cbt as mapped to neutral density surfaces',&
          '(kg/m^3)/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_neutral_rho_diff_wave_on_nrho = register_diag_field ('ocean_model',                                  &
          'neutral_rho_diff_wave_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                    &
          'vertical diffusion of neutral density from diff_cbt_wave as mapped to neutral density surfaces',&
          '(kg/m^3)/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_neutral_rho_diff_drag_on_nrho = register_diag_field ('ocean_model',                                   &
          'neutral_rho_diff_drag_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                     &
          'vertical diffusion of neutral density from diff_cbt_drag as mapped to neutral density surfaces',&
          '(kg/m^3)/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  ...
  id_wdian_rho_vdiffuse_on_nrho = register_diag_field ('ocean_model',                          &
   ...
   'dianeutral velocity component due to vert diffusion as mapped to neutral density surfaces',&
   'm/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_wdian_rho_k33_on_nrho = register_diag_field ('ocean_model',                                         &
    'wdian_rho_k33_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                                 &
    'dianeutral velocity component due to vert diffusion from K33 as mapped to neutral density surfaces',&
    'm/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_wdian_rho_diff_cbt_on_nrho = register_diag_field ('ocean_model',                                         &
    ...
    'dianeutral velocity component due to vert diffusion from diff_cbt as mapped to neutral density surfaces',&
    'm/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_wdian_rho_diff_wave_on_nrho = register_diag_field ('ocean_model',                                         &
    ...
    'dianeutral velocity component due to vert diffusion from diff_wave as mapped to neutral density surfaces',&
    'm/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_wdian_rho_diff_drag_on_nrho = register_diag_field ('ocean_model',                                         &
    ...
    'dianeutral velocity component due to vert diffusion from diff_drag as mapped to neutral density surfaces',&
    'm/sec',missing_value=missing_value, range=(/-1e10,1e10/))


  ! register sea level tendency arising from vertical diffusion 
  id_eta_tendency_vdiffuse = register_diag_field ('ocean_model', 'eta_tendency_vdiffuse', &
          Grid%tracer_axes(1:2), Time%model_time,                                         &
          'sea level tendency from vertical diffusion',                                   &
          'm/s',missing_value=missing_value, range=(/-1e10,1e10/))
...
subroutine vert_mix_coeff(Time, Thickness, Velocity, T_prog, T_diag, Dens, &
  ...
  if(id_eta_tendency_vdiffuse > 0) then 
      ...
      do k=1,nk
         ...
         enddo
      enddo
      used = send_data (id_eta_tendency_vdiffuse, wrk1_2d(:,:), &
  ...
  if(aidif==0.0) then   
      if(ntracer==index_temp) then 
          ...
          do k=1,nk
             do j=jsc,jec
                do i=isc,iec
                   temporary                    = Thickness%rho_dzt(i,j,k,tau)
                   neutral_temp_vdiffuse(i,j,k) = Dens%drhodT(i,j,k)*wrk1(i,j,k)/(epsln+temporary) 
                enddo
             enddo
          enddo
      endif
      if(ntracer==index_salt) then 
          ...
          do k=1,nk
             do j=jsc,jec
                do i=isc,iec
                   temporary                    = Thickness%rho_dzt(i,j,k,tau)
                   neutral_salt_vdiffuse(i,j,k) = Dens%drhodS(i,j,k)*wrk1(i,j,k)/(epsln+temporary) 
                enddo
             enddo
          enddo
      endif
      ...
      if(aidif==0.0) then 
          ...
          if(id_wdian_rho_vdiffuse > 0) then 
              ...
              do k=1,nk
                 do j=jsc,jec
                    do i=isc,iec
                       temporary   = Dens%drhodz_zt(i,j,k)
...
subroutine vert_diffuse_implicit(diff_cbt, index_salt, Time, Thickness, Dens, T_prog)
  ...
  integer :: i, j, k, n, nt2
  integer :: km1, kp1
  integer :: neutral_rho_wdian_diag=-1 
  real    :: temporary
  real    :: dTdz
  ...
  do n=1,num_prog_tracers
     ...
     ! diagnose the update due to the effects from neutral diffusion K33 term 
     ...
     ! diagnose the update due to the effects from diff_cbt (does not include K33)
     ...
     ! save some terms from temperature to diagnose the update to neutral density 
     if(n==index_temp) then 

         ! full vertical diffusion tendency (diff_cbt + K33)
         if(id_neutral_rho_vdiffuse         > 0 .or. id_wdian_rho_vdiffuse         > 0 .or. &
            id_neutral_rho_vdiffuse_on_nrho > 0 .or. id_wdian_rho_vdiffuse_on_nrho > 0) then 
             neutral_temp_vdiffuse=0.0
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec
                      neutral_temp_vdiffuse(i,j,k) = dtime_tr*Dens%drhodT(i,j,k) &
                           *(T_prog(n)%field(i,j,k,taup1)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         ! update from k33
         if(id_neutral_rho_k33         > 0 .or. id_wdian_rho_k33         > 0 .or. &
            id_neutral_rho_k33_on_nrho > 0 .or. id_wdian_rho_k33_on_nrho > 0) then 
             wrk1_2d=0.0
             wrk2   =0.0
             do k=1,nk
                do j=jsc,jec
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk1_v(i,j,k,1) = dtime_tr*Dens%drhodT(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         ! update from diff_cbt (does not include K33)
         if(id_neutral_rho_diff_cbt         > 0 .or. id_wdian_rho_diff_cbt         > 0 .or. &
            id_neutral_rho_diff_cbt_on_nrho > 0 .or. id_wdian_rho_diff_cbt_on_nrho > 0) then 
             wrk2=0.0
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk2_v(i,j,k,1) = dtime_tr*Dens%drhodT(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         ! update from baroclinic wave drag diffusivity 
         if(id_neutral_rho_diff_wave         > 0 .or. id_wdian_rho_diff_wave         > 0 .or. &
            id_neutral_rho_diff_wave_on_nrho > 0 .or. id_wdian_rho_diff_wave_on_nrho > 0) then 
             wrk1_2d=0.0
             wrk2   =0.0
             do k=1,nk
                do j=jsc,jec
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk3_v(i,j,k,1) = dtime_tr*Dens%drhodT(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         ! update from barotropic wave drag diffusivity 
         if(id_neutral_rho_diff_drag         > 0 .or. id_wdian_rho_diff_drag         > 0 .or. &
            id_neutral_rho_diff_drag_on_nrho > 0 .or. id_wdian_rho_diff_drag_on_nrho > 0) then 
             wrk1_2d=0.0
             wrk2   =0.0
             do k=1,nk
                do j=jsc,jec
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk4_v(i,j,k,1) = dtime_tr*Dens%drhodT(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         if(id_neutral_rho_vdiffuse         > 0 .or. id_wdian_rho_vdiffuse         > 0 .or. &
            id_neutral_rho_vdiffuse_on_nrho > 0 .or. id_wdian_rho_vdiffuse_on_nrho > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec
                      neutral_salt_vdiffuse(i,j,k) = dtime_tr*Dens%drhodS(i,j,k) &
                           *(T_prog(n)%field(i,j,k,taup1)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         if(id_neutral_rho_k33         > 0 .or. id_wdian_rho_k33         > 0 .or. &
            id_neutral_rho_k33_on_nrho > 0 .or. id_wdian_rho_k33_on_nrho > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk1_v(i,j,k,2) = dtime_tr*Dens%drhodS(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         if(id_neutral_rho_diff_cbt         > 0 .or. id_wdian_rho_diff_cbt         > 0 .or. &
            id_neutral_rho_diff_cbt_on_nrho > 0 .or. id_wdian_rho_diff_cbt_on_nrho > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk2_v(i,j,k,2) = dtime_tr*Dens%drhodS(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         if(id_neutral_rho_diff_wave         > 0 .or. id_wdian_rho_diff_wave         > 0 .or. &
            id_neutral_rho_diff_wave_on_nrho > 0 .or. id_wdian_rho_diff_wave_on_nrho > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk3_v(i,j,k,2) = dtime_tr*Dens%drhodS(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
         ...
         if(id_neutral_rho_diff_drag         > 0 .or. id_wdian_rho_diff_drag         > 0 .or. &
            id_neutral_rho_diff_drag_on_nrho > 0 .or. id_wdian_rho_diff_drag_on_nrho > 0) then 
             ...
             do k=1,nk
                do j=jsc,jec
                   do i=isc,iec  
                      wrk4_v(i,j,k,2) = dtime_tr*Dens%drhodS(i,j,k) &
                           *(wrk2_vmix(i,j,k)-wrk1_vmix(i,j,k))
                   enddo
                enddo
             enddo
  ...
  ! send diagnostics for neutral density evolution and dianeutral velocity component  
  if(id_neutral_temp_vdiffuse           > 0 .or. &
       id_neutral_salt_vdiffuse         > 0 .or. &
       id_neutral_rho_vdiffuse          > 0 .or. &
       id_neutral_rho_vdiffuse_on_nrho  > 0 .or. &
       id_neutral_rho_k33               > 0 .or. &
       id_neutral_rho_k33_on_nrho       > 0 .or. &
       id_neutral_rho_diff_cbt          > 0 .or. &
       id_neutral_rho_diff_cbt_on_nrho  > 0 .or. &
       id_neutral_rho_diff_wave         > 0 .or. &
       id_neutral_rho_diff_wave_on_nrho > 0 .or. &
       id_neutral_rho_diff_drag         > 0 .or. &
       id_neutral_rho_diff_drag_on_nrho > 0 .or. &
       id_wdian_temp_vdiffuse           > 0 .or. &
       id_wdian_salt_vdiffuse           > 0 .or. &
       id_wdian_rho_vdiffuse            > 0 .or. &
       id_wdian_rho_vdiffuse_on_nrho    > 0 .or. &
       id_wdian_rho_k33                 > 0 .or. &
       id_wdian_rho_k33_on_nrho         > 0 .or. &
       id_wdian_rho_diff_cbt            > 0 .or. &
       id_wdian_rho_diff_cbt_on_nrho    > 0 .or. &
       id_wdian_rho_diff_wave           > 0 .or. &
       id_wdian_rho_diff_wave_on_nrho   > 0 .or. &
       id_wdian_rho_diff_drag           > 0 .or. &
       id_wdian_rho_diff_drag_on_nrho   > 0) then 
      wrk4(:,:,:) = 0.0
      ! scaling to account for implicit calculation 
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               temporary   = Thickness%rho_dzt(i,j,k,tau)
               wrk4(i,j,k) = Thickness%rho_dzt(i,j,k,taup1)/(epsln+temporary)
            enddo
         enddo
      enddo
  endif
  ...
  if(id_neutral_temp_vdiffuse > 0) then 
      used = send_data(id_neutral_temp_vdiffuse, wrk4(:,:,:)*neutral_temp_vdiffuse(:,:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,:),                                        &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if(id_neutral_salt_vdiffuse > 0) then 
      used = send_data(id_neutral_salt_vdiffuse, wrk4(:,:,:)*neutral_salt_vdiffuse(:,:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,:),                                        &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  ...
  if(id_neutral_rho_vdiffuse > 0) then 
      used = send_data(id_neutral_rho_vdiffuse,                                     &
           wrk4(:,:,:)*(neutral_temp_vdiffuse(:,:,:)+neutral_salt_vdiffuse(:,:,:)), &
           Time%model_time, rmask=Grd%tmask(:,:,:),                                 &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if(id_neutral_rho_vdiffuse_on_nrho > 0) then 
      wrk1(:,:,:)      = 0.0
      nrho_work(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               wrk1(i,j,k) = wrk4(i,j,k)*(neutral_temp_vdiffuse(i,j,k)+neutral_salt_vdiffuse(i,j,k))
            enddo
         enddo
      enddo
      call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
      used = send_data (id_neutral_rho_vdiffuse_on_nrho, nrho_work(:,:,:), &
           Time%model_time,                                                &
  ...
  if(id_neutral_rho_k33 > 0) then 
      used = send_data(id_neutral_rho_k33,               &
           wrk4(:,:,:)*(wrk1_v(:,:,:,1)+wrk1_v(:,:,:,2)),&
           Time%model_time, rmask=Grd%tmask(:,:,:),      &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if(id_neutral_rho_k33_on_nrho > 0) then 
      wrk1(:,:,:)      = 0.0
      nrho_work(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               wrk1(i,j,k) = wrk4(i,j,k)*(wrk1_v(i,j,k,1)+wrk1_v(i,j,k,2))
            enddo
         enddo
      enddo
      call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
      used = send_data (id_neutral_rho_k33_on_nrho, nrho_work(:,:,:), &
           Time%model_time,                                           &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
  ...
  if(id_neutral_rho_diff_cbt > 0) then 
      used = send_data(id_neutral_rho_diff_cbt,          &
           wrk4(:,:,:)*(wrk2_v(:,:,:,1)+wrk2_v(:,:,:,2)),&
           Time%model_time, rmask=Grd%tmask(:,:,:),      &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if(id_neutral_rho_diff_cbt_on_nrho > 0) then 
      wrk1(:,:,:)      = 0.0
      nrho_work(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               wrk1(i,j,k) = wrk4(i,j,k)*(wrk2_v(i,j,k,1)+wrk2_v(i,j,k,2))
            enddo
         enddo
      enddo
      call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
      used = send_data (id_neutral_rho_diff_cbt_on_nrho, nrho_work(:,:,:), &
           Time%model_time,                                                &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
  ...
  if(id_neutral_rho_diff_wave > 0) then 
      used = send_data(id_neutral_rho_diff_wave,         &
           wrk4(:,:,:)*(wrk3_v(:,:,:,1)+wrk3_v(:,:,:,2)),&
           Time%model_time, rmask=Grd%tmask(:,:,:),      &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if(id_neutral_rho_diff_wave_on_nrho > 0) then 
      wrk1(:,:,:)      = 0.0
      nrho_work(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               wrk1(i,j,k) = wrk4(i,j,k)*(wrk3_v(i,j,k,1)+wrk3_v(i,j,k,2))
            enddo
         enddo
      enddo
      call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
      used = send_data (id_neutral_rho_diff_wave_on_nrho, nrho_work(:,:,:), &
           Time%model_time,                                                 &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
  ...
  if(id_neutral_rho_diff_drag > 0) then 
      used = send_data(id_neutral_rho_diff_drag,         &
           wrk4(:,:,:)*(wrk4_v(:,:,:,1)+wrk4_v(:,:,:,2)),&
           Time%model_time, rmask=Grd%tmask(:,:,:),      &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  if(id_neutral_rho_diff_drag_on_nrho > 0) then 
      wrk1(:,:,:)      = 0.0
      nrho_work(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               wrk1(i,j,k) = wrk4(i,j,k)*(wrk4_v(i,j,k,1)+wrk4_v(i,j,k,2))
            enddo
         enddo
      enddo
      call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
      used = send_data (id_neutral_rho_diff_drag_on_nrho, nrho_work(:,:,:), &
           Time%model_time,                                                 &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
  endif
  ...
  if(id_wdian_temp_vdiffuse > 0) then 
      ...
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               temporary   = Dens%drhodz_zt(i,j,k)
               wrk1(i,j,k) = Grd%tmask(i,j,k)*wrk4(i,j,k)*neutral_temp_vdiffuse(i,j,k)/(epsln+temporary)
            enddo
         enddo
      enddo
      used = send_data(id_wdian_temp_vdiffuse, wrk1(:,:,:), &
  ...
  if(id_wdian_salt_vdiffuse > 0) then 
      ...
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               temporary   = Dens%drhodz_zt(i,j,k)
               wrk1(i,j,k) = Grd%tmask(i,j,k)*wrk4(i,j,k)*neutral_salt_vdiffuse(i,j,k)/(epsln+temporary)
            enddo
         enddo
      enddo
      used = send_data(id_wdian_salt_vdiffuse, wrk1(:,:,:), &
  ...
  if(id_wdian_rho_vdiffuse > 0) then 
      ...
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
               temporary   = Dens%drhodz_zt(i,j,k)
               wrk1(i,j,k) = Grd%tmask(i,j,k)*wrk4(i,j,k)* &
                 (neutral_temp_vdiffuse(i,j,k)+neutral_salt_vdiffuse(i,j,k))/(epsln+temporary)
            enddo
         enddo
      enddo
...
subroutine vert_friction_implicit(visc_cbu, visc_cbu_form_drag, Time, Thickness, Velocity)
  ...
  if (aidif /= 0.0) then  
      ...
      if(Velocity%bmf_implicit) then 
          ...
          call invtri_bmf (wrk1_v(:,:,:,1), Velocity%smf(:,:,1), Velocity%gamma(:,:),&
               rho0*(visc_cbu(:,:,:)+visc_cbu_form_drag(:,:,:,1)), dtime_u, Grd%kmu, &
               Grd%umask, wrk1, Thickness%dzwu, aidif, nk)
          call invtri_bmf (wrk1_v(:,:,:,2), Velocity%smf(:,:,2), Velocity%gamma(:,:),&
            ...
            do j=jsc,jec
               do i=isc,iec
                  ...
                  Velocity%accel(i,j,k,n)       = Velocity%accel(i,j,k,n)  &
                       + Thickness%rho_dzu(i,j,k,taup1)*Velocity%vfrict_impl(i,j,k,n)
                  !
                  !Due to a Compiler bug the following line affects answers in the static + production mode (no fltconsistency).
                  !This line was in the quebec_2009_10 version and to reproduce those answers we must keep it.
                  !Note that if we keep this we have to modify the send_data statements for this diagnostics variable.
                  !

...
integer :: id_neutral_rho_mixdown_on_nrho=-1
...
character(len=128) :: version=&
       '=>Using: ocean_mixdownslope.f90 ($Id: ocean_mixdownslope.F90,v 16.0.108.1.18.3 2010/03/29 16:52:37 smg Exp $)'
...
! provide for namelist over-ride of default values
  ...
  id_neutral_rho_mixdown = register_diag_field ('ocean_model', 'neutral_rho_mixdown', &
                           Grd%tracer_axes(1:3), Time%model_time,                     &
                           'update of neutral density from mixdowslope',              &
  ...
  id_wdian_rho_mixdown = register_diag_field ('ocean_model', 'wdian_rho_mixdown',   &
                         Grd%tracer_axes(1:3), Time%model_time,                     &
                         'dianeutral velocity component due to mixdowslope',        &
                         'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_mixdown_on_nrho = register_diag_field ('ocean_model',                   &
     'neutral_rho_mixdown_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,          &
     'update of neutral density from mixdownslope as mapped to neutral density surfaces',&
                           '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_mixdown_on_nrho = register_diag_field ('ocean_model',                           &
     ...
     'dianeutral velocity component due to mixdowslope as mapped to neutral density surfaces', &
     'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine mixdownslope (Time, Thickness, T_prog, Dens, index_temp, index_salt)
  ...
  do nt=1,num_prog_tracers 
     ...
     if(id_neutral_rho_mixdown         > 0 .or. id_wdian_rho_mixdown         > 0  .or. &
        id_neutral_rho_mixdown_on_nrho > 0 .or. id_wdian_rho_mixdown_on_nrho > 0) then 
         if(nt==index_temp) then 
             wrk1_v(:,:,:,1) = 0.0
             do k=1,nk
                do j=jsc,jec   
  ...
  if (id_neutral_rho_mixdown > 0) then
      ...
      do k=1,nk
         ...
         enddo
      enddo
      used = send_data (id_neutral_rho_mixdown,wrk1(:,:,:), Time%model_time,&
             rmask=Grd%tmask(:,:,:),                                        &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  ...
  if(nrho_diagnostics) then 
      if (id_neutral_rho_mixdown_on_nrho > 0) then 
         wrk1(:,:,:)      = 0.0
         nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_mixdown_on_nrho, nrho_work(:,:,:),&
          Time%model_time,                                                   &

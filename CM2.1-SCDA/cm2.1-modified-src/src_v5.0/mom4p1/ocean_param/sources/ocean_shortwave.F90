...
character(len=128)  :: version='$Id: ocean_shortwave.F90,v 16.0.2.2.24.2.12.1.22.1.4.1.6.4 2010/03/29 16:53:25 smg Exp $'
...
integer :: id_neutral_rho_sw_on_nrho=-1
...
namelist /ocean_shortwave_nml/ use_this_module, use_shortwave_gfdl,       &
                               ...
                               use_shortwave_ext, nrho_diagnostics
...
! </DESCRIPTION>
  subroutine ocean_shortwave_init(Grid, Domain, Time, Dens, vert_coordinate, Ocean_options)
    ...
    id_neutral_rho_sw_on_nrho = register_diag_field ('ocean_model',                             &
        'neutral_rho_sw_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,                   &
        'update of neutral density from shortwave penetration as mapped to neutral rho surface',&
        '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

    id_wdian_rho_sw_on_nrho = register_diag_field ('ocean_model',                                           &
      'wdian_rho_sw_on_nrho',Dens%neutralrho_axes(1:3), Time%model_time,                                    &
      'dianeutral velocity component due to penetrative shortwave heating as mapped to neutral rho surface',&
      'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine sw_source (Time, Thickness, Dens, T_diag, swflx, swflx_vis, Temp, sw_frac_zt, opacity)
  ...
  if (id_neutral_rho_sw > 0) then 
     wrk1(:,:,:) = 0.0
     do k=1,nk
        do i=isc,iec
           do j=jsc,jec
              wrk1(i,j,k) = cp_r*Grd%tmask(i,j,k)*Dens%drhodT(i,j,k)*Temp%wrk1(i,j,k)*Thickness%rho_dztr(i,j,k)
           enddo
        enddo
     enddo
     used = send_data (id_neutral_rho_sw, wrk1(:,:,:),         &
                       Time%model_time,rmask=Grd%tmask(:,:,:), &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  ...
  if(nrho_diagnostics) then
      if (id_neutral_rho_sw_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do i=isc,iec
                do j=jsc,jec
                   wrk1(i,j,k) = cp_r*Grd%tmask(i,j,k)*Dens%drhodT(i,j,k) &
                                     *Temp%wrk1(i,j,k)*Thickness%rho_dztr(i,j,k)
                enddo
             enddo
          enddo
          call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
          used = send_data (id_neutral_rho_sw_on_nrho, nrho_work(:,:,:),&
               Time%model_time,                                         &

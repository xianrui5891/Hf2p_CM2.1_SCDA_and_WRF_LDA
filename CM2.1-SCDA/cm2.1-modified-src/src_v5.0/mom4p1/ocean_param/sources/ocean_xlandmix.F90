...
integer :: id_neutral_rho_xmix        =-1
integer :: id_wdian_rho_xmix          =-1
integer :: id_neutral_rho_xmix_on_nrho=-1
...
character(len=128) :: version=&
       '$Id: ocean_xlandmix.F90,v 16.0.108.1.18.3 2010/03/29 16:53:25 smg Exp $'
...
subroutine ocean_xlandmix_init(Grid, Domain, Time, Dens, T_prog, Ocean_options, dtime)
  ...
  id_neutral_rho_xmix = register_diag_field ('ocean_model', 'neutral_rho_xmix', &
                        Grd%tracer_axes(1:3), Time%model_time,                        &
                        'update of neutral density from xlandmix',                 &
                        '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_wdian_rho_xmix = register_diag_field ('ocean_model', 'wdian_rho_xmix',  &
                         Grd%tracer_axes(1:3), Time%model_time,                    &
                         'dianeutral velocity component due to xlandmix',       &
                         'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_xmix_on_nrho = register_diag_field ('ocean_model',                  &
     'neutral_rho_xmix_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,         &
     'update of neutral density from xlandmix as mapped to neutral density surfaces',&
     '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_wdian_rho_xmix_on_nrho = register_diag_field ('ocean_model',                        &
     'wdian_rho_xmix_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,               &
     'dianeutral velocity component from xlandmix as mapped to neutral density surfaces',&
     '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine xlandmix (Time, Ext_mode, Dens, Thickness, T_prog)
  ...
  if (id_neutral_rho_xmix > 0) then
      ...
      do k=1,nk
         ...
         enddo
      enddo
     used = send_data (id_neutral_rho_xmix, wrk1(:,:,:),      &
                       Time%model_time,rmask=Grd%tmask(:,:,:),&
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  ...
  if (id_wdian_rho_xmix > 0) then 
      ...
      do k=1,nk
         ...
         enddo
      enddo
      used = send_data (id_wdian_rho_xmix, wrk1(:,:,:), &
           Time%model_time,rmask=Grd%tmask(:,:,:),      &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif

  if(nrho_diagnostics) then 
      if (id_neutral_rho_xmix_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_xmix_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                            &

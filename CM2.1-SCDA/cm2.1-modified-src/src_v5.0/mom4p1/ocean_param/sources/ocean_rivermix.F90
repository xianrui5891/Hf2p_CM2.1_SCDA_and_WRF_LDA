...
integer :: id_neutral_rho_rivermix_on_nrho   =-1
integer :: id_neutral_rho_runoffmix_on_nrho  =-1
integer :: id_neutral_rho_calvingmix_on_nrho =-1
...
character(len=128) :: version=&
       '$Id: ocean_rivermix.F90,v 16.0.2.1.10.1.2.1.52.1.18.4 2010/03/29 16:53:25 smg Exp $'
...
!
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    ! for diagnostic remapping to neutral density space 
  ...
  id_neutral_rho_rivermix_on_nrho = register_diag_field ('ocean_model',                     &
         'neutral_rho_rivermix_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,        &
          'update of neutral density from rivermix scheme as mapped to neutral rho surface',&
          '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_neutral_rho_runoffmix_on_nrho = register_diag_field ('ocean_model',                     &
         'neutral_rho_runoffmix_on_nrho',Dens%neutralrho_axes(1:3), Time%model_time,         &
         'update of neutral density from runoffmix scheme as mapped to neutral rho surface', &
         '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_neutral_rho_calvingmix_on_nrho = register_diag_field ('ocean_model',                   &
        'neutral_rho_calvingmix_on_nrho',Dens%neutralrho_axes(1:3), Time%model_time,        &
        'update of neutral density from calvingmix scheme as mapped to neutral rho surface',&
        '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_rivermix_on_nrho = register_diag_field ('ocean_model',                          &
       ...
       'dianeutral velocity component due to rivermix scheme as mapped to neutral rho surface',&
       'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_runoffmix_on_nrho = register_diag_field ('ocean_model',                         &
      ...
      'dianeutral velocity component due to runoffmix scheme as mapped to neutral rho surface',&
      'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_calvingmix_on_nrho = register_diag_field ('ocean_model',                          &
       ...
       'dianeutral velocity component due to calvingmix scheme as mapped to neutral rho surface',&
       'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine  river_diagnostics (Time, Thickness, Dens, temp_wrk, salt_wrk, mix_type)
  ...
  if(mix_type==1) then 
      ...
      if (id_neutral_rho_runoffmix > 0) then
          ...
          do k=1,nk
             do j=jsc,jec
                do i=isc,iec
                   wrk1(i,j,k) = Grd%tmask(i,j,k)*Thickness%rho_dztr(i,j,k) &  
      ...
      if(nrho_diagnostics) then
          if (id_neutral_rho_runoffmix_on_nrho > 0) then
              wrk1(:,:,:)      = 0.0
              nrho_work(:,:,:) = 0.0
              do k=1,nk
                 do j=jsc,jec
              ...
              used = send_data (id_neutral_rho_runoffmix_on_nrho, nrho_work(:,:,:), &
                   Time%model_time,                                                 &
  ...
  elseif(mix_type==2) then 
      ...
      if(nrho_diagnostics) then
          if (id_neutral_rho_calvingmix_on_nrho > 0) then
              wrk1(:,:,:)      = 0.0
              nrho_work(:,:,:) = 0.0
              do k=1,nk
                 do j=jsc,jec
              ...
              enddo
              call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
              used = send_data (id_neutral_rho_calvingmix_on_nrho, nrho_work(:,:,:), &
                   Time%model_time,                                                  &
                   is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
          endif
  ...
  elseif(mix_type==3) then 
      ...
      if(nrho_diagnostics) then
          if (id_neutral_rho_rivermix_on_nrho > 0) then
              wrk1(:,:,:)      = 0.0
              nrho_work(:,:,:) = 0.0
              do k=1,nk
                 do j=jsc,jec
              ...
              enddo
              call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
              used = send_data (id_neutral_rho_rivermix_on_nrho, nrho_work(:,:,:), &
                   Time%model_time,                                                &
                   is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
          endif

...
integer :: id_neutral_rho_overexch_on_nrho=-1
integer :: id_wdian_rho_overexch_on_nrho  =-1

integer, dimension(:), allocatable :: id_overexch


character(len=128) :: version=&
       '=>Using: ocean_overexchange.f90 ($Id: ocean_overexchange.F90,v 16.0.108.1.18.4 2010/03/29 16:53:25 smg Exp $)'
...
namelist /ocean_overexchange_nml/ use_this_module, debug_this_module, overexch_npts, overexch_width, &
                                  ...
                                  overexch_check_extrema, nrho_diagnostics  
...
!
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    id_neutral_rho_overexch_on_nrho = register_diag_field ('ocean_model',             &
      'neutral_rho_overexch_on_nrho',Dens%neutralrho_axes(1:3), Time%model_time,      &
      'update of neutral density from overexchange as mapped to neutral rho surface', &
      '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

    id_wdian_rho_overexch = register_diag_field ('ocean_model',      &
        'wdian_rho_overexch', Grd%tracer_axes(1:3), Time%model_time, &
        'dianeutral velocity component due to overexchange',         &
        'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

    id_wdian_rho_overexch_on_nrho = register_diag_field ('ocean_model',                    &
      'wdian_rho_overexch_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,            &
      'dianeutral velocity component due to overexchange as mapped to neutral rho surface',&
      'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine overexchange (Time, Thickness, T_prog, Dens, index_temp, index_salt)
  ...
  if (id_neutral_rho_overexch > 0) then 
      ...
      do k=1,nk
         ...
         enddo
      enddo    
     used = send_data (id_neutral_rho_overexch, wrk1(:,:,:),   &
                       Time%model_time,rmask=Grd%tmask(:,:,:), &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  ...
  if(nrho_diagnostics) then
      if (id_neutral_rho_overexch_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_overexch_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                                &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
      if (id_wdian_rho_overexch_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          enddo
          call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
          used = send_data (id_wdian_rho_overexch_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                              &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
  endif

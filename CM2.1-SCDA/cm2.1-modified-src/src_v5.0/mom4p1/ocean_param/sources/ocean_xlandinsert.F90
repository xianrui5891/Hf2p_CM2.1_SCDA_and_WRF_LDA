...
integer :: id_neutral_rho_xinsert        =-1
integer :: id_wdian_rho_xinsert          =-1
integer :: id_neutral_rho_xinsert_on_nrho=-1
...
character(len=128) :: version=&
       '$Id: ocean_xlandinsert.F90,v 16.0.108.1.18.3 2010/03/29 16:53:25 smg Exp $'
...
! define arrays and xland_domain
  ...
  ! for diagnostic remapping to neutral density surfaces 
  ...
  id_neutral_rho_xinsert = register_diag_field ('ocean_model', 'neutral_rho_xinsert', &
                        Grd%tracer_axes(1:3), Time%model_time,                        &
                        'update of neutral density from xlandinsert',                 &
  ...
  id_wdian_rho_xinsert = register_diag_field ('ocean_model', 'wdian_rho_xinsert',  &
                         Grd%tracer_axes(1:3), Time%model_time,                    &
                         'dianeutral velocity component due to xlandinsert',       &
                         'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

  id_neutral_rho_xinsert_on_nrho = register_diag_field ('ocean_model',                  &
     'neutral_rho_xinsert_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,         &
     'update of neutral density from xlandinsert as mapped to neutral density surfaces',&
     '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  ...
  id_wdian_rho_xinsert_on_nrho = register_diag_field ('ocean_model',                        &
     ...
     'dianeutral velocity component from xlandinsert as mapped to neutral density surfaces',&
     '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
!        adjustment was the only process contributing to the tendencies
     ...
     if(id_neutral_rho_xinsert         > 0 .or. id_wdian_rho_xinsert         > 0   .or. &
        id_neutral_rho_xinsert_on_nrho > 0 .or. id_wdian_rho_xinsert_on_nrho > 0  ) then 
  ...
  ! send diagnostics for update of neutral density and dianeutral 
  ! velocity component as determined from the xlandinsert scheme.
  if (id_neutral_rho_xinsert > 0) then 
      wrk1(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
         ...
         enddo
      enddo
      used = send_data (id_neutral_rho_xinsert, wrk1(:,:,:),   &
                        Time%model_time,rmask=Grd%tmask(:,:,:),&
                        is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  if (id_wdian_rho_xinsert > 0) then 
      ...
      do k=1,nk
         ...
         enddo
      enddo
      used = send_data (id_wdian_rho_xinsert, wrk1(:,:,:), &
           Time%model_time,rmask=Grd%tmask(:,:,:),         &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif

  if(nrho_diagnostics) then 
      if (id_neutral_rho_xinsert_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_xinsert_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                               &

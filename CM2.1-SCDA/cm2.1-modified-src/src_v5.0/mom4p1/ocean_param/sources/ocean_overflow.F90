...
integer :: id_neutral_rho_overflow        =-1
integer :: id_wdian_rho_overflow          =-1
integer :: id_neutral_rho_overflow_on_nrho=-1
integer :: id_wdian_rho_overflow_on_nrho  =-1


character(len=128) :: version=&
       '=>Using: ocean_overflow.f90 ($Id: ocean_overflow.F90,v 16.0.54.2.44.1.18.1 2010/03/29 16:53:25 smg Exp $)'
...
namelist /ocean_overflow_nml/ use_this_module, debug_this_module, overflow_mu,     &
                              ...
                              no_return_flow, nrho_diagnostics 
...
!
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    id_neutral_rho_overflow = register_diag_field ('ocean_model', 'neutral_rho_overflow', &
                        Grd%tracer_axes(1:3), Time%model_time,                            &
                        'update of neutral density from overflow scheme',                 &
                        '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

    id_neutral_rho_overflow_on_nrho = register_diag_field ('ocean_model',        &
      'neutral_rho_overflow_on_nrho',Dens%neutralrho_axes(1:3), Time%model_time, &
      'update of neutral density from overflow as mapped to neutral rho surface',&
      '(kg/m^3)/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

    id_wdian_rho_overflow = register_diag_field ('ocean_model',      &
        'wdian_rho_overflow', Grd%tracer_axes(1:3), Time%model_time, &
        'dianeutral velocity component due to overflow',             &
        'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))

    id_wdian_rho_overflow_on_nrho = register_diag_field ('ocean_model',                &
      'wdian_rho_overflow_on_nrho', Dens%neutralrho_axes(1:3), Time%model_time,        &
      'dianeutral velocity component due to overflow as mapped to neutral rho surface',&
      'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
...
subroutine overflow (Time, Thickness, T_prog, Dens, index_temp, index_salt)
  ...
  do n=1,num_prog_tracers 
     ...
     source_overflow(:,:,:) = 0.0
     flux_int_z(:,:,:)      = 0.0 
     wrk1=0.0 ; wrk2=0.0 ; wrk3=0.0 ; wrk4=0.0 ; wrk1_v(:,:,:,:) = 0.0
  ...
  ! diagnostics for update of neutral density from overflow scheme 
  if (id_neutral_rho_overflow > 0) then 
      wrk1(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
         ...
         enddo
      enddo    
     used = send_data (id_neutral_rho_overflow, wrk1(:,:,:),   &
                       Time%model_time,rmask=Grd%tmask(:,:,:), &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  if (id_wdian_rho_overflow > 0) then 
      wrk1(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
         ...
         enddo
      enddo
      used = send_data (id_wdian_rho_overflow, wrk1(:,:,:), &
           Time%model_time,rmask=Grd%tmask(:,:,:),          &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  ...
  if(nrho_diagnostics) then
      if (id_neutral_rho_overflow_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          used = send_data (id_neutral_rho_overflow_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                                &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
      if (id_wdian_rho_overflow_on_nrho > 0) then 
          wrk1(:,:,:)      = 0.0
          nrho_work(:,:,:) = 0.0
          do k=1,nk
             do j=jsc,jec
          ...
          enddo
          call remap_onto_nrho (Time, Dens, wrk1, nrho_work) 
          used = send_data (id_wdian_rho_overflow_on_nrho, nrho_work(:,:,:), &
               Time%model_time,                                              &
               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
  endif

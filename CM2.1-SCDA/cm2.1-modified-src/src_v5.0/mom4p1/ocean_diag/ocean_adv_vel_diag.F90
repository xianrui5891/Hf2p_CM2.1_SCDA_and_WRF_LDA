...
character(len=128) :: version=&
     '$Id: ocean_adv_vel_diag.F90,v 16.0.2.2.48.3.22.1.4.1.14.4 2010/04/08 12:58:41 smg Exp $'
character (len=128) :: tagname = &
     '$Name: mom4p1_riga_08apr2010_smg $'
      ...
      do k_rho=1,neutralrho_nk
         ...
         do k=1,nk
            do j=jsc,jec
               do i=isc,iec
                  if (k_rho == 1) then
                     if(Dens%neutralrho(i,j,k) <= Dens%neutralrho_ref(k_rho)) then 
                  ...
                  elseif (k_rho == neutralrho_nk) then
                     ...
                     if( (Dens%neutralrho_ref(k_rho) <  Dens%neutralrho(i,j,k)) .and.  &
                         (Dens%neutralrho(i,j,k)     <= Dens%neutralrho_ref(k_rho+1)) ) then 
                           weight = (Dens%neutralrho(i,j,k)      -Dens%neutralrho_ref(k_rho)) &
                                   /(Dens%neutralrho_ref(k_rho+1)-Dens%neutralrho_ref(k_rho))
                           tmp(1,i,j)   = tmp(1,i,j)   + weight*Adv_vel%uhrho_et(i,j,k)
                           tmp(2,i,j)   = tmp(2,i,j)   + weight*Adv_vel%vhrho_nt(i,j,k)
                           work1(i,j,k_rho+1) = work1(i,j,k_rho+1) + (1.0-weight)*Adv_vel%uhrho_et(i,j,k)
                           work2(i,j,k_rho+1) = work2(i,j,k_rho+1) + (1.0-weight)*Adv_vel%vhrho_nt(i,j,k)
                     endif
                  endif
               enddo
            enddo
      ...
      do k_rho=1,potrho_nk
         ...
         do k=1,nk
            do j=jsc,jec
               do i=isc,iec
                  if (k_rho == 1) then 
                     if(Dens%potrho(i,j,k) <= Dens%potrho_ref(k_rho)) then 
                        tmp(1,i,j) = tmp(1,i,j) + Adv_vel%uhrho_et(i,j,k)
                        tmp(2,i,j) = tmp(2,i,j) + Adv_vel%vhrho_nt(i,j,k)
                     endif
                  elseif (k_rho == potrho_nk) then
                     ...
                     if( (Dens%potrho_ref(k_rho) <  Dens%potrho(i,j,k)) .and. &
                         (Dens%potrho(i,j,k)     <= Dens%potrho_ref(k_rho+1)) ) then 
      ...
      do k_theta=1,theta_nk
         ...
         do k=1,nk
            do j=jsc,jec
               do i=isc,iec
                  if (k_theta == 1) then
                     if(Theta%field(i,j,k,tau) <= Dens%theta_ref(k_theta)) then 
                         tmp(1,i,j) = tmp(1,i,j) + Adv_vel%uhrho_et(i,j,k)
                         tmp(2,i,j) = tmp(2,i,j) + Adv_vel%vhrho_nt(i,j,k)
                     endif
                  elseif (k_theta == theta_nk) then
                     ...
                     if( (Dens%theta_ref(k_theta) <  Theta%field(i,j,k,tau)) .and. &
                         (Theta%field(i,j,k,tau)  <= Dens%theta_ref(k_theta+1)) ) then 

                          weight = (Theta%field(i,j,k,tau)   -Dens%theta_ref(k_theta)) &
                                  /(Dens%theta_ref(k_theta+1)-Dens%theta_ref(k_theta))
                          tmp(1,i,j)   = tmp(1,i,j)   + weight*Adv_vel%uhrho_et(i,j,k)
                          tmp(2,i,j)   = tmp(2,i,j)   + weight*Adv_vel%vhrho_nt(i,j,k)
                          work1(i,j,k_theta+1) = work1(i,j,k_theta+1) + (1.0-weight)*Adv_vel%uhrho_et(i,j,k)
                          work2(i,j,k_theta+1) = work2(i,j,k_theta+1) + (1.0-weight)*Adv_vel%vhrho_nt(i,j,k)
                     endif
                  endif
               enddo
            enddo

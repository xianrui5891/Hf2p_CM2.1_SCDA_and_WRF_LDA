...
!  </DATA>
!
!  <DATA NAME="diagnose_sea_level_forcing" TYPE="logical">
!  To enable a suite of diagnostics that compute the contributions of 
!  surface flux forcing on sea level, according to the sea level equation 
!  derived from a non-Boussinesq fluid.  These diagnostics are mapped through 
!  diag_table entries.  The diagnostic table entry must be registered, as well 
!  as setting diagnose_sea_level_forcing=.true.
!  Default diagnose_sea_level_forcing=.false.
!  </DATA> 
!</NAMELIST>
!
...
integer :: id_eta_tendency_sw           = -1
integer :: id_eta_tendency_lw           = -1
integer :: id_eta_tendency_sens         = -1
integer :: id_eta_tendency_evap_heat    = -1
integer :: id_eta_tendency_fprec_melt   = -1
integer :: id_eta_tendency_iceberg_melt = -1
integer :: id_eta_tendency_heat_coupler = -1
integer :: id_eta_tendency_heat_restore = -1

integer :: id_eta_tendency_salt_coupler = -1
integer :: id_eta_tendency_salt_restore = -1

integer :: id_eta_tendency_evap          = -1
integer :: id_eta_tendency_lprec         = -1
integer :: id_eta_tendency_fprec         = -1
integer :: id_eta_tendency_runoff        = -1
integer :: id_eta_tendency_iceberg       = -1
integer :: id_eta_tendency_water_coupler = -1
integer :: id_eta_tendency_water_restore = -1
...
character(len=128) :: version=&
     '$Id: ocean_sbc.F90,v 16.0.2.11.8.1.2.3.10.5.10.1.4.2.10.6 2010/03/22 20:30:15 smg Exp $'
...
logical :: diagnose_sea_level_forcing   =.false.
...
subroutine ocean_sbc_init(Grid, Domain, Time, T_prog, T_diag, Velocity, &
 ...
 if(.not. diagnose_sea_level_forcing) then 
    write(stdoutunit,*) &
    '==>Note that diagnose_sea_level_forcing==.false., so no "eta_tendency_" fields will be diagnosed.'
 endif 
 ...
 id_eta_tendency_sw = register_diag_field('ocean_model','eta_tendency_sw', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from shortwave heating', 'm/s',                     &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_lw = register_diag_field('ocean_model','eta_tendency_lw', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from longwave heating', 'm/s',                      &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_sens = register_diag_field('ocean_model','eta_tendency_sens', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from sensible heating', 'm/s',                          &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_evap_heat = register_diag_field('ocean_model','eta_tendency_evap_heat', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from latent heat of vaporization', 'm/s',                         &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_fprec_melt = register_diag_field('ocean_model','eta_tendency_fprec_melt', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from latent heat of melting frozen precip', 'm/s',                  &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_iceberg_melt = register_diag_field('ocean_model','eta_tendency_iceberg_melt', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from latent heat of melting icebergs', 'm/s',                           &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_heat_coupler = register_diag_field('ocean_model','eta_tendency_heat_coupler', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from total sfc heating through coupler (sans frazil)', 'm/s',           &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_heat_restore = register_diag_field('ocean_model','eta_tendency_heat_restore', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from total sfc heating through restore', 'm/s',                         &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_salt_coupler = register_diag_field('ocean_model','eta_tendency_salt_coupler', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from salt flux entering through coupler', 'm/s',                        &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_salt_restore = register_diag_field('ocean_model','eta_tendency_salt_restore', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from salt flux entering through restoring', 'm/s',                      &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_fprec = register_diag_field('ocean_model','eta_tendency_fprec', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from frozen precipitation', 'm/s',                        &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_lprec = register_diag_field('ocean_model','eta_tendency_lprec', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from liquid precipitation', 'm/s',                        &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_evap = register_diag_field('ocean_model','eta_tendency_evap', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from evaporating water', 'm/s',                         &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_runoff = register_diag_field('ocean_model','eta_tendency_runoff', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from liquid runoff', 'm/s',                                 &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_iceberg = register_diag_field('ocean_model','eta_tendency_iceberg', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from icebergs', 'm/s',                                        &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_water_coupler = register_diag_field('ocean_model','eta_tendency_water_coupler', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from surface water fluxes passed through coupler', 'm/s',                 &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   

 id_eta_tendency_water_restore = register_diag_field('ocean_model','eta_tendency_water_restore', Grd%tracer_axes(1:2),&
       Time%model_time, 'sea level tendency from surface water fluxes computed from surface restoring', 'm/s',        &
       missing_value=missing_value,range=(/-1.e10,1.e10/))   
...
subroutine get_ocean_sbc(Time, Ice_ocean_boundary, Thickness, Dens, Ext_mode, T_prog, Velocity, &
  ...
  ! net heat flux from radiation, latent, sensible (passed through coupler), and mass transport 
  ...
  ! for diagnosing various contributions to sea level forcing 
  if(diagnose_sea_level_forcing) then 

      ! for holding sum of sfc heat flux terms 
      wrk2_2d(:,:) = 0.0

      ! for holding sum of water flux terms  
      wrk3_2d(:,:) = 0.0 
      ...
      if(id_eta_tendency_sw > 0) then 
          ...
          do j=jsc,jec
             ...
             enddo
          enddo
          used = send_data(id_eta_tendency_sw, wrk1_2d(:,:), &
                 Time%model_time, rmask=Grd%tmask(:,:,1),    &
                 is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc_bnd,jec_bnd
         do i=isc_bnd,iec_bnd
            ii=i+i_shift
            jj=j+j_shift  
            tmp_flux(ii,jj) = cp_ocean_r*Ice_ocean_boundary%lw_flux(i,j)
         enddo
      enddo
      do j=jsc,jec
         do i=isc,iec
      ...
      if(id_eta_tendency_lw > 0) then 
          ...
          do j=jsc,jec
             ...
             enddo
          enddo
          used = send_data(id_eta_tendency_lw, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),    &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif

      ! sensible contribution to sea level 
      do j=jsc_bnd,jec_bnd
         do i=isc_bnd,iec_bnd
            ii=i+i_shift
            jj=j+j_shift  
            tmp_flux(ii,jj) = -cp_ocean_r*Ice_ocean_boundary%t_flux(i,j)
         enddo
      enddo
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      if(id_eta_tendency_sens > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = alphasfc(i,j)*tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_sens, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),        &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc_bnd,jec_bnd
         do i=isc_bnd,iec_bnd
            ii=i+i_shift
            jj=j+j_shift  
            tmp_flux(ii,jj) = -cp_ocean_r*hlv*Ice_ocean_boundary%q_flux(i,j)
         enddo
      enddo
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      if(id_eta_tendency_evap_heat > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = alphasfc(i,j)*tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_evap_heat, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),             &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc_bnd,jec_bnd
         do i=isc_bnd,iec_bnd
            ii=i+i_shift
            jj=j+j_shift  
            tmp_flux(ii,jj) = -cp_ocean_r*Ice_ocean_boundary%fprec(i,j)*hlf
         enddo
      enddo
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      if(id_eta_tendency_fprec_melt > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = alphasfc(i,j)*tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_fprec_melt, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),              &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         do i=isc,iec
            tmp_flux(i,j) = -cp_ocean_r*calving(i,j)*hlf
         enddo
      enddo
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      if(id_eta_tendency_iceberg_melt > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = alphasfc(i,j)*tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_iceberg_melt, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),                &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif

      ! sum of sfc heat contributions to sea level from heat passed through coupler
      if(id_eta_tendency_heat_coupler > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
             ...
             enddo
          enddo
          used = send_data(id_eta_tendency_heat_coupler, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),                &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         ...
         enddo
      enddo
      if(id_eta_tendency_salt_coupler > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
             ...
             enddo
          enddo
          used = send_data(id_eta_tendency_salt_coupler, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),                &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         do i=isc,iec
            wrk3_2d(i,j) = tmp_flux(i,j)*Grd%tmask(i,j,1)
         enddo
      enddo
      if(id_eta_tendency_evap > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
             ...
             enddo
          enddo
          used = send_data(id_eta_tendency_evap, wrk1_2d(:,:), &
               Time%model_time, rmask=Grd%tmask(:,:,1),        &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         do i=isc,iec
            wrk3_2d(i,j) = wrk3_2d(i,j) + tmp_flux(i,j)*Grd%tmask(i,j,1)
         enddo
      enddo
      if(id_eta_tendency_lprec > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_lprec, wrk1_2d(:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,1),        &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         ...
         enddo
      enddo
      if(id_eta_tendency_fprec > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_fprec, wrk1_2d(:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,1),        &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         ...
         enddo
      enddo
      if(id_eta_tendency_runoff > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_runoff, wrk1_2d(:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,1),         &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
      ...
      do j=jsc,jec
         ...
         enddo
      enddo
      if(id_eta_tendency_iceberg > 0) then 
          wrk1_2d = 0.0
          do j=jsc,jec
             do i=isc,iec
                wrk1_2d(i,j) = tmp_flux(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_iceberg, wrk1_2d(:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,1),          &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif

      ! sum of the water fluxes passed through coupler contribution to sea level 
      if(id_eta_tendency_water_coupler > 0) then
          do j=jsc,jec
             do i=isc,iec
                wrk3_2d(i,j) = wrk3_2d(i,j)*rhosfc_inv(i,j) 
             enddo
          enddo
          used = send_data(id_eta_tendency_water_coupler, wrk3_2d(:,:),&
               Time%model_time, rmask=Grd%tmask(:,:,1),                &
               is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
      endif
...
subroutine flux_adjust(Time, T_diag, T_prog, Velocity, river, melt, pme)
  ...
  if(id_eta_tendency_salt_restore > 0) then 
      wrk1_2d = 0.0
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      used = send_data(id_eta_tendency_salt_restore, wrk1_2d(:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,1),                &
           is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif

  ! contribution to sea level from water restoring flux 
  if(id_eta_tendency_water_restore > 0) then 
      wrk1_2d = 0.0
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      used = send_data(id_eta_tendency_water_restore, wrk1_2d(:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,1),                 &
           is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif
  ...
  if(id_eta_tendency_heat_restore > 0) then 
      wrk1_2d = 0.0
      do j=jsc,jec
         do i=isc,iec
         ...
         enddo
      enddo
      used = send_data(id_eta_tendency_heat_restore, wrk1_2d(:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,1),                &
           is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif

...
USE fms_mod,          ONLY: file_exist, FATAL, WARNING, NOTE, mpp_pe
...
USE mpp_mod,          ONLY: mpp_broadcast, stdout, stdlog, mpp_root_pe
    ...
    if (max_dt_for_cgint == max_dt_for_cgint0 .and. nint(dtuv) > 0) then
        IF (dtuv <= max_dt_for_cgint) THEN
           IF (mpp_root_pe() .EQ. mpp_pe()) THEN
            ...
            write(stdoutunit,'(a,e12.6)') '         The dxu grid distance (m) at this point is ',Grd%dxu(icg,jcg) 
            WRITE(stdoutunit,'(a,e12.6)') '         The dyu grid distance (m) at this point is ',Grd%dyu(icg,jcg) 
           END IF
            cfl_error=.false.
        ELSE
           IF (mpp_root_pe() .EQ. mpp_pe()) then
        ...
        IF (mpp_root_pe().EQ.mpp_pe()) WRITE (stdoutunit,'(a,f5.2,a/a,f6.0,a,f6.0,a)')&
        ...
        IF (mpp_root_pe().EQ.mpp_pe()) WRITE (stdlogunit,'(/a/)') &

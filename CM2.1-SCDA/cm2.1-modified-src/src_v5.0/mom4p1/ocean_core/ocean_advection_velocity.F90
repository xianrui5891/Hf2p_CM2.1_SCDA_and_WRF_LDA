...
integer :: id_wb_rho           =-1
integer :: id_eta_tendency_press=-1
...
character(len=128) :: version=&
     '$Id: ocean_advection_velocity.F90,v 16.0.2.1.84.1.18.3 2010/02/01 19:15:11 smg Exp $'
...
subroutine ocean_advection_velocity_init(Grid, Domain, Time, Time_steps, Thickness, Adv_vel, &
  ...
  id_eta_tendency_press = register_diag_field ('ocean_model', 'eta_tendency_press', Grd%tracer_axes(1:2), &
  Time%model_time, 'eta tendency from motion across pressure surfaces (when vert coord = press)', 'm/s',  &
...
subroutine ocean_advection_velocity (Velocity, Time, Thickness, Dens, pme, river, Adv_vel)
  ...
  if(id_eta_tendency_press > 0) then 
      ...
      do j=jsc,jec
         ...
         enddo
      enddo
      used = send_data (id_eta_tendency_press, wrk1_2d(:,:),&

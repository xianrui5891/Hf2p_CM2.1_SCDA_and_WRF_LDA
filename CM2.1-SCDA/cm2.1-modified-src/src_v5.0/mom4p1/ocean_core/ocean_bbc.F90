...
!  </DATA> 
!
!  <DATA NAME="bmf_barotropic" TYPE="logical">
!  For computing bmf using the vertically averaged velocity, rather than
!  the bottom velocity. bmf_barotropic=.true. is similar to how POM 
!  computed bottom friction. Default is bmf_barotropic=.false. 
!  </DATA> 
!
...
character(len=128) :: version=&
     '$Id: ocean_bbc.F90,v 16.0.2.3.48.1.32.1.18.5 2010/02/22 16:45:40 smg Exp $'
...
integer :: id_eta_tendency_geoheat
integer :: id_geo_heat 
...
if(bmf_barotropic) then 
   call mpp_error(WARNING,'==>Warning from ocean_bbc_mod: bmf_barotropic=.true. has not been fully tested.')
   write(stdoutunit,'(a)') &
   '==>Note from ocean_bbc_mod: Computing bottom drag using barotropic velocity rather than bottom-most velocity.'
   write(stdoutunit,'(a)') &
   '==>Use of bmf_barotropic may necessitate use of implicit_bmf for stability.'
   Ocean_options%bmf_barotropic = 'Computed bottom drag using barotropic velocity.'
   Velocity%bmf_barotropic = .true.
else
   Ocean_options%bmf_barotropic = 'Computed bottom drag using velocity from bottom-most level.'
   Velocity%bmf_barotropic = .false.
endif 
...
id_eta_tendency_geoheat = register_diag_field('ocean_model','eta_tendency_geoheat',      &
       Grd%tracer_axes(1:2),Time%model_time,'sea level tendency from geothermal heating',&
...
! </DESCRIPTION>
!
subroutine get_ocean_bbc(Time, Thickness, Ext_mode, Dens, Velocity, T_prog)
...
if(bmf_barotropic) then 
    do j=jsd,jed
       do i=isd,ied
          ...
          if (kmu > 0) then
              mass_inv    = 1.0/(Thickness%mass_u(i,j,tstep)+epsln)
              ubarotropic = Ext_mode%udrho(i,j,1,tstep)*mass_inv
              vbarotropic = Ext_mode%udrho(i,j,2,tstep)*mass_inv
              uvmag = sqrt(uresidual2 + ubarotropic**2 + vbarotropic**2)
              uvmag = min(uvmag,uvmag_max)
              Velocity%gamma(i,j) = drag_coeff(i,j)*uvmag
              umag                = abs(ubarotropic)
              vmag                = abs(vbarotropic)
              Velocity%bmf(i,j,1) = min(bmf_max, Velocity%gamma(i,j)*umag)*sign(1.0,ubarotropic)
              Velocity%bmf(i,j,2) = min(bmf_max, Velocity%gamma(i,j)*vmag)*sign(1.0,vbarotropic)
          endif
       enddo
    enddo
else
    do j=jsd,jed
       do i=isd,ied
          ...
          Velocity%gamma(i,j) = 0.0
          if (kmu > 0) then
              uvmag = sqrt(uresidual2 + Velocity%u(i,j,kmu,1,tstep)**2 + Velocity%u(i,j,kmu,2,tstep)**2)
              uvmag = min(uvmag,uvmag_max)
              Velocity%gamma(i,j)  = drag_coeff(i,j)*uvmag
              umag                 = abs(Velocity%u(i,j,kmu,1,tstep))
              vmag                 = abs(Velocity%u(i,j,kmu,2,tstep))
              Velocity%bmf(i,j,1)  = min(bmf_max, Velocity%gamma(i,j)*umag)*sign(1.0,Velocity%u(i,j,kmu,1,tstep))
              Velocity%bmf(i,j,2)  = min(bmf_max, Velocity%gamma(i,j)*vmag)*sign(1.0,Velocity%u(i,j,kmu,2,tstep))
          endif
       enddo
    enddo
endif
...
if (id_eta_tendency_geoheat > 0) then 
    wrk1_2d = 0.0
    do j=jsc,jec
       do i=isc,iec
          kbot=Grd%kmt(i,j)
       ...
       enddo
    enddo
    used = send_data(id_eta_tendency_geoheat, wrk1_2d(:,:), &

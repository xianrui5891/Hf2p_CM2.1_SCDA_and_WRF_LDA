...
!  </DATA> 
!
!  <DATA NAME="bmf_updated_on_barotropic" TYPE="logical">
!  To apply bottom drag over each barotropic time step. 
!  Note that enhanced drag from unresolved tides is not 
!  facilitated with this approach. Also, only use constant cdbot
!  for this approach.   
!  Default bmf_updated_on_barotropic=.false.
!  </DATA> 
!  <DATA NAME="bmf_barotropic_cdbot" TYPE="real" UNITS="dimensionless">
!  Dimensionless bottom drag coefficient for applying the bottom drag
!  on the barotropic time step. Default bmf_barotropic_cdbot=1e-3.
!  </DATA> 
!
...
logical  :: bmf_updated_on_barotropic = .false.  
real     :: bmf_barotropic_cdbot      = 1e-3
...
character(len=128) :: &
     version='$Id: ocean_barotropic.F90,v 1.1.4.1.2.62.6.12.6.2.2.6.4.3.22.1.4.3.4.3 2010/02/22 16:10:24 smg Exp $'
...
real, dimension(isd:ied,jsd:jed)     :: massu_r    ! inverse mass per area on U-cell at time tau (m^2/kg)
...
real, dimension(:,:), allocatable     :: massu_r    ! inverse mass per area on U-cell at time tau (m^2/kg)
...
namelist /ocean_barotropic_nml/ write_a_restart,                                &
	 ...
	 initsum_with_bar_mom4p0, initsum_with_bar_mom4p1,                      &
         ...
         bmf_updated_on_barotropic, bmf_barotropic_cdbot,                       &
...
subroutine ocean_barotropic_init(Grid, Domain, Time, Time_steps, Ocean_options, Ext_mode, obc, &
  ...
  allocate (massu_r(isd:ied,jsd:jed))
  ...
  if(bmf_updated_on_barotropic) then 
     call mpp_error(NOTE,&
     '==>ocean_barotropic_mod: bmf_updated_on_barotropic=.true. computes bottom drag on each barotropic step.')
  endif
  ...
  if (nint(dtuv) > 0) then
    ...
    elseif (nint(dtuv) == nint(dtbt)) then
      ...
      if (abs((2.0*dtuv)/nts - dtbt) > crit) then
        ...
        write (stdoutunit,*)      '          Current setting: '
        write (stdoutunit,*)      '             dtuv =  ',dtuv,' sec.'
        write (stdoutunit,*)      '             dtbt =  ',dtbt,' sec.'
        write (stdoutunit,*)      '          New setting should be:  '
        write (stdoutunit,*)      '             dtbt =  ',(2.0*dtuv)/nts,' sec.'
  ...
  if (nint(dtbt) > 0) then
    ...
    if (max_dt_for_cgext == max_dt_for_cgext0) then
      ...
      if (dtbt <= max_dt_for_cgext) then
        write (stdoutunit,'(/a,i4,a,i4,a,f6.2,a,f6.2,a)')                      &
        ' Barotropic stability most nearly violated at U-cell (i,j) = (',&
        icg+Dom%ioff,',',jcg+Dom%joff,'), (lon,lat) = (',Grd%xu(icg,jcg),',',Grd%yu(icg,jcg),').'
        write(stdoutunit,'(a,i6)')    '         The number of kmu-levels at this point is ',Grd%kmu(icg,jcg) 
        write(stdoutunit,'(a,e12.6)') '         The dxu grid spacing (m) at this point is ',Grd%dxu(icg,jcg) 
        write(stdoutunit,'(a,e12.6)') '         The dyu grid spacing (m) at this point is ',Grd%dyu(icg,jcg)
        cfl_error=.false.
      else
        write (stdoutunit,'(/a,i4,a,i4,a,f6.2,a,f6.2,a)')                   &
      ...
      write (stdoutunit,'(a,f5.1,a/a,f8.3,a,f8.3,a)')                             &
...
subroutine update_ocean_barotropic (Time, Dens, Thickness, Velocity, Adv_vel, &
  ...
  ! compute inverse thickness and inverse mass per area of U-cell column
  do j=jsd,jed
     do i=isd,ied
        thicku_r(i,j) = 0.0
        k = Grd%kmu(i,j)
        if(k > 1) then 
            ...
            massu_r(i,j)  = Grd%umask(i,j,1)/(epsln+Thickness%mass_u(i,j,tau))  
        endif
     enddo
  enddo
  ...
  if(splitting) then 
      ...
      if(vert_coordinate_class==DEPTH_BASED) then 
        if(barotropic_leap_frog) call leap_frog_barotropic_depth (Time, Thickness, Velocity, Ext_mode, patm, pme, river)
        if(barotropic_pred_corr) call pred_corr_barotropic_depth (Time, Thickness, Velocity, Ext_mode, patm, pme, river)
      elseif(vert_coordinate_class==PRESSURE_BASED) then 
        if(barotropic_pred_corr) call pred_corr_barotropic_press (Time, Thickness, Velocity, Ext_mode, pme, river)
      endif  
...
! </DESCRIPTION>
!
subroutine leap_frog_barotropic_depth (Time, Thickness, Velocity, Ext_mode, patm, pme, river)
  ...
  do itime=1,nts
     ...
     if(bmf_updated_on_barotropic) then 
         do n=1,2
            do j=jsd,jed
               do i=isd,ied
                  wrk1_v2d(i,j,n) = -Velocity%gamma(i,j)*udrho_bt(i,j,n,fstau)*massu_r(i,j)
               enddo
            enddo
         enddo
     endif
...
! </DESCRIPTION>
!
subroutine pred_corr_barotropic_depth (Time, Thickness, Velocity, Ext_mode, patm, pme, river)
  ...
  do itime=1,nts
     ...
     if(bmf_updated_on_barotropic) then 
         do n=1,2
            do j=jsd,jed
               do i=isd,ied
                  wrk1_v2d(i,j,n) = wrk1_v2d(i,j,n)-Velocity%gamma(i,j)*udrho_bt(i,j,n,fstau)*massu_r(i,j)
               enddo
            enddo
         enddo
     endif
...
! </DESCRIPTION>
!
subroutine pred_corr_barotropic_press (Time, Thickness, Velocity, Ext_mode, pme, river)
...
subroutine maximum_convrhoud(Time, Ext_mode)
  ...
  if (abs(convrhoudt0) == convrhoudt .and. abs(convrhoudt0) /= 0.0) then
    ...
    write (stdoutunit,9112) convrhoudt/fudge, iconvrhoudt, jconvrhoudt, &
  ...
  if (abs(convrhoudu0) == convrhoudu .and. abs(convrhoudu0) /= 0.0) then
    ...
    write (stdoutunit,9113) convrhoudu/fudge, iconvrhoudu, jconvrhoudu, &
...
subroutine eta_check(Time, Ext_mode)
  ...
  if (thin_t0 == thin_t_cell) then
    write (stdoutunit,7000) ' Minimum thickness surface T cell = ',thin_t_cell/fudge, &
  ...
  if (thick_t0 == thick_t_cell) then
    write (stdoutunit,7000) ' Maximum thickness surface T cell = ',thick_t_cell/fudge, &
  ...
  if (thin_u0 == thin_u_cell) then
    write (stdoutunit,7000) ' Minimum thickness surface U cell = ',thin_u_cell/fudge, &
  ...
  if (thick_u0 == thick_u_cell) then
    write (stdoutunit,7000) ' Maximum thickness surface U cell = ',thick_u_cell/fudge, &

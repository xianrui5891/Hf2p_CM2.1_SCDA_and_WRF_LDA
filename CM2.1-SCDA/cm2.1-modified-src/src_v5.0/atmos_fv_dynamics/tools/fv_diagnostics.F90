...
! --- tracer manager ---
 ...
 integer, save ::  id_ps,   id_ua,   id_va,   id_wa,  id_ta,   id_pt,   &
                   ...
                   id_u300, id_v300, id_u500, id_v500, id_tm, id_vort850,  &
                   id_h850, id_h300, id_dnflux, id_upflux, id_area
...
! 300 mb Height
!--------------
    id_h300 = register_diag_field ( mod_name, 'h300', axes(1:2),  Time,   &
...
! 850 mb Height
!--------------
    id_h850 = register_diag_field ( mod_name, 'h850', axes(1:2),  Time,   &
         '850-mb hght', 'm', missing_value=missing_value )
!-----------------------------
! mean temp between 300-500 mb
!-----------------------------
   id_tm = register_diag_field (mod_name, 'tm', axes(1:2),  Time,   &
...
! 850-mb vorticity
!--------------------------
    id_vort850 = register_diag_field ( mod_name, 'vort850', axes(1:2), Time,       &
     '850-mb vorticity', '1/s', missing_value=missing_value )
!--------------------------
! 300-mb winds:
!--------------------------
  id_u300 = register_diag_field ( mod_name, 'u300', axes(1:2), Time,       &
                           '300-mb u-wind', '1/s', missing_value=missing_value )
  id_v300 = register_diag_field ( mod_name, 'v300', axes(1:2), Time,       &
...
!$omp private(i, j, k)
   ...
   if(id_vort850>0) then
             call interpolate_vertical(im, jfirst, jlast, km,   &
                                       850.e2, peln, omga, a2)
...
!$omp private(i, j, k)
 ...
 ! 300-mb
    if ( id_u300>0 ) then
      ...
      call interpolate_vertical(im, jfirst, jlast, km,   &
               300.e2, peln, ua(beglon:endlon,beglat:endlat,:), a2)
      used=send_data(id_u300, a2, Time)
      call fv_array_sync()
   endif
   if ( id_v300>0 ) then

      call interpolate_vertical(im, jfirst, jlast, km,   &
               300.e2, peln, va(beglon:endlon,beglat:endlat,:), a2)
      used=send_data(id_v300, a2, Time)
      call fv_array_sync()
   endif
...
! Cumpute SLP (pressure at height=0)
      if(id_slp > 0) then
         ...
         ! The following sync call is required to ensure that send_data completes before 
         ! array a2 is touched by another thread. 
         if( id_h500>0 .or. id_h300>0 .or. id_h850>0 ) call fv_array_sync()
      endif

! Compute H500 and/or H850
      if(id_h500 > 0 .or. id_tm >0 .or. id_h300 > 0 ) then
         log_p(1)  = log( 1000. )
         log_p(2)  = log( 5000. )
         log_p(3)  = log( 10000. )
         log_p(4)  = log( 20000. )
         ...
         log_p(9)  = log( 85000. )
         log_p(10) = log(100000. )
         call get_height_given_pressure(im, jfirst, jlast, km, wz, 10, log_p, a3)
         if(id_h500>0) used = send_data ( id_h500, a3(beglon:endlon,beglat:endlat,7), Time )
         if(id_h850>0) used = send_data ( id_h850, a3(beglon:endlon,beglat:endlat,9), Time )
         if(id_h300>0) used = send_data ( id_h300, a3(beglon:endlon,beglat:endlat,6), Time )
      endif
     ! mean temp 300mb to 500mb
      if( id_tm>0 ) then
         do j=jsp,jep
         do i=1,im
            a2(i,j) = grav*(a3(i,j,6)-a3(i,j,7))/(rdgas*(log_p(7)-log_p(6)))
         enddo
         enddo
...
!! x1y copyed this from cubic fv core and modified for am2 mpi
subroutine interpolate_vertical(im,jfirst,jlast, km, plev, peln, a3, a2)

 integer,  intent(in):: im, jfirst, jlast, km
 real, intent(in):: peln(im,km+1,jfirst:jlast)
 real, intent(in):: a3(im,jfirst:jlast,km)
 real, intent(in):: plev
 real, intent(out):: a2(im,jfirst:jlast)
! local:
 real pm(km)
 real logp
 integer i,j,k

...
! --- FMS ----
   ...
   use mpp_domains_mod, only: mpp_define_domains, mpp_domains_init, domain2D, &
        mpp_get_compute_domain, mpp_get_data_domain !lulv add mpp_get_data_domain 20200819
...
! Default case for the shallow water test: Rossby wave-4
   ...
   character(len=24) :: restart_read_format = 'native'   ! native or netcdf
   character(len=24) :: restart_write_format = 'netcdf'   ! native or netcdf
   ...
   namelist /fv_core_nml/nlon,      mlat,     nlev,                           &
                         ...
                         restart_read_format,  restart_write_format,          &
...
!-----------------------------------------------------------------
   ...
   integer beglond            ! beginning lon index lulv add 20200819
   integer endlond            ! ending    lon index lulv add 20200819
   integer beglatd            ! beginning lat index lulv add 20200819
   integer endlatd            ! ending    lat index lulv add 20200819
...
!PUBLIC VARIABLES
   public :: nlon, mlat, nlev, beglon, endlon, beglat, endlat, &
        ...
        change_time, grid_weight, restart_read_format, restart_write_format
#ifdef USE_LIMA
...
!set some variables from SPMD correctly...
      ...
      call mpp_get_data_domain( fv_domain, beglond, endlond, beglatd,endlatd ) !lulv add 20200819

...
module oda_driver_ecda_mod
  ...
  use time_manager_mod, only : time_type, decrement_time, increment_time, get_time, get_date !lulv add get_data 20201108
  ...
  use oda_core_ecda_tra_mod, only : oda_core_init_tra, get_obs_tra !lulv 20210629
  use eakf_oda_mod, only : ensemble_filter, ensemble_filter_sfc
  use eakf_oda_mod, only : ensemble_filter_sfc_hea !lulv 20201127
  ...
  integer :: id_ens_t_stn = -1 !lulv 20201118
  integer :: id_ens_s_stn = -1 !lulv 20201118
  integer :: id_ens_t_lfq = -1 !lulv 20201118
  integer :: id_ens_s_lfq = -1 !lulv 20201118
  integer :: id_ens_t_hfq = -1 !lulv 20201118
  integer :: id_ens_s_hfq = -1 !lulv 20201118
  integer :: id_corr_u = -1
  integer :: id_corr_v = -1
  ...
  logical :: hfq_first_20days = .false. !lulv 20201122
  ...
  type(ocean_profile_type) :: Profiles_tra(max_profiles) !lulv 20210629 for tra_EnKF
  ...
  !lulv add below
  type(domain2d), private, save :: Hea_Filter_domain 
  ...
  type(field_type) :: Tracer_lfq_clim(max_prog_tracers, max_ensemble_size) !lulv 20201105 
  type(field_type) :: Tracer_hfq_clim(max_prog_tracers, 365) !lulv 20201108
  type(field_type) :: T_ens(max_prog_tracers, max_ensemble_size) !lulv add 2020-04-26
  type(field_type) :: T_ens_day(max_prog_tracers, max_ensemble_size) !lulv add 20201108
  !lulv add above
...
!  type(obs_clim_type) :: sst_climo
  type(obs_clim_type) :: sst_climo_hea !lulv 20210629 for hea_EnKF
  type(obs_clim_type) :: sst_climo_tra !lulv 20210629 for tra_EnKF
...
!-----------------------------------------------------------------------
    ...
    namelist /oda_nml/ assim_method, assim_layout, assim_prog_tracers,&
         ...
         & lfq_filter, hfq_filter, HeEnKF, hea_ens_size, hfq_first_20days !lulv 20201122
    ...
    time_s = decrement_time(Ocn_Time%model_time, 0, 10) !lulv 20201217
    time_e = increment_time(Ocn_Time%model_time, 0, 740)!lulv 20201217
...
!lulv change layout to Domain_ocn%layout
       call mpp_define_domains((/1,ni,1,nj/), Domain_ocn%layout, Hea_Filter_domain, &
           & xflags=Domain_ocn%xflags, yflags=Domain_ocn%yflags, &
           & xhalo = filter_halo_x, yhalo = filter_halo_y, name='hea_filter') !lulv 20210613 filter to hea_filter 
...
!    & mpp_pe(),isc_flth,iec_flth,jsc_flth,jec_flth,isd_flth,ied_flth,jsd_flth,jed_flth !lulv
       ...
       do n = 1, num_prog_tracers
         ...
         allocate(Tracer_lfq_clim(n,m)%data(isd_flth:ied_flth,jsd_flth:jed_flth,Grid%nk)) !lulv 20201105
         allocate(Tracer_ens_hfq(n,m)%data(isd_flth:ied_flth,jsd_flth:jed_flth,Grid%nk)) !lulv add 20180323
         allocate(T_ens(n,m)%data(isd_flth:ied_flth,jsd_flth:jed_flth,Grid%nk)) !lulv add 2020/04/29
       enddo
       enddo
 
       do m = 1, 31 !lulv change 30 to 31 20201117
        do n = 1, num_prog_tracers
         allocate(T_ens_day(n,m)%data(isd_flth:ied_flth,jsd_flth:jed_flth,Grid%nk)) !lulv 20201111
         T_ens_day(n,m)%data(isd_flth:ied_flth,jsd_flth:jed_flth,Grid%nk) = 0.0 !lulv 201111
        enddo
       enddo
       do m=1,365
        do n = 1, num_prog_tracers
          allocate(Tracer_hfq_clim(n,m)%data(isd_flth:ied_flth,jsd_flth:jed_flth,Grid%nk)) !lulv 20201108
        enddo
       enddo
       !do n = 1, num_prog_tracers
    ...
    enddo
       ...
       if (lfq_filter) then
         do m = 1, hea_ens_size
           do n = 1, num_prog_tracers
             if (n == 1) then
               call read_data('INPUT/ocean_ens.nc','ens_t_lfq',work,Domain_ocn%domain2d,timelevel=m)
               Tracer_ens_lfq(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:)
             end if
             if (n == 2) then
               call read_data('INPUT/ocean_ens.nc','ens_s_lfq',work,Domain_ocn%domain2d,timelevel=m)
               Tracer_ens_lfq(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:)
             end if
           end do
         end do

         do m = 1, 12 !lulv 20201119
           do n = 1, num_prog_tracers
             if (n == 1) then
               call read_data('INPUT/tempc.nc','temp_c',work,Domain_ocn%domain2d,timelevel=m)
               Tracer_lfq_clim(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:) !lulv 20201105
             end if
             if (n == 2) then
               call read_data('INPUT/saltc.nc','salt_c',work,Domain_ocn%domain2d,timelevel=m)
               Tracer_lfq_clim(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:) !lulv 20201105
             end if
           enddo
         enddo
         do m = 1, hea_ens_size
           do n= 1, num_prog_tracers
             ...
             call mpp_update_domains(Tracer_lfq_clim(n, m)%data(:,:,:), Hea_Filter_domain) !lulv 20201105
           enddo
         enddo
       end if ! lfq_filter

     if (hfq_filter) then !lulv add hfq_filter if-block 20201108
       do m = 1, hea_ens_size
         do n = 1, num_prog_tracers
           if (n == 1) then !lulv add if-bolck 20201117
             call read_data('INPUT/ocean_ens.nc','ens_t_hfq',work,Domain_ocn%domain2d,timelevel=m)
             Tracer_ens_hfq(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:)
           end if !add domain information lulv
           if (n == 2) then !lulv add if-bolck 20201117
             call read_data('INPUT/ocean_ens.nc','ens_s_hfq',work,Domain_ocn%domain2d,timelevel=m)
             Tracer_ens_hfq(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:)
           end if
           call mpp_update_domains(Tracer_ens_hfq(n, m)%data(:,:,:), Hea_Filter_domain) 
         enddo
       enddo
       ...
       do m = 1, 365
         do n = 1, num_prog_tracers
           if (n == 1) then
             call read_data('INPUT/TS_daily_clim.nc','tempc',work,Domain_ocn%domain2d,timelevel=m)
             Tracer_hfq_clim(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:) 
           end if
           if (n == 2) then
             call read_data('INPUT/TS_daily_clim.nc','saltc',work,Domain_ocn%domain2d,timelevel=m)
             Tracer_hfq_clim(n,m)%data(isc:iec,jsc:jec,:) = work(isc:iec,jsc:jec,:) 
           end if
           call mpp_update_domains(Tracer_hfq_clim(n, m)%data(:,:,:), Hea_Filter_domain) 
         end do
       end do
     end if ! hfq_filter
!lulv add above
...
!lulv 2020-02-25 if ( T_grid_hea%y(i,j) < assim_start_lat .or. T_grid%y(i,j) > assim_end_lat ) then
       ...
       !allocate(sst_climo%sst_obs(isd_filt:ied_filt, jsd_filt:jed_filt)) !lulv 20210622
       allocate(sst_climo_hea%sst_obs(isd_flth:ied_flth, jsd_flth:jed_flth)) !lulv 20210622
       sst_climo_hea%sst_obs(:,:) = 0.0
       allocate(sst_climo_tra%sst_obs(isd_filt:ied_filt, jsd_filt:jed_filt)) !lulv 20210629
       sst_climo_tra%sst_obs(:,:) = 0.0
...
!       call oda_core_init(Domain_ocn%domain2d, T_grid, time_s, time_e, Filter_domain, localize=.false.)       
       ...
       call oda_core_init_tra(Domain_ocn%domain2d, T_grid, time_s, time_e, Filter_domain, localize=.false.) !lulv 20200629    
...
! add id_corr_t_stn lulv 
    !lulv 20201118 start
    id_ens_t_stn = register_diag_field('oda', 'ens_temp_stn', Grd%tracer_axes(1:3),&
         & Time, 'ensemble for stn temperature', 'deg_C', missing_value=-9999., range=(/-50.,50./))
    id_ens_s_stn = register_diag_field('oda', 'ens_salt_stn', Grd%tracer_axes(1:3),&
         & Time, 'ensemble for stn salinity', 'g/kg', missing_value=-9999., range=(/-50.,50./))
    id_ens_t_lfq = register_diag_field('oda', 'ens_temp_lfq', Grd%tracer_axes(1:3),&
         & Time, 'ensemble for lfq temperature', 'deg_C', missing_value=-9999., range=(/-50.,50./))
    id_ens_s_lfq = register_diag_field('oda', 'ens_salt_lfq', Grd%tracer_axes(1:3),&
         & Time, 'ensemble for lfq salinity', 'g/kg', missing_value=-9999., range=(/-50.,50./))
    id_ens_t_hfq = register_diag_field('oda', 'ens_temp_hfq', Grd%tracer_axes(1:3),&
         & Time, 'ensemble for hfq temperature', 'deg_C', missing_value=-9999., range=(/-50.,50./))
    id_ens_s_hfq = register_diag_field('oda', 'ens_salt_hfq', Grd%tracer_axes(1:3),&
         & Time, 'ensemble for hfq salinity', 'g/kg', missing_value=-9999., range=(/-50.,50./))
    !lulv 20201118 end
  end subroutine init_oda

!  subroutine oda(Ocn_Time, T_prog, Velocity, flux, Ext_mode)
  subroutine oda(Ocn_Time, T_prog, Velocity, flux, Ext_mode, ocn_end_flag) !lulv 20201118
    ...
    integer :: kl
    integer, intent(in) :: ocn_end_flag !lulv 20201118

    integer :: assim_freq_day = 0
    integer :: i_steps, total_steps
    integer :: m, n, nprof, itt, tau, taup1, nprof_tra !lulv 20210629
    integer :: second0, day0, ii0, jj0, ii, jj, kk, i, j, k
    integer :: no_prf, no_prf_tra, no_sst, no_eta, no_suv, no_sst_tra !lulv 20210629
...
!!$    integer :: num_obs_sice_dn
    ...
    no_prf = 0
    no_prf_tra = 0 !lulv 20210629
    no_sst = 0
    no_sst_tra = 0 !lulv 20210629
    no_eta = 0
    no_suv = 0
    ...
    assim_time = .false.
    if(lfq_filter)call mk_ens_lfq(time,T_prog,Tracer_ens_lfq,itt,taup1) !lulv 20201105
    if(hfq_filter)call mk_ens_hf(time,T_prog,Tracer_ens_hfq,itt,taup1) !lulv
    ...
    if ( mod(second0,assim_frequency*3600) == 0 ) then
       ...
       case (EAKF)
          ...
          if ( (use_sst_as_obs) .and. (use_ssh_as_obs) .and. (use_suv_as_obs) ) then
             call get_obs_sst(time, Profiles, no_sst, no_prf, sst_climo_hea, Filter_domain)
...
!             call get_obs_sst(time, Profiles, no_sst, no_prf, sst_climo, Filter_domain) !lulv
             call get_obs_sst(time, Profiles, no_sst, no_prf, sst_climo_hea, hea_Filter_domain) !lulv 20210622
             nprof = no_prf + no_sst
          end if
...
!-----------------------------------------
!lulv add below
          call mpp_update_domains(sst_climo_hea%sst_obs(:,:), Hea_Filter_domain) !lulv 20201130
        if (stn_filter) then
        do m = 1, hea_ens_size
        ...
        if ( sub_sfc_filtering) then
         ...
         do m = 1, hea_ens_size
           call mpp_update_domains(Tracer_eHe_uflx(m)%data(:,:,:), Hea_Filter_domain)
           call mpp_update_domains(Tracer_eHe_vflx(m)%data(:,:,:), Hea_Filter_domain)
           do n = 1, num_prog_tracers
             call mpp_update_domains(Tracer_eHe_tau(n, m)%data(:,:,:), Hea_Filter_domain)
           enddo
         enddo
         ...
         if (sst_filtering) then !lulv 20201127 add
           call ensemble_filter_sfc_hea(Tracer_ens_stn(1:num_prog_tracers,1:hea_ens_size),&
                & Tracer_eHe_tau(1,1:hea_ens_size), Tracer_eHe_tau(2,1:hea_ens_size), &
                & Tracer_eHe_uflx(1:hea_ens_size), Tracer_eHe_vflx(1:hea_ens_size), sst_climo_hea,&
                & isc_flth,iec_flth,jsc_flth,jec_flth, filter_halo_x, filter_halo_y, T_grid_hea, time)
...
!        if ((lfq_filter) .and. (mod(day,1) == 0)) then
        ...
        if ( sub_sfc_filtering) then
         ...
         if (sst_filtering) then !lulv 20201127 add
           call ensemble_filter_sfc_hea(Tracer_ens_lfq(1:num_prog_tracers,1:hea_ens_size),&
                & Tracer_eHe_tau(1,1:hea_ens_size), Tracer_eHe_tau(2,1:hea_ens_size), &
                & Tracer_eHe_uflx(1:hea_ens_size), Tracer_eHe_vflx(1:hea_ens_size), sst_climo_hea,&
                & isc_flth,iec_flth,jsc_flth,jec_flth, filter_halo_x, filter_halo_y, T_grid_hea, time)
         end if ! finishing sst_filtering
...
! hfq_filter
        ...
        if (hfq_filter .and. (.not. hfq_first_20days)) then !lulv 20201122
          do m=1,hea_ens_size
            Tracer_eHe_uflx(m)%data(isc:iec,jsc:jec,1) = flux%u_flux(isc:iec,jsc:jec) !lulv
            Tracer_eHe_vflx(m)%data(isc:iec,jsc:jec,1) = flux%v_flux(isc:iec,jsc:jec) !lulv
            do n=1,num_prog_tracers
              Tracer_eHe_tau(n,m)%data(isc:iec,jsc:jec,:)=T_prog(n)%field(isc:iec,jsc:jec,:,taup1) !lulv 20201123
              call mpp_update_domains(Tracer_eHe_tau(n, m)%data(:,:,:), Hea_Filter_domain)
            enddo
          enddo
 
        if ( sub_sfc_filtering) then
          call ens_hfq_filter(Tracer_ens_hfq(1:num_prog_tracers,1:hea_ens_size),& !lulv 20201123
               Tracer_eHe_tau(1,1:hea_ens_size), Tracer_eHe_tau(2,1:hea_ens_size), &
               Tracer_eHe_uflx(1:hea_ens_size), Tracer_eHe_vflx(1:hea_ens_size), &
               Profiles, nprof, isc_flth,iec_flth,jsc_flth,jec_flth,filter_halo_x, &
         ...
         if (sst_filtering) then !lulv 20201127 add
           call ensemble_filter_sfc_hea(Tracer_ens_hfq(1:num_prog_tracers,1:hea_ens_size),&
           !call ensemble_filter_sfc(Tracer_ens_hfq(1,1:hea_ens_size),Tracer_ens_hfq(2,1:hea_ens_size),&
                & Tracer_eHe_tau(1,1:hea_ens_size), Tracer_eHe_tau(2,1:hea_ens_size), &
                & Tracer_eHe_uflx(1:hea_ens_size), Tracer_eHe_vflx(1:hea_ens_size), sst_climo_hea,&
                & isc_flth,iec_flth,jsc_flth,jec_flth, filter_halo_x, filter_halo_y, T_grid_hea, time)
         end if ! finishing sst_filtering
        ...
        corr_t_hfq(isc:iec,jsc:jec,1:nk) =&
             & Tracer_eHe_tau(1,1)%data(isc:iec,jsc:jec,:) - T_prog(1)%field(isc:iec,jsc:jec,1:nk,taup1)
        corr_s_hfq(isc:iec,jsc:jec,1:nk) =&
             & Tracer_eHe_tau(2,1)%data(isc:iec,jsc:jec,:) - T_prog(2)%field(isc:iec,jsc:jec,1:nk,taup1)
        call mpp_update_domains(corr_t_hfq(:,:,1:nk), Dom%domain2d, complete=.false.)
        call mpp_update_domains(corr_s_hfq(:,:,1:nk), Dom%domain2d, complete=.true.)

        !end if !lulv comment 20201119
        end if ! hfq_filter

        !if (sst_filtering) then !lulv add 20200926 
        !  call mpp_update_domains(sst_climo%sst_obs(:,:), Hea_Filter_domain)
        !  call ensemble_filter_sfc(Tracer_eHe_tau(1,1:hea_ens_size), Tracer_eHe_tau(2,1:hea_ens_size), &
        !       & Tracer_eHe_uflx(1:hea_ens_size),Tracer_eHe_vflx(1:hea_ens_size), sst_climo, &
        !       & isc_filt,iec_filt,jsc_filt,jec_filt, filter_halo_x, filter_halo_y, T_grid_hea, time)
        !end if ! finishing sst_filtering
...
!----------------------------------------
        if (HeEnKF) then !lulv
          ...
          if ( (use_sst_as_obs) .and. (.NOT.use_ssh_as_obs) .and. (.NOT.use_suv_as_obs) ) then !lulv 20210629
             call get_obs_sst(time, Profiles_tra, no_sst_tra, no_prf_tra, sst_climo_tra, Filter_domain)
          end if
          if ( (use_prf_as_obs) .and. (.NOT.use_woa05_t) .and. (.NOT.use_woa05_s) ) then
             call get_obs_tra(time, Profiles_tra, no_prf_tra)
             nprof_tra = no_prf_tra
          end if
             ...
             ! snz to run EAKF
                ...
                if ( sub_sfc_filtering) then
                   ...
                   call ensemble_filter(temp_ens_tau(1:ensemble_size), salt_ens_tau(1:ensemble_size),&
                        & uflx_ens(1:ensemble_size), vflx_ens(1:ensemble_size), Profiles_tra, nprof_tra,&
...
!             else ! assimilating surface data
                ...
                call mpp_update_domains(sst_climo_tra%sst_obs(:,:), Filter_domain)
                ...
                if (sst_filtering) then
                   ...
                   call ensemble_filter_sfc(temp_ens_tau(1:ensemble_size), salt_ens_tau(1:ensemble_size),&
                        & uflx_ens(1:ensemble_size), vflx_ens(1:ensemble_size), sst_climo_tra, &
...
!            & Time,rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))     !lulv
       ...
       if ( id_corr_ty_hea > 0 ) used = send_data(id_corr_ty_hea,corr_ty_hea(isc:iec,jsc:jec),&!lulv
            & Time,rmask=Grd%umask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,1))     !lulv
      !lulv 20201118 start
       if(ocn_end_flag==1) then
        do m =1,hea_ens_size+1 !fail to use hea_ada_ens_size, not know why,succeseed to plus 1 
         if (stn_filter .and. id_ens_t_stn > 0 ) used = send_data(id_ens_t_stn,Tracer_ens_stn(1,m)%data(isc:iec,jsc:jec,:),& 
                         & increment_time(Time,0,m),rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))  
         if (stn_filter .and. id_ens_s_stn > 0 ) used = send_data(id_ens_s_stn,Tracer_ens_stn(2,m)%data(isc:iec,jsc:jec,:),& 
                         & increment_time(Time,0,m),rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))  
         if (lfq_filter .and. id_ens_t_lfq > 0 ) used = send_data(id_ens_t_lfq,Tracer_ens_lfq(1,m)%data(isc:iec,jsc:jec,:),&
                         & increment_time(Time,0,m),rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))
         if (lfq_filter .and. id_ens_s_lfq > 0 ) used = send_data(id_ens_s_lfq,Tracer_ens_lfq(2,m)%data(isc:iec,jsc:jec,:),&
                         & increment_time(Time,0,m),rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))
         if (hfq_filter .and. id_ens_t_hfq > 0 ) used = send_data(id_ens_t_hfq,Tracer_ens_hfq(1,m)%data(isc:iec,jsc:jec,:),&
                         & increment_time(Time,0,m),rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))
         if (hfq_filter .and. id_ens_s_hfq > 0 ) used = send_data(id_ens_s_hfq,Tracer_ens_hfq(2,m)%data(isc:iec,jsc:jec,:),&
                         & increment_time(Time,0,m),rmask=Grd%tmask(isc-Dom%ioff:iec-Dom%ioff,jsc-Dom%joff:jec-Dom%joff,:))
        enddo
       endif
      !lulv 20201118 start
...
!!$               & domain=Dom%domain2d,append_pelist_name= .true.)
  ...
  subroutine oda_end()
    ...
    if ( associated(sst_climo_hea%sst_obs) ) then
       deallocate(sst_climo_hea%sst_obs)
       nullify(sst_climo_hea%sst_obs)
    end if
    if ( associated(sst_climo_tra%sst_obs) ) then !lulv 20210629
       deallocate(sst_climo_tra%sst_obs)
       nullify(sst_climo_tra%sst_obs)
    end if
  ...
  !lulv add below
  subroutine mk_ens_hf(time,T_prog, T_ens_tau, it_step, taup1)
  !chenyuhu find need add taup1
  type(ocean_prog_tracer_type), intent(in) :: T_prog(:)
  type(field_type), intent(inout) :: T_ens_tau(:, :)
  type(time_type) :: time !lulv 20201108
  integer, dimension(12) :: n_days !lulv 20201108
  integer, intent(in)  :: it_step
  integer  :: n, m,taup1, i, i_a, i_b  !lulv
  integer :: iy0, in0, id0, ih0, im0, is0, day_idx !lulv 20201108

  if(it_step == 0) call mpp_error(FATAL)
  !lulv add 11 lines below 20201108
    n_days = (/ 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 /) ! Days in month
  ...
  if(i_a==12) then
    call get_date(decrement_time(time,60,0), iy0, in0, id0, ih0, im0, is0) 
    if(in0==1) then 
      day_idx = id0
    else 
      day_idx=0
      do i=1, in0-1
        day_idx = day_idx + n_days(i)
      enddo 
      day_idx = day_idx + id0
    endif
    !if(mpp_pe()==0) write(*,'("in oda mk_ens_hf,iy0,in0,id0,ih0,day_idx=",1xi4,1xi2,1xi2,1xi2,1xi4)') iy0,in0,id0,ih0,day_idx
    if(mpp_pe()==0)print*,'in oda mk_ens_hf,in0,id0,ih0,day_idx,i_b=',in0,id0,ih0,day_idx,i_b !lulv 20201109
 
    if(hfq_first_20days .and. i_b<=hea_ens_size) then
       ...
       do n=1, num_prog_tracers
         ...
         enddo
         T_ens_tau(n,i_b)%data(isc:iec,jsc:jec,:) = T_ens_tau(n,i_b)%data(isc:iec,jsc:jec,:) &
                                                    - Tracer_hfq_clim(n,day_idx)%data(isc:iec,jsc:jec,:) !lulv 20201108
       enddo
       if(i_b==hea_ens_size) hfq_first_20days=.false.
    else !i_b > hea_ens_size
    if(mpp_pe()==0)print*,'in oda mk_ens_hf,i_b,day_idx=',i_b,day_idx
      do n=1, num_prog_tracers
        ...
        enddo
        !if(mpp_pe()==0)print*,'in oda mk_ens_hf,in0,id0,day_idx=',in0,id0,day_idx !lulv 20201109
        T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:) = T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:) &
                                                            - Tracer_hfq_clim(n,day_idx)%data(isc:iec,jsc:jec,:) !lulv 20201108
      enddo
    endif
  endif
  ...
  do m = 1, hea_ens_size
    do n= 1, num_prog_tracers
      call mpp_update_domains(T_ens_tau(n, m)%data(:,:,:),Hea_Filter_domain) !lulv 20201108
    enddo
  enddo
   !lulv add 2020-04-26 end
  end subroutine mk_ens_hf
  !lulv add above
  !lulv add below 20201105
  subroutine mk_ens_lfq(time,T_prog, T_ens_tau, it_step, taup1)
  type(ocean_prog_tracer_type), intent(in) :: T_prog(:)
  type(field_type), intent(inout) :: T_ens_tau(:, :)
  type(time_type) :: time
  integer, intent(in)  :: it_step
  integer  :: n, m,taup1, i, i_a, i_b,i_c,i_e  !lulv
  integer  :: iy0,in0,id0, ih0, im0, is0 
  integer, save :: step=0 !lulv 20201117
  integer, dimension(12) :: n_days !lulv 20201117

  n_days = (/ 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 /) ! Days in month

  if(it_step == 0) call mpp_error(FATAL) 
  call get_date(decrement_time(time,60,0), iy0, in0, id0, ih0, im0, is0)
  !it's Feb-1 00:00 instead of Jan-31 24:00 when it_step=48*31, so do decrement
  !for time,let in0=1 when it_step=48*31 lulv 20201117
...
!  if(it_step<=12*30) then 
!    i_c=ceiling(it_step/12.0)
!  else 
!    i_e=ceiling(it_step/12.0)
!    i_c=mod(i_e,30)
!    if(i_c==0) i_c=30
!  endif
  if(i_a==12) then
  if(mpp_pe()==0)print*,'in oda mk_ens_lfq,it_step,in0,id0,ih0=',it_step,in0,id0,ih0 !lulv 20201117
    do n=1, num_prog_tracers
      T_ens_day(n,id0)%data(isc:iec,jsc:jec,:) = 0.0
      do i=1,12 
        T_ens_day(n,id0)%data(isc:iec,jsc:jec,:) = T_ens_day(n,id0)%data(isc:iec,jsc:jec,:)+T_ens(n,i)%data(isc:iec,jsc:jec,:)*1./12
      enddo
    enddo
  endif
  ...
  i_b=it_step-step
  i_c=12*n_days(in0)
  if((it_step-step) == 12*n_days(in0)) then
    if(mpp_pe()==0) print*,'in oda mk_ens_lfq,it_step,step=',it_step,step !lulv 20201117
    do n=1, num_prog_tracers
      do m =1 , hea_ens_size-1
        T_ens_tau(n,m)%data(isc:iec,jsc:jec,:) = T_ens_tau(n,m+1)%data(isc:iec,jsc:jec,:)
      enddo
      T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:) = 0.0
      do i=1,n_days(in0)
        T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:) = T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:)+T_ens_day(n,i)%data(isc:iec,jsc:jec,:)*1./n_days(in0)
      enddo
      T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:) = T_ens_tau(n,hea_ens_size)%data(isc:iec,jsc:jec,:) - Tracer_lfq_clim(n,in0)%data(isc:iec,jsc:jec,:) 
    enddo

    do m = 1, hea_ens_size
      do n= 1, num_prog_tracers
        call mpp_update_domains(T_ens_tau(n, m)%data(:,:,:),Hea_Filter_domain)
      enddo
    enddo
    step = step + 12*n_days(in0) !lulv 20201117
  endif
 end subroutine mk_ens_lfq
  !lulv add above 20201105

...
contains
  ...
  subroutine obs_init(isd_ens, ied_ens, jsd_ens, jed_ens, halox, haloy, Profiles, nprof, max_levels,&
    ...
    integer :: i, k, k0, ii, jj, kk0, blk, i_o, lon_len, ie, kk
...
if ( associated(Profiles(i_o)%data) ) then
if ( Profiles(i_o)%data(1) > 1.e4 .and. (Profiles(i_o)%flag(1)) ) then
  print*,'pe=',mpp_pe(),'isd,ied,jsd,jed=',isd_ens,ied_ens,jsd_ens,jed_ens,'ii(jj)_idx=',Profiles(i_o)%i_index,Profiles(i_o)%j_index
  print*,'pe=',mpp_pe(),'Profiles(',i_o,')%accepted=',Profiles(i_o)%accepted,'Profiles(',i_o,')%data,flag,k_idx=',Profiles(i_o)%data(1:10),Profiles(i_o)%flag(1:10),Profiles(i_o)%k_index(1:10)
end if
end if
...
!if (mpp_pe() ==1213 ) then
!print*,'pe=',mpp_pe(),'num_obs=',num_obs
!end if
...
!!$    write (UNIT=stdout_unit, FMT=*) "Running here 0"
    ...
    do i=1, num_prfs_loc_halo
       ...
       if ( ied_ens == ni .and. ii <= halox ) then ! for rightmost domain, count halox area
          ii = ii + ni
       end if
       if ( isd_ens == 1 .and. ii > ied_ens+halox ) then ! for leftmost domain, count halox area
          ii = ii - ni
       end if
       ...
       do k=1, kk0
          ...
          if ( k0 == 0 ) then
            if (ii >= isd_ens-halox .and. ii < ied_ens+halox .and. &
                jj >= jsd_ens-haloy .and. jj < jed_ens+haloy ) then ! interior
            ...
            elseif (ii == ied_ens+halox .and. jj >= jsd_ens-haloy .and. jj < jed_ens+haloy ) then ! rightmost bound
              state_index(1) = (jj-jsd_ens+haloy)*lon_len + ii-isd_ens+halox + 1
              state_index(2) = state_index(1) 
              state_index(3) = (jj-jsd_ens+haloy+1)*lon_len + ii-isd_ens+halox + 1
              state_index(4) = state_index(3)
            elseif (ii >= isd_ens-halox .and. ii < ied_ens+halox .and. jj == jed_ens+haloy ) then ! northmost bound
              state_index(1) = (jj-jsd_ens+haloy)*lon_len + ii-isd_ens+halox + 1
              state_index(3) = state_index(1)
              state_index(2) = (jj-jsd_ens+haloy)*lon_len + ii-isd_ens+halox + 2
              state_index(4) = state_index(2)
            else ! northmost at the rightmost bound (i.e. the point@ii=ied_ens+halox,jj=jed_ens+haloy) 
              state_index(1) = (jj-jsd_ens+haloy)*lon_len + ii-isd_ens+halox + 1
              state_index(2) = state_index(1)
              state_index(3) = state_index(1)
              state_index(4) = state_index(1)
            end if ! done for handling rightmost and northmost bounds
            ...
            state_index(7) = state_index(3)
            state_index(8) = state_index(4)
          else ! vertical interior. i.e k0 > 1 .and. k0 < nk
            if (ii >= isd_ens-halox .and. ii < ied_ens+halox .and. &
                jj >= jsd_ens-haloy .and. jj < jed_ens+haloy ) then ! interior 
              state_index(1) = (k0-1)*blk + (jj-jsd_ens+haloy)*lon_len + ii-isd_ens+halox + 1
              state_index(2) = (k0-1)*blk + (jj-jsd_ens+haloy)*lon_len + ii-isd_ens+halox + 2
...
!          if ( i_m < 1 .or. i_m > ni .or. j_m < 1 .or. j_m > nj ) then
           if ( i_m < isd_ens-halox .or. i_m > ied_ens+halox .or. j_m < jsd_ens-haloy .or. j_m > jed_ens+haloy ) then
          ...
          if ( low_lat < model_loc%lat .and. model_loc%lat < hi_lat ) then
             ...
             if ( low_lon < 80.0 ) low_lon = low_lon + 360.0
             hi_lon  = olon + close_lon_window/cos(olat0*DEG_TO_RAD)
             if ( hi_lon >= 440.0 ) hi_lon = hi_lon - 360.0
             ...
             if (low_lon < hi_lon) then
                ...
                if ( (model_loc%lon > low_lon .and. model_loc%lon < 440.0) .or.&
                     & (model_loc%lon >= 80.0 .and. model_loc%lon < hi_lon ) ) then
                   num = num + 1
                   list(num) = (j-jsd_ens+haloy)*(ied_ens-isd_ens+2*halox+1) + i-isd_ens+halox+1
                end if
             else

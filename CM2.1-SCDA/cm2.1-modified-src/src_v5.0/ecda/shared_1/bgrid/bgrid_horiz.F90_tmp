module bgrid_horiz_mod
  ...
  use mpp_domains_mod, only: mpp_domains_init, mpp_define_domains, domain1D, domain2D
  use mpp_domains_mod, only: mpp_get_global_domain, mpp_get_data_domain, mpp_get_compute_domain, mpp_get_compute_domains
  use mpp_domains_mod, only: mpp_get_domain_components, mpp_get_pelist, mpp_define_layout, mpp_get_layout
  use mpp_domains_mod, only: CYCLIC_GLOBAL_DOMAIN, mpp_global_sum, BITWISE_EXACT_SUM
  use constants_mod, only: RADIUS, PI, DEG_TO_RAD
  ...
  !------- public interfaces -------
  public  horiz_grid_init, get_horiz_grid_bound, get_horiz_grid_size, update_np, update_sp
  ! public parameters
  public TGRID, VGRID
  ! public types
  public bgrid_type, bgrid_interp_type, horiz_grid_type
  ...
  type bgrid_type
     ...
     real, pointer, dimension(:) :: area=>NULL(), rarea=>NULL()
     real, pointer, dimension(:) :: tph=>NULL(),   tlm=>NULL()
  ...
  !-----------------------------------------------------------------------
  character(len=*), parameter :: VERSION = '$Id: bgrid_horiz.f90,v 11.0 2004/09/28 19:07:28 fms Exp $'
...
contains
  ...
  subroutine horiz_grid_init(Hgrid, nlon, nlat, layout)
    ...
    !-----------------------------------------------------------------------
    real, parameter :: HPI = PI/2 !< half pi (PI/2)

    !------------------- local/private declarations ------------------------
    real, allocatable, dimension(:) :: slat
    real, allocatable, dimension(:,:) :: wt
    integer, allocatable, dimension(:) :: xrows, yrows

    integer :: i, j, ilb, iub, jlb, jub, npes, yflags, nlatv
    integer :: stdlog_unit
    ...
    if ( Hgrid%channel ) then
       write (stdlog_unit,FMT='(A)') 'Channel model option has been selected.'
    end if

    if ( do_fplane_approx ) then
       if ( .not.Hgrid%channel ) call error_mesg('bgrid_horiz_mod::horiz_grid_init',&
            & 'f-plane approximation cannot be used without do_channel=TRUE', FATAL)
       write (stdlog_unit,FMT='(A)') '... the f-plane approximation will be used'
    end if

    if ( do_double_periodic ) then
       ! error check
       if ( .not.do_fplane_approx ) call error_mesg('bgrid_horiz_mod::horiz_grid_init',&
            & 'double periodic option cannot be used without do_fplane_approx=TRUE', FATAL)
       yflags = CYCLIC_GLOBAL_DOMAIN
       nlatv  = nlat
       write (stdlog_unit,FMT='(A)') '... and double periodic boundary conditions will be used'
    else
       yflags = 0
       nlatv  = nlat-1
    end if
    ...
    if ( Hgrid%channel ) then
       write (stdlog_unit,FMT='(4X,A4,F10.3,4X,A4,F10.3)') 'WBD=',wbd,'EBD=',ebd
       write (stdlog_unit,FMT='(4X,A4,F10.3,4X,A4,F10.3)') 'SBD=',sbd,'NBD=',nbd
       if ( do_fplane_approx ) then
          write (stdlog_unit,FMT='(4X,A4,F10.3)') 'FPHD=',fphd
       end if
    end if
    ...
    if ( domain_layout(1)*domain_layout(2) /= npes ) call error_mesg('bgrid_horiz_mod::horiz_grid_init',&
         & 'number of processors requested not compatible with grid', FATAL )
    ...
    call mpp_define_domains( (/1,nlon,1,nlat/), domain_layout,&
         & Hgrid%Tmp%Domain,&
         & xflags=CYCLIC_GLOBAL_DOMAIN, yflags=yflags,&
         & xhalo=1, yhalo=1, name='ATMOSPHERIC (B-GRID) MODEL, temperature grid, halo=1,')
    call mpp_define_domains( (/1,nlon,1,nlat/), domain_layout,&
         & Hgrid%Tmp%Domain_nohalo,&
         & xflags=CYCLIC_GLOBAL_DOMAIN, yflags=yflags, xhalo=0, yhalo=0)
    ...
    call mpp_define_domains( (/1,nlon,1,nlatv/), domain_layout,&
         & Hgrid%Vel%Domain,&
         & xflags=CYCLIC_GLOBAL_DOMAIN, yflags=yflags,&
         & xhalo=1, yhalo=1, xextent=xrows, yextent=yrows)
    call mpp_define_domains( (/1,nlon,1,nlatv/), domain_layout,&
         & Hgrid%Vel%Domain_nohalo,&
         & xflags=CYCLIC_GLOBAL_DOMAIN, yflags=yflags,&
         & xhalo=0, yhalo=0, xextent=xrows, yextent=yrows    )
    ...
    allocate(Hgrid%sinphv(ilb:iub,jlb:jub), Hgrid%tanphv(ilb:iub,jlb:jub))
    ...
    if ( Hgrid%channel ) then
       ...
       Hgrid%sb = sbd*DEG_TO_RAD
       Hgrid%nb = nbd*DEG_TO_RAD
       Hgrid%wb = wbd*DEG_TO_RAD
       Hgrid%eb = ebd*DEG_TO_RAD
       ...
       Hgrid%nb =  +HPI
       Hgrid%wb =   0.0
       Hgrid%eb = 360.0*DEG_TO_RAD
       global_x = .true.
       global_y = .true.
    end if

    Hgrid%dlm = Hgrid%dlmd*DEG_TO_RAD
    Hgrid%dph = Hgrid%dphd*DEG_TO_RAD
    ...
    Hgrid%Tmp%blong(isg-1:ieg+2) = get_grid(nlon+3, Hgrid%wb-Hgrid%dlm, Hgrid%dlm)
    ...
    Hgrid%Tmp%blatg(hsg+1:heg) = get_grid(nlat-1, Hgrid%sb+Hgrid%dph, Hgrid%dph  )
    ! polar halos
    ...
    if ( do_fplane_approx ) then
       ...
       Hgrid%Tmp%dx (hs-1:he+1) = RADIUS * Hgrid%dlm / Hgrid%dph * abs(slat(hs:he+2)-slat(hs-1:he+1))
    end if

    !-------------initialize lat/lon at velocity points---------------------
    Hgrid%Vel%blong(isg-1:ieg+2) = get_grid (nlon+3, Hgrid%wb-0.5*Hgrid%dlm, Hgrid%dlm  )
    ...
    Hgrid%Vel%blatg(vsg:veg+1) = get_grid (nlat, Hgrid%sb+0.5*Hgrid%dph, Hgrid%dph  )
    ! polar halos
    ...
    if ( do_fplane_approx ) then
       ...
       Hgrid%Vel%dx (vs-1:ve+1) = RADIUS * Hgrid%dlm / Hgrid%dph * (slat(vs:ve+2)-slat(vs-1:ve+1))
    end if
    ...
    if ( tph0d > eps .or. tlm0d > eps ) then
       ...
       call trans_latlon(tph0d, tlm0d, Hgrid%Tmp%tlm, Hgrid%Tmp%tph, Hgrid%Tmp%alm, Hgrid%Tmp%aph)
       ! at velocity points
       call trans_latlon(tph0d, tlm0d, Hgrid%Vel%tlm, Hgrid%Vel%tph, Hgrid%Vel%alm, Hgrid%Vel%aph)
    else
       ! no transformation
       do j=jlb, jub
          do i=ilb, iub
    ...
    if ( do_fplane_approx ) then
       ! --- channel model ---
       Hgrid%sinphv = sin(fphd*DEG_TO_RAD)
       Hgrid%tanphv = tan(fphd*DEG_TO_RAD)
    else
       ! --- sphere ---
  ...
  subroutine bgrid_interp_init(Hgrid)
    ...
    real, parameter :: HPI = PI/2

    real, dimension(1:2*Hgrid%nlat+1) :: sph
    real, dimension(-1:2*Hgrid%nlat+2) :: area
    real, dimension(Hgrid%Tmp%jsd:Hgrid%Tmp%jed) :: areah
    real, dimension(Hgrid%Vel%jsd:Hgrid%Vel%jed) :: areav
  ...
  !#######################################################################
  function get_grid(npts, start, space)
    integer, intent(in) :: npts
    real, intent(in) :: start, space
    ! Return
    real, dimension(npts) :: get_grid(npts)

    integer :: j

    !---- compute equally spaced grid ----
    do j=1, npts
       get_grid(j) = start + real(j-1)*space
    end do
  ...
  subroutine trans_latlon(tph0d, tlm0d, tlm, tph, alm, aph)
    ...
    real, dimension(:), intent(in) :: tlm, tph
    real, dimension(:,:), intent(out) :: alm, aph

    real :: tph0, tlm0, stph0, ctph0, ttph0, cc, ee
  ...
  subroutine get_horiz_grid_bound(Hgrid, grid, blon, blat, global)
    ...
    real, dimension(:), intent(out) :: blon, blat
    ...
    case default
       call error_mesg('bgrid_horiz_mod::get_horiz_grid_bound', 'invalid grid', FATAL)
    end select
  ...
  subroutine get_horiz_grid_size ( Hgrid, grid, nlon, nlat, global )
    ...
    case default
       call error_mesg('bgrid_horiz_mod::get_horiz_grid_size', 'invalid grid', FATAL)
    end select
  ...
  subroutine horiz_grid_bound(Grid, blon, blat, global)
    ...
    if ( size(blon) /= ie-is+2 ) call error_mesg('bgrid_horiz_mod::get_horiz_grid_bound',&
         & 'invalid argument dimension for blon', FATAL)

    blon = Grid%blong (is:ie+1)

    !----- define latitudinal grid box edges -----
    if ( size(blat) /= je-js+2 ) call error_mesg('bgrid_horiz_mod::get_horiz_grid_bound',&
  ...
  !#######################################################################
  logical function update_np(Hgrid, grid)
    !   Hgrid = horizontal grid constants
    !   grid  = grid identifier, possible values: TGRID, VGRID
    !
    ...
    select case (grid)
    case (TGRID)
       if ( Hgrid%Tmp%je == Hgrid%Tmp%jeg ) update_np = .true.
    case(VGRID)
       if ( Hgrid%Vel%je == Hgrid%Vel%jeg ) update_np = .true.
    case default
       call error_mesg('bgrid_horix_mod::update_np', 'invalid grid', FATAL)
    end select
  end function update_np

  !#######################################################################
  logical function update_sp(Hgrid, grid)
    !   Hgrid = horizontal grid constants
    !   grid  = grid identifier, possible values: TGRID, VGRID
    !
    ...
    select case (grid)
    case (TGRID)
       if ( Hgrid%Tmp%js == Hgrid%Tmp%jsg ) update_sp = .true.
    case(VGRID)
       if ( Hgrid%Vel%js == Hgrid%Vel%jsg ) update_sp = .true.
    case default
       call error_mesg('bgrid_horiz_mod::update_sp', 'invalid grid', FATAL)
    end select
  ...
  subroutine alloc_array_space(ilb, iub, jlb, jub, Grid)
    ...
    allocate(Grid%blong(Grid%isg-1:Grid%ieg+2), Grid%blatg(Grid%jsg-1:Grid%jeg+2))

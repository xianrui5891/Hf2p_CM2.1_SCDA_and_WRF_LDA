module fms_io_hyb_mod !lulv change fms_io_mod to fms_io_hyb_mod
...
character(len=128) :: tagname = '$Name: mom4_patch20041210 $'
...
!       print*,'lijy pe=',mpp_pe(),'pelist_name=',pelist_name !lijy        
        call mpp_get_current_pelist(pelist,pelist_name)
        if(mpp_pe() .eq.0) pelist_name='_ens01' !lijy add 
        if(mpp_pe() .eq.1) pelist_name='_ens02' !lijy add 
        if(mpp_pe() .eq.2) pelist_name='_ens03' !lijy add 
        if(mpp_pe() .eq.3) pelist_name='_ens04' !lijy add 
        if(mpp_pe() .eq.4) pelist_name='_ens05' !lijy add 
        if(mpp_pe() .eq.5) pelist_name='_ens06' !lijy add 
        if(mpp_pe() .eq.6) pelist_name='_ens07' !lijy add 
        if(mpp_pe() .eq.7) pelist_name='_ens08' !lijy add 
        if(mpp_pe() .eq.8) pelist_name='_ens09' !lijy add 
        if(mpp_pe() .eq.9) pelist_name='_ens10' !lijy add 
        if(mpp_pe() .eq.10) pelist_name='_ens11' !lijy add 
        if(mpp_pe() .eq.11) pelist_name='_ens12' !lijy add 
        if(mpp_pe() .eq.12) pelist_name='_ens13' !lijy add 
        if(mpp_pe() .eq.13) pelist_name='_ens14' !lijy add 
        if(mpp_pe() .eq.14) pelist_name='_ens15' !lijy add 
        if(mpp_pe() .eq.15) pelist_name='_ens16' !lijy add 
        if(mpp_pe() .eq.16) pelist_name='_ens17' !lijy add 
        if(mpp_pe() .eq.17) pelist_name='_ens18' !lijy add 
        if(mpp_pe() .eq.18) pelist_name='_ens19' !lijy add 
!the above line is added by lijy 2017-4-8 just to realise the assignment of
!restart file's name, which is not solve the problem completely 
...
! Initialize files to default values
module_is_initialized=.true. !lulv
...
!if(mpp_pe()==10)print*,'filename=',filename,'pe=',mpp_pe() ! lijy
  ...
  if (PRESENT(append_pelist_name)) then
     if (append_pelist_name) then
        ...
        if(mpp_pe() .eq.18) pelist_name='_ens19' !lijy add 
        fname = trim(filename)//trim(pelist_name)
!print*,'middle fname=',fname,'pe=',mpp_pe()
        deallocate(pelist)
     endif
  endif    

!if(mpp_pe()==10)print*,'lijy filename=',filename,'pelist_name=',pelist_name !lijy
!if(mpp_pe()==10)print*,'after fname=',fname,'append_pelist_name=',append_pelist_name,'pe=',mpp_pe() !lijy
...
! Need to check if filename has been opened or not
 ...
 thread_r = MPP_SINGLE !lulv only for read_data in stn_filter and lfq_filter
 form = MPP_NETCDF !lulv only for read_data in stn_filter and lfq_filter
 fset_r = MPP_SINGLE  !lulv only for read_data in stn_filter and lfq_filter
...
!  !#######################################################################
!lulv comment 1461-1679,because in heEnKF_oda_driver, only use read_data_3d_new
!lulv when complie all , it makes errors which are compiler bug
...
!  integer :: len
!
!  include "hybrid_read_data_2d.inc"  
...
!  integer :: len
!
!  include "hybrid_read_data_3d.inc"
...
! 
!  include "hybrid_read_data_4d.inc"  
...
end module fms_io_hyb_mod !lulv change fms_io_mod to fms_io_hyb_mod

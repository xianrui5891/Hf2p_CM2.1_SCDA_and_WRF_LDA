  ...
  subroutine horiz_interp_bilinear_new_2d ( Interp, lon_in, lat_in, lon_out, lat_out, &
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          ...
          if ( abs(a) .lt. epsln*1.e5) then  ! a = 0 is a linear equation
             ...
             if(abs(y1) < epsln*1.e5) y1 = 0.0
             if(abs(y2) < epsln*1.e5) y2 = 0.0
             if(abs(1.0-y1) < epsln*1.e5) y1 = 1.0
             if(abs(1.0-y2) < epsln*1.e5) y2 = 1.0
             num_solution = 0
...
print*,'lon_out,lat_out=',lon_out,lat_out,'y,y1,y2=',y,y1,y2
print*,'a,b,c,quadra=',a,b,c,quadra
print*,'lon{1-4},lat{1-4}=',lon1,lon2,lon3,lon4,lat1,lat2,lat3,lat4
           ...
           if(abs(y) < epsln*1.e5) y = 0.0
           if(abs(1.0-y) < epsln*1.e5) y = 1.0
           x = (lon-b1*y-d1)/(a1+c1*y)
           if(abs(x) < epsln*1.e5) x = 0.0
           if(abs(1.0-x) < epsln*1.e5) x = 1.0
...
!               "horiz_interp_bilinear_mod: weight should be between 0 and 1")
           if( x>1.0 .or. x<0.0 .or. y>1.0 .or. y < 0.0)then
!print*,'lon_out,lat_out=',lon_out,lat_out,'x,y=',x,y
!print*,'lon{1-4},lat{1-4}=',lon1,lon2,lon3,lon4,lat1,lat2,lat3,lat4
             if (x > 1.0) x = 1.0
             if (y > 1.0) y = 1.0

...
subroutine ensemble_filter_sice_dn(u_ens_tau, v_ens_tau, t_ens_tau, s_ens_tau, &
             c_ens_ice, u_ens_ice, v_ens_ice, m_ens_ice, t1_ens_ice, t2_ens_ice, &
             sice_obs, num_obs,isd, ied, jsd, jed, halox, haloy, nk, nk_ice, &
             T_grid, iass, m_time)
...
logical :: ass_ti = .false., ti_impact_t1 = .false., ti_impact_t2 = .false.
...
logical :: ui_impact_t = .false., ui_impact_u = .false., ui_impact_v = .false., &
           ui_impact_s = .false., ui_impact_ui = .false., ui_impact_vi = .false.
logical :: vi_impact_t = .false., vi_impact_u = .false., vi_impact_v = .false., &
           vi_impact_s = .false., vi_impact_ui = .false., vi_impact_vi = .false.
...
namelist /eakf_ida_dn_nml/cutoff_vd, sigma_o_ti, sigma_o_ui, sigma_o_vi, &
	  ...
	  sice_ass_lat, ass_ti, ti_impact_t1, ti_impact_t2, ti_impact_t, &
	  ti_impact_uv, ti_impact_s, ass_uv, ui_impact_ui, ui_impact_vi, &
	  ui_impact_t, ui_impact_u, ui_impact_v, ui_impact_s, &
	  vi_impact_ui, vi_impact_vi, vi_impact_t, vi_impact_u, vi_impact_v, &
	  vi_impact_s, std_oi_fraction, time_window
...
integer, intent(in) :: isd, ied, jsd, jed, halox, haloy, nk, nk_ice
...
real :: cov_factor0, cov_factor, cov_factor_v, cov_factor_t, cov_factor_h
...
integer :: i_o, j_o, k_o
integer :: i_t, i_s, i_u, i_v, i_t1, i_t2, i_ui, i_vi
...
!--------------------------------------------------------------------------
call init_model_sice(isd_ens, ied_ens, jsd_ens, jed_ens, halox, haloy, nk, &
                     nk_ice, ass_method)
...
! observed ti corrects t
    ...
    cor_oi = cov_factor_h
    cov_factor0 = cov_factor_h * cov_factor_t
    ...
    if (sum(ens(i_t, :)) /= 0.0 .and. cov_factor0 /= 0.0) then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
...
! observed ti corrects t1
    if (ti_impact_t1) then

    ass_variable = 2
    do kk_ens = 1, nk_ice
    i_t1  = blk*nk*4+blk*4 + (kk_ens-1)*blk + i_h
    if (sum(ens(i_t1, :)) /= 0.0 .and. cov_factor0 /= 0.0) then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor = cov_factor_h * cov_factor_t * ramp_coef
      call update_from_obs_inc(enso_ti, obs_inc_eakf_ti, &
           obs_inc_oi_ti, ens(i_t1, :), ens_size, &
           ens_inc, cov_factor, cor_oi, std_oi_o, std_oi_g, &
           ass_method, ass_variable)
      ens(i_t1, :)   = ens(i_t1, :) + ens_inc(:)

      do j_ens = 1, ens_size
        if (ens(i_t1, j_ens) < -40.0 .or. ens(i_t1, j_ens) > 39.0) then
          print*,'ii_ens,jj_ens,kk_ens=',ii_ens,jj_ens,kk_ens,'t_adj=',ens_inc(j_ens), &
            't_ice1=',ens(i_t1, j_ens)
        end if
        if (ens(i_t1, j_ens) < -40.0) ens(i_t1, j_ens) = -40.0
        if (ens(i_t1, j_ens) > 39.0) ens(i_t1, j_ens) = 39.0
      end do
...
! observed ti corrects t2
    if (ti_impact_t2) then

    ass_variable = 2
    do kk_ens = 1, nk_ice
    i_t2  = blk*nk*4+blk*4 + blk*nk_ice + (kk_ens-1)*blk + i_h
    if (sum(ens(i_t2, :)) /= 0.0 .and. cov_factor0 /= 0.0) then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor = cov_factor_h * cov_factor_t * ramp_coef
      call update_from_obs_inc(enso_ti, obs_inc_eakf_ti, &
           obs_inc_oi_ti, ens(i_t2, :), ens_size, &
           ens_inc, cov_factor, cor_oi, std_oi_o, std_oi_g, &
           ass_method, ass_variable)
      ens(i_t2, :)   = ens(i_t2, :) + ens_inc(:)

      do j_ens = 1, ens_size
        if (ens(i_t2, j_ens) < -40.0 .or. ens(i_t2, j_ens) > 39.0) then
          print*,'ii_ens,jj_ens,kk_ens=',ii_ens,jj_ens,kk_ens,'t_adj=',ens_inc(j_ens), &
            't_ice2=',ens(i_t2, j_ens)
        end if
        if (ens(i_t2, j_ens) < -40.0) ens(i_t2, j_ens) = -40.0
        if (ens(i_t2, j_ens) > 39.0) ens(i_t2, j_ens) = 39.0
      end do
...
! observed ti corrects uv
    ...
    if(sum(ens(i_u, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor_v = comp_cov_factor(d_g(kk_ens), cutoff_vd)
...
! observed ti corrects s
    ...
    if(sum(ens(i_s, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor_v = comp_cov_factor(d_g(kk_ens), cutoff_vd)
...
! observed ui corrects ui
    if (ui_impact_ui) then

    ass_variable = 1
    i_ui = blk*nk*4 + blk*4 + blk*nk_ice*2 + i_h
    if(sum(ens(i_ui, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor = cov_factor_h * cov_factor_t * ramp_coef
      call update_from_obs_inc(enso_ui, obs_inc_eakf_ui, &
           obs_inc_oi_ui, ens(i_ui, :), ens_size, &
           ens_inc, cov_factor, cor_oi, std_oi_o, std_oi_g, &
           ass_method, ass_variable)
      ens(i_ui, :)   = ens(i_ui, :) + ens_inc(:) 
    end if

    end if

! observed ui corrects vi
    if (ui_impact_vi) then

    ass_variable = 2
    i_vi = blk*nk*4 + blk*4 + blk*nk_ice*2 + blk + i_h
    if(sum(ens(i_vi, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor = cov_factor_h * cov_factor_t * ramp_coef
      call update_from_obs_inc(enso_ui, obs_inc_eakf_ui, &
           obs_inc_oi_ui, ens(i_vi, :), ens_size, &
           ens_inc, cov_factor, cor_oi, std_oi_o, std_oi_g, &
           ass_method, ass_variable)
      ens(i_vi, :)   = ens(i_vi, :) + ens_inc(:) 
    end if
...
! observed ui corrects v
    ...
    do kk_ens = 1, nk
    i_v = 2* blk * nk + i_h
    if(sum(ens(i_v, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      std_oi_g = 0.0
      std_oi_o = 0.0
      ens_inc(:) = 0.0
      cov_factor_v = comp_cov_factor(d_g(kk_ens), cutoff_vd)
...
! observed vi corrects ui
    if (vi_impact_ui) then
    ...
    if(sum(ens(i_ui, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      ...
      cov_factor = cov_factor_h * cov_factor_t * ramp_coef
      call update_from_obs_inc(enso_vi, obs_inc_eakf_vi, &
           obs_inc_oi_vi, ens(i_ui, :), ens_size, &
           ens_inc, cov_factor, cor_oi, std_oi_o, std_oi_g, &
           ass_method, ass_variable)
      ens(i_ui, :)   = ens(i_ui, :) + ens_inc(:) 
    end if
...
! observed vi corrects vi
    if (vi_impact_vi) then
    ...
    if(sum(ens(i_vi, :)) /= 0.0 .and. cov_factor0 /= 0.0)then
      ...
      cov_factor = cov_factor_h * cov_factor_t * ramp_coef
      call update_from_obs_inc(enso_vi, obs_inc_eakf_vi, &
           obs_inc_oi_vi, ens(i_vi, :), ens_size, &
           ens_inc, cov_factor, cor_oi, std_oi_o, std_oi_g, &
           ass_method, ass_variable)
      ens(i_vi, :)   = ens(i_vi, :) + ens_inc(:) 
    end if

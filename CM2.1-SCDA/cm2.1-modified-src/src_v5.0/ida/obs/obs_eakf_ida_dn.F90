...
private
public take_single_obs_sice, sice_obs_init, sice_obs_end, sice_obs_def, &
       get_close_grids_sice
...
subroutine sice_obs_init(isd_ens, ied_ens, jsd_ens, jed_ens, halox, haloy, &
           sice_obs, num_obs, T_grid, &
	   list_loc_halo_obs, num_prfs_loc_halo)
...
list_loc_halo_obs(:) = 0
do i = 1, num_obs
   ii = sice_obs(i)%i_index_dn
   jj = sice_obs(i)%j_index_dn
...
do i = 1, num_prfs_loc_halo
   ...
   ii = sice_obs(i_o)%i_index_dn
   jj = sice_obs(i_o)%j_index_dn
   ...
   frac_lat = sice_obs(i_o)%j_index_dn - jj
   frac_lon = sice_obs(i_o)%i_index_dn - ii
...
end subroutine sice_obs_init

!------------------------------------------------------------------------

subroutine sice_obs_end()
...
subroutine get_close_grids_sice(obs_loc, isd_ens, ied_ens, jsd_ens, jed_ens, &
...
do i = isd_ens-halox, ied_ens+halox
  ...
  if(low_lat < model_loc%lat .and. model_loc%lat < hi_lat) then
    ...
    elseif (hi_lon < low_lon) then
      ...
      print*,'low_lon=hi_lon=?',low_lon,hi_lon,'in pe ',mpp_pe()
      num = num + 1
      list(num) = (j-jsd_ens+haloy)*(ied_ens-isd_ens+2*halox+1) + i-isd_ens+halox+1
    end if  
...
end subroutine get_close_grids_sice

end module obs_eakf_sice_dn_mod

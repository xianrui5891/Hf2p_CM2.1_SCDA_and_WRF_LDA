...
!#######################################################################
    ...
    subroutine tool_cm2_cda_maininit(num_cpld_calls_o, num_atmos_calls_o)
...
!------ ocean/slow-ice integration loop ------
        ...
        num_cpld_calls_o = num_cpld_calls
        num_atmos_calls_o = num_atmos_calls

        print*,'num_cpld_calls,num_atmos_calls in mainsubs',num_cpld_calls,num_atmos_calls

    end subroutine tool_cm2_cda_maininit

!#######################################################################

    subroutine tool_atmos_step_pre(nc)
...
!        real, intent(inout) :: SSU(360, 200), SSV(360, 200), SST(360, 200)
        ...
        if (check_stocks > 0) then
            if (check_stocks*((ncc - 1)/check_stocks) == ncc - 1 .AND. ncc > 1) then
                call mpp_set_current_pelist()
...
!#######################################################################
    ...
    subroutine tool_atmos_step(U_bot, V_bot, T_bot, P_bot, nc, na, nu_scda)

        real, intent(inout) :: U_bot(144, 90), V_bot(144, 90), T_bot(144, 90), P_bot(144, 90)
        ...
        if (Atm%pe) then
            ...
            if (nu_scda == 1) CBVar%do_scda_flag = .true. 
            if (mod(nc,12) == 7 .and. na == 1 .and. CBVar%do_scda_flag) then ! 20250511snz

                CBVar%ua_bot_adj(:, :) = U_bot(:, :)-CBVar%ua_bot_adj(:, :)
                CBVar%va_bot_adj(:, :) = V_bot(:, :)-CBVar%va_bot_adj(:, :)
                CBVar%ta_bot_adj(:, :) = T_bot(:, :)-CBVar%ta_bot_adj(:, :)
                CBVar%ps_bot_adj(:, :) = P_bot(:, :)-CBVar%ps_bot_adj(:, :)
                U_bot(:, :) = 0.0
                V_bot(:, :) = 0.0
                T_bot(:, :) = 0.0
                P_bot(:, :) = 0.0

            end if ! end of 20250511snz

            print*,'nc,na,nu in mainsubs',nc,na,nu_scda,'mpp_pe()=',mpp_pe()
            if (mpp_pe() == 0) print*,'ua_adj=',CBVar%ua_bot_adj(72, :)
...
!lulv 20200608                call update_atmos_model_down( !Land_ice_atmos_boundary, Atm )
                call update_atmos_model_down(Land_ice_atmos_boundary, Atm, &
                                             atm_step, atm_end_flag, CBVar) !lulv !snz 20250426

! if Atm%{t,u,v}_bot(:,:) are in global domain 20250511snz
            if (mod(nc,12) == 6 .and. na == num_atmos_calls .and. CBVar%abot_data_flag) then
                U_bot(:, :) = CBVar%ua_bot(:, :)
                V_bot(:, :) = CBVar%va_bot(:, :)
                T_bot(:, :) = CBVar%ta_bot(:, :)
                P_bot(:, :) = CBVar%ps_bot(:, :)
               
                CBVar%ua_bot_adj(:, :) = U_bot(:, :)
                CBVar%va_bot_adj(:, :) = V_bot(:, :)
                CBVar%ta_bot_adj(:, :) = T_bot(:, :)
                CBVar%ps_bot_adj(:, :) = P_bot(:, :)
                CBVar%abot_data_flag = .false.
            end if
! end output atm_bot 20250511snz
...
!#######################################################################
    ...
    subroutine tool_ocean_step(SSU, SSV, SST, SSZ, nc, na, nu_scda)

!        integer :: nlno = 360, nlto = 200 ! for global ocean top dimensions
        real, intent(inout) :: SSU(360, 200), SSV(360, 200), SST(360, 200), SSZ(360, 200)
        integer,intent(in) :: nc, na, nu_scda
        ...
        if (Ocean%is_ocean_pe) then
            ...
            if (nu_scda == 1) CBVar%do_scda_flag = .true. 
            if (mod(nc,12) == 7 .and. CBVar%do_scda_flag) then ! 20250511snz

                CBVar%to_top_adj(:, :) = SST(:, :)-CBVar%to_top_adj(:, :)
                CBVar%uo_top_adj(:, :) = SSU(:, :)-CBVar%uo_top_adj(:, :)
                CBVar%vo_top_adj(:, :) = SSV(:, :)-CBVar%Vo_top_adj(:, :)
                CBVar%eta_top_adj(:, :) = SSZ(:, :)-CBVar%eta_top_adj(:, :)
...
!                                        Time_ocean, Time_step_cpld)
            call update_ocean_model(Ice_ocean_boundary, Ocean_state,  Ocean, &
                 Time_ocean, Time_step_cpld, ocn_end_flag, CBVar) !lulv 20201118,20250427snz
...
! snz add following lines for AI scda 20250427
            if (mod(nc,12) == 6 .and. CBVar%otop_data_flag) then

                SST(:, :) = CBVar%to_top(:, :)
                SSU(:, :) = CBVar%uo_top(:, :)
                SSV(:, :) = CBVar%vo_top(:, :)
                SSZ(:, :) = CBVar%eta_top(:, :)
                CBVar%to_top_adj(:, :) = SST(:, :)
                CBVar%uo_top_adj(:, :) = SSV(:, :)
                CBVar%vo_top_adj(:, :) = SSU(:, :)
                CBVar%eta_top_adj(:, :) = SSZ(:, :)

               CBVar%otop_data_flag = .false.
            end if
! end 20250427
            CBVar%do_scda_flag = .false.
        end if
...
!#######################################################################
    ...
    subroutine tool_restart_step(nc)
        ...
        !--------------
        if (do_chksum) call coupler_chksum('MAIN_LOOP+', ncc)
        write (text, '(a,i4)') 'Main loop at coupling timestep=', ncc
...
! snz 20250427 add the following lines for AI scda

        CBVar%abot_data_flag = .false.
        CBVar%otop_data_flag = .false.
        CBVar%do_scda_flag = .false.
        allocate (CBVar%ua_bot(144,90), CBVar%va_bot(144,90), CBVar%ta_bot(144,90), CBVar%ps_bot(144,90))
        allocate (CBVar%ua_bot_adj(144,90), CBVar%va_bot_adj(144,90), CBVar%ta_bot_adj(144,90), CBVar%ps_bot_adj(144,90))
        CBVar%ua_bot(:, :) = 0.0
        CBVar%va_bot(:, :) = 0.0
        CBVar%ta_bot(:, :) = 0.0
        CBVar%ps_bot(:, :) = 0.0
        CBVar%ua_bot_adj(:, :) = 0.0
        CBVar%va_bot_adj(:, :) = 0.0
        CBVar%ta_bot_adj(:, :) = 0.0
        CBVar%ps_bot_adj(:, :) = 0.0
        allocate (CBVar%uo_top(360,200), CBVar%vo_top(360,200), CBVar%to_top(360,200), CBVar%eta_top(360,200))
        allocate (CBVar%uo_top_adj(360,200), CBVar%vo_top_adj(360,200), CBVar%to_top_adj(360,200), CBVar%eta_top_adj(360,200))
        CBVar%uo_top(:, :) = 0.0
        CBVar%vo_top(:, :) = 0.0
        CBVar%to_top(:, :) = 0.0
        CBVar%eta_top(:, :) = 0.0
        CBVar%uo_top_adj(:, :) = 0.0
        CBVar%vo_top_adj(:, :) = 0.0
        CBVar%to_top_adj(:, :) = 0.0
        CBVar%eta_top_adj(:, :) = 0.0

! end snz 20250427 for AI scda

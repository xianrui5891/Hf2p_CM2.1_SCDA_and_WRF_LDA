...
lat_write = ncread(outputFile, 'XLAT_U');
lon_write = ncread(outputFile, 'XLONG_U');
znu_write = ncread(outputFile, 'ZNU');
t_write   = ncread(outputFile, 'U');
dims_write = size(t_write);

% Read the input NetCDF file
lat_read  = ncread(inputFile, 'XLAT_U');
lon_read  = ncread(inputFile, 'XLONG_U');
znu_read  = ncread(inputFile, 'ZNU');
t_read    = ncread(inputFile, 'U');
...
parfor i = 1:20
    i
    T = ncread(inputFile, sprintf('ENS5DAYU%d', i));
...
ntime=size(1)
for i=1:20
    % nccreate(outputFile, sprintf('ENS5DAYU%d', i), 'Dimensions', {'west_east_stag', dims_write(1) , 'south_north', dims_write(2), 'bottom_top', dims_write(3),'Time', ntime(1) });
    t_write(:,:,:,1) = t_out( :, :, :,i);
    ncwrite(outputFile, sprintf('ENS5DAYU%d', i), t_write ) ;
    i
end
...
parfor i = 1:20
    i
    T = ncread(inputFile, sprintf('ENS1DAYU%d', i));
    T_1D = reshape(T, [dims_read(1) * dims_read(2) * dims_read(3), 1]);
    T_1D = double(T_1D);
...
ntime=size(1)
for i=1:20
    % nccreate(outputFile, sprintf('ENS1DAYU%d', i), 'Dimensions', {'west_east_stag', dims_write(1) , 'south_north', dims_write(2), 'bottom_top', dims_write(3),'Time', ntime(1) });
    t_write(:,:,:,1) = t_out( :, :, :,i);
    ncwrite(outputFile, sprintf('ENS1DAYU%d', i), t_write ) ;
    i
end
...
parfor i = 1:20
    i
    T = ncread(inputFile, sprintf('ENS6HU%d', i));
    T_1D = reshape(T, [dims_read(1) * dims_read(2) * dims_read(3), 1]);
    T_1D = double(T_1D);
...
ntime=size(1)
for i=1:20
    % nccreate(outputFile, sprintf('ENS6HU%d', i), 'Dimensions', {'west_east_stag', dims_write(1) , 'south_north', dims_write(2), 'bottom_top', dims_write(3),'Time', ntime(1) });
    t_write(:,:,:,1) = t_out( :, :, :,i);
    ncwrite(outputFile, sprintf('ENS6HU%d', i), t_write ) ;
    i
end
...
parfor i = 1:20
    i
    T = ncread(inputFile, sprintf('ENS3HU%d', i));
    T_1D = reshape(T, [dims_read(1) * dims_read(2) * dims_read(3), 1]);
    T_1D = double(T_1D);
...
ntime=size(1)
for i=1:20
    % nccreate(outputFile, sprintf('ENS3HU%d', i), 'Dimensions', {'west_east_stag', dims_write(1) , 'south_north', dims_write(2), 'bottom_top', dims_write(3),'Time', ntime(1) });
    t_write(:,:,:,1) = t_out( :, :, :,i);
    ncwrite(outputFile, sprintf('ENS3HU%d', i), t_write ) ;
    i
end
